...
| Desktop | `cd desktop/electron && npm run dev` | Frameless Electron, auto-spawns backend |
...
3. **Electron test** — `npm test` in `desktop/electron/` (payload-filter + preload-contract)
