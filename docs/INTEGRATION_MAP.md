# INTEGRATION_MAP.md — AI-Assistant
...
```
            ...
            ▼                             ▼                              ▼
 ┌──────────────────┐        ┌────────────────────────┐      ┌────────────────────┐
 │ /chat/stream     │        │ /api/hermes/chat       │      │ /api/image-gen/*   │
 │ (primary chat)   │        │ (Hermes proxy + bridge)│      │ (multi-provider)   │
 └────────┬─────────┘        └──────────┬─────────────┘      └─────────┬──────────┘
          │                             │                              │
          │             ┌───────────────┴─────────────┐                │
          │             │ core/image_intent.py        │                │
          │             │ (ONLY chatbot importer of   │                │
          │             │  image_pipeline.reasoning)  │                │
          │             └───────────────┬─────────────┘                │
          │                             │ if both flags on             │
          │                             ▼                              │
          │             ┌────────────────────────────┐                 │
          │             │ /api/reasoning-image-gen/* │                 │
          │             │ (multi-panel reasoning)    │                 │
          │             └─────────────┬──────────────┘                 │
          │                           │                                │
          │                           ▼                                │
          │   ┌─────────────────────────────────────────────┐          │
          │   │ image_pipeline.reasoning  (parse → plan →   │          │
          │   │   run_panel → maybe_correct → assemble)     │          │
          │   └────────────────────┬────────────────────────┘          │
          │                        │                                   │
 ┌────────┴────────┐       ┌───────┴──────────┐         ┌──────────────┴─────────┐
 │ /api/anime-     │       │ ComfyUI          │         │ core/image_gen/        │
 │  pipeline/*     │──────▶│ (local, port     │◀────────│ (fal/bfl/replicate/    │
 │ (character-aware│       │  8188 by default)│         │  stepfun + comfyui)    │
 │  layered)       │       └──────────────────┘         └──────────────┬─────────┘
 └────────┬────────┘                                                   │
          │                                                            │
          ▼                                                            ▼
 ┌────────────────────────────────────────────────────────────────────────┐
 │ Storage tiers:                                                         │
 │   storage/character_db/         (registry seed JSON)                   │
 │   storage/character_refs/<tag>/ (downloaded references + seen_urls)    │
 │   storage/character_loras/<tag>/(LoRA cache + meta)                    │
 │   storage/metadata/<job_id>.json(ResultStore manifests — anime path)   │
 │   ComfyUI/output/               (raw images from local ComfyUI)        │
 │   services/chatbot/Storage/Image_Gen/ (multi-provider gallery)         │
 │   storage/{outputs,intermediate,references,prompts}/  (reserved, empty)│
 └────────────────────────────────────────────────────────────────────────┘

       SAA picker (separate process)            Hermes Agent (separate process)
 ┌──────────────────────────────┐         ┌────────────────────────────────────┐
 │ character_select_stand_alone │         │ NousResearch/hermes-agent          │
 │  _app-main/  (Electron)       │         │ HTTP, default :8080                │
 │ port 51028, npm start         │         │ HERMES_ENABLED=true to enable      │
 │ CHARACTER_SELECT_ENABLED=true │         │                                    │
 └──────────────┬───────────────┘         └─────────────────┬──────────────────┘
                │ data/ files read in-proc by                │
                │ image_pipeline/anime_pipeline/             │ HTTP proxy
                │   saa_character_db.py                      ▼
                │ (no HTTP call from chatbot to SAA           core/hermes_adapter.py
...
**Fallback behaviour.**
- `HERMES_ENABLED=false` → 422 with Vietnamese "chưa bật" message.
- Bridge (image redirect) is opt-in: requires `HERMES_ENABLED=true` AND
  ...
  conditions hold:
    1. The message contains an explicit image-generation keyword
       (Vietnamese or English) — see `_IMAGE_KEYWORD_RE` in
...
- Persist images, write to `storage/`, or own job state.
...
ComfyUI. Cycles 1–6 of `image_pipeline/reasoning/`.
...
**Owner / responsibility.** Owns: prompt → panel spec → ComfyUI workflow
→ image bytes. Owns the schema and the correction loop.
...
- ComfyUI unreachable → returns failure descriptor; no retry.
...
- Write into `storage/`. (Today it doesn't. Future curator owns that.)

---

## 3. SAA character picker (sidecar + offline DB)

SAA = `character_select_stand_alone_app-main/`. Two distinct uses inside
the chatbot — keep them separated.
...
**Entry files.**
- Route: [services/chatbot/routes/character_select.py](../services/chatbot/routes/character_select.py) — `/api/character-select/status`, `/api/character-select/url`.
- Adapter: [services/chatbot/core/character_select_adapter.py](../services/chatbot/core/character_select_adapter.py) — HTTP probe only.
...
**Fallback.** Not running → status reports `reachable=false`; chatbot
...
zero data files inside `character_select_stand_alone_app-main/`.

### 3b. SAA offline DB (in-process, always available)

**Purpose.** In-memory access to the SAA data files
(`wai_characters.csv` — 5149 verified WAI SDXL characters,
`danbooru_e621_merged.csv` — tag autocomplete, `wai_character_thumbs.json`).

**Entry files.**
- Module: [image_pipeline/anime_pipeline/saa_character_db.py](../image_pipeline/anime_pipeline/saa_character_db.py) (read-only by chatbot).
...
**Fallback.** SAA folder missing → every function returns empty; rest
of the system continues.

**MUST NOT.** Mutate any file under `character_select_stand_alone_app-main/`.
...
- Topbar buttons: `templates/index.html` → `#characterPickerBtn`,
  `#jobQueueBtn`.
...
**Entry files.**
- Singleton: [services/chatbot/core/character_registry.py](../services/chatbot/core/character_registry.py) — `get_registry()`.
- Route: [services/chatbot/routes/characters.py](../services/chatbot/routes/characters.py) — `/api/characters/*`.
- Data: [storage/character_db/characters.json](../storage/character_db/characters.json),
  [storage/character_db/series_aliases.json](../storage/character_db/series_aliases.json).
...
category) — see `CharacterRecord` dataclass.

**Owner.** Local registry data + alias normalisation +
collision detection (`detect_collisions(display_name)`).

**Fallback.** Missing JSON file → empty registry, warning logged. Search
endpoints with `extended=1` then augment from SAA DB (3b).

**MUST NOT.**
- Be the only character vocabulary — SAA covers long tail.
...
**Entry files.**
- Module: [image_pipeline/anime_pipeline/lora_manager.py](../image_pipeline/anime_pipeline/lora_manager.py).
- Registry: [configs/lora_registry.yaml](../configs/lora_registry.yaml).
...
**Fallback.**
- No CivitAI hit → no LoRA, pipeline runs without one.
- Vision verdict below threshold → file deleted, manifest records
  `verdict: "rejected"`, no LoRA returned.
- Cache hit within TTL → skip network.
...
- Path A — `core/image_gen/providers/comfyui_provider.py` (one provider
  among fal/bfl/replicate/stepfun in the multi-provider router).
- Path B — `image_pipeline/anime_pipeline/comfy_client.py` used by
  `AnimePipelineOrchestrator`.
- Path C — `_ComfyClientAdapter` inside
...
**Fallback.** Timeout, missing `prompt_id`, missing image → return
`success=false` with explicit error string. No silent retry.

**MUST NOT.**
- Be unified into a single client without explicit cross-path testing —
  the three callers have different timeout, polling, and workflow
  conventions today.
- Read or write to `storage/` directly. Output handling belongs to the
  caller.
...
|---|---|---|---|
| Registry seed | `storage/character_db/` | git (manual) | Source of truth for §4 |
| Reference images | `storage/character_refs/<tag>/` | `image_pipeline/anime_pipeline/character_research.py` | Includes `seen_urls.json` (url-hash + sha256 dedupe). Override reuse via `CHAR_RESEARCH_REUSE_REFS=1` |
| Research cache | `storage/character_research/<tag>/research.json` | `character_research.py` | 7-day TTL |
| LoRA cache + meta | `storage/character_loras/<tag>/lora_meta.json` | `lora_manager.py` | 7-day TTL |
| LoRA files | `ComfyUI/models/loras/characters/<tag>/` | `lora_manager.py` | Outside `storage/` |
| Anime job manifest | `storage/metadata/<job_id>.json` | `image_pipeline/anime_pipeline/result_store.py` | Path B only |
| Multi-provider gallery | `services/chatbot/Storage/Image_Gen/` | `core/image_gen/storage.py` | Path A only |
| Video gallery | `services/chatbot/Storage/Video_Gen/` | video routes | Path A video |
...
**Reserved but empty (today):** `storage/outputs/`, `storage/intermediate/`,
`storage/references/`, `storage/prompts/` — only `.gitkeep`. These were
provisioned for a unified layout that has not landed. Do not start
writing into them ad-hoc; they are the curator's future home (§8).
...
**MUST NOT.**
- Hand storage paths around as strings — always use the writer's helper.
...
2. Indexes new artefacts with the canonical character identity (§ Gaps)
...
- Index: `storage/index/<asset_id>.json` (planned location — verify
  before adopting).
...
- **CPU-only.** No vision model, no GPU dependency, no embedding model.
  Identity matching uses canonical IDs (§ Gaps), filename heuristics, and
...
- **No new storage tier.** Build on top of existing `storage/`,
  `ComfyUI/output/`, `services/chatbot/Storage/Image_Gen/`. The reserved
  empty dirs (`storage/{outputs,intermediate,references,prompts}/`) are
...
1. **SAA is integrated as picker / API surface but is not the
   ...
   - Path B (`/api/anime-pipeline/*`): consumes `character_key` via
     `_enrich_with_character` (registry → SAA → `character_nlu`), but
...
2. **Hermes bridge is separate from storage curation.**
   ...
   with the job queue. When/if curator (§8) lands, bridge-routed runs
   must still flow through the same indexing seam as direct
   reasoning-pipeline calls — no special bridge-only path.
...
4. **Duplicate identity / alias / game / scene must be solved with
   ...
   `character_key` — they will widen the drift.
...
- **Before adding a route**, find which numbered subsystem owns the
  behaviour. If two subsystems claim it, that is a §"Current known gaps"
  item and needs a separate decision before code lands.
- **Before adding a storage write**, check § 7. If your tier isn't
  listed, you are probably about to invent an eighth one — stop and
  reread § 8.
- **Before importing `image_pipeline.*` from chatbot code**, check
  § Reasoning and § ComfyUI providers. The only allowed import of
  `image_pipeline.reasoning` from `services/chatbot/` is in
  `core/image_intent.py`.
- **When this file goes stale** (port changes, route rename, env flag
  rename, new subsystem), update it in the same PR — see
