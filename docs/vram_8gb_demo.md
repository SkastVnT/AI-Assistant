...
**Purpose:** Run the same 7-stage anime pipeline flow on an 8 GB VRAM laptop for workflow prototyping. Not a production-quality run — quality trade-offs are intentional.

**Constraint:** `configs/anime_pipeline.yaml` and `configs/models.yaml` are the official 12 GB PC configs and must not be modified.

---

## How it works

`image_pipeline/anime_pipeline/config.py` now reads `ANIME_PIPELINE_CONFIG` env var. If set, it loads that YAML instead of `configs/anime_pipeline.yaml`. All other env overrides (`ANIME_PIPELINE_VRAM_PROFILE`, `ANIME_PIPELINE_COMPOSITION_MODEL`, etc.) still apply on top.
...
```bash
# Windows — PowerShell
$env:ANIME_PIPELINE_CONFIG = "configs/anime_pipeline_laptop.yaml"
...
```bash
# Windows — cmd
set ANIME_PIPELINE_CONFIG=configs/anime_pipeline_laptop.yaml
...
```bash
# Linux / macOS
ANIME_PIPELINE_CONFIG=configs/anime_pipeline_laptop.yaml \
...
Edit `configs/anime_pipeline_laptop.yaml` and change the three `checkpoint:` lines (composition, beauty, final) to one of:
...
All 7 stages run identically — the checkpoint name is just a string:
