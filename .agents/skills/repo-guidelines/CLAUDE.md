...
**Primary streaming endpoint:** `routes/stream.py` → `POST /chat/stream`. Flask is the active chatbot path. Do not reintroduce or merge a FastAPI path without an explicit task.
