...
class BaseModel(torch.nn.Module):
        ...
        **kwargs,
        ...
        return comfy.patcher_extension.WrapperExecutor.new_class_executor(
            ...
            comfy.patcher_extension.get_all_wrappers(
                comfy.patcher_extension.WrappersMP.APPLY_MODEL, transformer_options
            ),
        ...
        **kwargs,
        ...
        if c_concat is not None:
            xc = torch.cat(
                [xc]
                + [
                    comfy.model_management.cast_to_device(c_concat, xc.device, xc.dtype)
                ],
                dim=1,
            )
                    ...
                    if concat_latent_image.shape[-3] < noise.shape[-3]:
                        concat_latent_image = torch.nn.functional.pad(
                            ...
                            (
                                ...
                                0,
                                0,
                                noise.shape[-3] - concat_latent_image.shape[-3],
                            ),
                            "constant",
                            0,
                        )
    ...
    def memory_required(self, input_shape, cond_shapes={}):
        ...
        if (
            ...
            area = sum(
                map(
                    lambda input_shape: input_shape[0] * math.prod(input_shape[2:]),
                    input_shapes,
                )
            )
            return (
...
class IP2P:
    def concat_cond(self, **kwargs):
        ...
        if image.shape[1:] != noise.shape[1:]:
            image = utils.common_upscale(
                image, noise.shape[-1], noise.shape[-2], "bilinear", "center"
            )
...
class SDXL_instructpix2pix(IP2P, SDXL):
    def __init__(self, model_config, model_type=ModelType.EPS, device=None):
        ...
        if model_type == ModelType.V_PREDICTION_EDM:
            self.process_ip2p_image_in = lambda image: (
                comfy.latent_formats.SDXL().process_in(image)
            )  # cosxl ip2p
        else:
...
class AuraFlow(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.aura.mmdit.MMDiT,
        )
...
class HunyuanDiT(BaseModel):
    def __init__(self, model_config, model_type=ModelType.V_PREDICTION, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.hydit.models.HunYuanDiT,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if conditioning_mt5xl is not None:
            out["encoder_hidden_states_t5"] = comfy.conds.CONDRegular(
                conditioning_mt5xl
            )

        attention_mask_mt5xl = kwargs.get("attention_mask_mt5xl", None)
        if attention_mask_mt5xl is not None:
            out["text_embedding_mask_t5"] = comfy.conds.CONDRegular(
...
class PixArt(BaseModel):
    def __init__(self, model_config, model_type=ModelType.EPS, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.pixart.pixartms.PixArtMS,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if width is not None and height is not None:
            out["c_size"] = comfy.conds.CONDRegular(
                torch.FloatTensor([[height, width]])
            )
            out["c_ar"] = comfy.conds.CONDRegular(
                torch.FloatTensor([[kwargs.get("aspect_ratio", height / width)]])
            )
...
class Flux(BaseModel):
    def __init__(
        ...
        device=None,
        unet_model=comfy.ldm.flux.model.Flux,
    ):
    ...
    def concat_cond(self, **kwargs):
        ...
        mask = (
            mask.view(mask.shape[0], mask.shape[2] // 8, 8, mask.shape[3] // 8, 8)
            .permute(0, 2, 4, 1, 3)
    ...
    def extra_conds(self, **kwargs):
        ...
        if attention_mask is not None:
            ...
            if mask_ref_size is not None:
                ...
                attention_mask = utils.upscale_dit_mask(
                    attention_mask, mask_ref_size, (h_tok, w_tok)
                )
    ...
    def extra_conds_shapes(self, **kwargs):
        ...
        if ref_latents is not None:
            out["ref_latents"] = list(
                [1, 16, sum(map(lambda a: math.prod(a.size()[2:]), ref_latents))]
            )
        return out
    ...
    def extra_conds(self, **kwargs):
        ...
        if cross_attn is not None:
            ...
            if cross_attn.shape[1] < target_text_len:
                cross_attn = torch.nn.functional.pad(
                    cross_attn, (0, 0, target_text_len - cross_attn.shape[1], 0)
                )
            out["c_crossattn"] = comfy.conds.CONDRegular(cross_attn)
        return out
...
class GenmoMochi(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        if attention_mask is not None:
            out["attention_mask"] = comfy.conds.CONDRegular(attention_mask)
            out["num_tokens"] = comfy.conds.CONDConstant(
                max(1, torch.sum(attention_mask).item())
            )
        cross_attn = kwargs.get("cross_attn", None)
        if cross_attn is not None:
            out["c_crossattn"] = comfy.conds.CONDRegular(cross_attn)
...
class LTXV(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLUX, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.lightricks.model.LTXVModel,
        )  # TODO
    ...
    def process_timestep(self, timestep, x, denoise_mask=None, **kwargs):
        if denoise_mask is None:
            return timestep
        return self.diffusion_model.patchifier.patchify(
            (
                (denoise_mask)
...
class HunyuanVideo(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.hunyuan_video.model.HunyuanVideo,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if guiding_frame_index is not None:
            out["guiding_frame_index"] = comfy.conds.CONDRegular(
                torch.FloatTensor([guiding_frame_index])
            )

        ref_latent = kwargs.get("ref_latent", None)
        if ref_latent is not None:
            out["ref_latent"] = comfy.conds.CONDRegular(
                self.process_latent_in(ref_latent)
            )
...
class CosmosVideo(BaseModel):
    def __init__(
        self, model_config, model_type=ModelType.EDM, image_to_video=False, device=None
    ):
        super().__init__(
            model_config,
            model_type,
            device=device,
            unet_model=comfy.ldm.cosmos.model.GeneralDIT,
        )
    ...
    def scale_latent_inpaint(self, sigma, noise, latent_image, **kwargs):
        ...
        latent_image = self.model_sampling.calculate_input(
            torch.tensor(
...
class CosmosPredict2(BaseModel):
    def __init__(
        self,
        model_config,
        model_type=ModelType.FLOW_COSMOS,
        ...
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.cosmos.predict2.MiniTrainDIT,
        )
        self.image_to_video = image_to_video
        if self.image_to_video:
            self.concat_keys = ("mask_inverted",)
    ...
    def process_timestep(self, timestep, x, denoise_mask=None, **kwargs):
        ...
        c_noise_B_1_T_1_1 = (
            0.0 * (1.0 - condition_video_mask_B_1_T_1_1)
            + timestep.reshape(timestep.shape[0], 1, 1, 1, 1)
...
class Lumina2(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.lumina.model.NextDiT,
        )
...
class WAN21(BaseModel):
    def __init__(
        ...
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.wan.model.WanModel,
        )
        self.image_to_video = image_to_video
    ...
    def concat_cond(self, **kwargs):
        ...
        if concat_mask_index != 0:
            return torch.cat(
                (image[:, :concat_mask_index], mask, image[:, concat_mask_index:]),
                dim=1,
            )
        else:
    ...
    def extra_conds(self, **kwargs):
        ...
        if clip_vision_output is not None:
            out["clip_fea"] = comfy.conds.CONDRegular(
                clip_vision_output.penultimate_hidden_states
            )

        time_dim_concat = kwargs.get("time_dim_concat", None)
        if time_dim_concat is not None:
            out["time_dim_concat"] = comfy.conds.CONDRegular(
                self.process_latent_in(time_dim_concat)
            )

        reference_latents = kwargs.get("reference_latents", None)
        if reference_latents is not None:
            out["reference_latent"] = comfy.conds.CONDRegular(
...
class WAN21_Vace(WAN21):
    def __init__(
        ...
        super(WAN21, self).__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.wan.model.VaceWanModel,
        )
        self.image_to_video = image_to_video
    ...
    def extra_conds(self, **kwargs):
        ...
        if vace_frames is None:
            noise_shape[1] = 32
            vace_frames = [
                torch.zeros(noise_shape, device=noise.device, dtype=noise.dtype)
            ]
        ...
        if mask is None:
            noise_shape[1] = 64
            mask = [
                torch.ones(noise_shape, device=noise.device, dtype=noise.dtype)
            ] * len(vace_frames)
        ...
        for j in range(len(vace_frames)):
            ...
            vf = torch.cat(
                [vf, mask[j].to(device=noise.device, dtype=noise.dtype)], dim=1
            )
...
class WAN21_Camera(WAN21):
    def __init__(
        ...
        super(WAN21, self).__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.wan.model.CameraWanModel,
        )
        self.image_to_video = image_to_video
...
class WAN21_HuMo(WAN21):
    def __init__(
        ...
        super(WAN21, self).__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.wan.model.HumoWanModel,
        )
        self.image_to_video = image_to_video
    ...
    def extra_conds(self, **kwargs):
        ...
        if "c_concat" not in out:  # 1.7B model
            ...
            if reference_latents is not None:
                out["reference_latent"] = comfy.conds.CONDRegular(
                    self.process_latent_in(reference_latents[-1])
                )
        else:
            noise_shape = list(noise.shape)
            noise_shape[1] += 4
            concat_latent = torch.zeros(
                noise_shape, device=noise.device, dtype=noise.dtype
            )
            zero_vae_values_first = torch.tensor(
                [
                    0.8660,
                    -0.4326,
                    -0.0017,
            ...
            zero_vae_values_second = torch.tensor(
                [
                    1.0869,
                    -1.2370,
                    0.0206,
            ...
            if reference_latents is not None:
                ...
                ref_latent_full = torch.zeros(
                    ref_latent_shape, device=ref_latent.device, dtype=ref_latent.dtype
                )
...
class WAN22_Animate(WAN21):
    def __init__(
        ...
        super(WAN21, self).__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.wan.model_animate.AnimateWanModel,
        )
        self.image_to_video = image_to_video
    ...
    def extra_conds(self, **kwargs):
        ...
        if pose_latents is not None:
            out["pose_latents"] = comfy.conds.CONDRegular(
                self.process_latent_in(pose_latents)
            )
        return out
...
class WAN22_S2V(WAN21):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super(WAN21, self).__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.wan.model.WanModel_S2V,
        )
        self.memory_usage_factor_conds = ("reference_latent", "reference_motion")
        self.memory_usage_shape_process = {
            "reference_motion": lambda shape: [
                shape[0],
                shape[1],
                1.5,
                shape[-2],
    ...
    def extra_conds(self, **kwargs):
        ...
        if reference_motion is not None:
            out["reference_motion"] = comfy.conds.CONDRegular(
                self.process_latent_in(reference_motion)
            )

        control_video = kwargs.get("control_video", None)
        if control_video is not None:
            out["control_video"] = comfy.conds.CONDRegular(
                self.process_latent_in(control_video)
            )
        return out
...
class Hunyuan3Dv2(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.hunyuan3d.model.Hunyuan3Dv2,
        )
...
class Hunyuan3Dv2_1(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.hunyuan3dv2_1.hunyuandit.HunYuanDiTPlain,
        )
...
class HiDream(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.hidream.model.HiDreamImageTransformer2DModel,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if conditioning_llama3 is not None:
            out["encoder_hidden_states_llama3"] = comfy.conds.CONDRegular(
                conditioning_llama3
            )
        image_cond = kwargs.get("concat_latent_image", None)
        if image_cond is not None:
            out["image_cond"] = comfy.conds.CONDNoiseShape(
                self.process_latent_in(image_cond)
            )
        return out
...
class Chroma(Flux):
    def __init__(
        ...
        model_type=ModelType.FLUX,
        device=None,
        unet_model=comfy.ldm.chroma.model.Chroma,
    ):
        super().__init__(model_config, model_type, device=device, unet_model=unet_model)
...
class ChromaRadiance(Chroma):
    def __init__(self, model_config, model_type=ModelType.FLUX, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.chroma_radiance.model.ChromaRadiance,
        )
...
class ACEStep(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.ace.model.ACEStepTransformer2DModel,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        out["speaker_embeds"] = comfy.conds.CONDRegular(
            torch.zeros(noise.shape[0], 512, device=noise.device, dtype=noise.dtype)
        )
        out["lyrics_strength"] = comfy.conds.CONDConstant(
            kwargs.get("lyrics_strength", 1.0)
        )
        return out
...
class Omnigen2(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.omnigen.omnigen2.OmniGen2Transformer2DModel,
        )
        self.memory_usage_factor_conds = ("ref_latents",)
...
class QwenImage(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLUX, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=comfy.ldm.qwen_image.model.QwenImageTransformer2DModel,
        )
        self.memory_usage_factor_conds = ("ref_latents",)
    ...
    def concat_cond(self, **kwargs):
        ...
        if image is None:
            ...
            if noise_augmentation > 0:
                ...
                image = (
                    noise_augmentation * noise
                    + min(1.0 - noise_augmentation, 0.75) * image
                )
            else:
                image = 0.75 * image
        return image
...
class HunyuanVideo15(HunyuanVideo):
    ...
    def concat_cond(self, **kwargs):
        noise = kwargs.get("noise", None)
        extra_channels = (
            self.diffusion_model.img_in.proj.weight.shape[1] - noise.shape[1] - 1
        )  # noise 32 img cond 32 + mask 1
        if extra_channels == 0:
            return None
    ...
    def concat_cond(self, **kwargs):
        ...
        if image is None:
            ...
            if noise_augmentation > 0:
                ...
                generator.manual_seed(kwargs.get("seed", 0) - 10)
                noise = torch.randn(
                    image[:, lq_image_slice].shape,
                    generator=generator,
                    dtype=image.dtype,
                    device="cpu",
                ).to(image.device)
                image[:, lq_image_slice] = (
                    noise_augmentation * noise
                    + min(1.0 - noise_augmentation, 0.75) * image[:, lq_image_slice]
                )
            else:
...
class Kandinsky5(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        if time_dim_replace is not None:
            out["time_dim_replace"] = comfy.conds.CONDRegular(
                self.process_latent_in(time_dim_replace)
            )
