...
class CLIP:
    def __init__(
        ...
        params["device"] = model_options.get(
            ...
            model_management.text_encoder_initial_device(
                load_device,
                offload_device,
                parameters * model_management.dtype_size(dtype),
            ),
        )
    ...
    def encode_from_tokens_scheduled(
        ...
        if all_hooks is None or not self.use_clip_schedule:
            ...
            self.load_model()
            self.cond_stage_model.set_clip_options(
                {"execution_device": self.patcher.load_device}
            )
            all_hooks.reset()
        ...
        if config is None:
            if "decoder.mid.block_1.mix_factor" in sd:
                ...
                if sd["decoder.conv_in.weight"].shape[1] == 64:
                    ...
                    self.memory_used_encode = lambda shape, dtype: (
                        (700 * shape[2] * shape[3]) * model_management.dtype_size(dtype)
                    )
                    self.memory_used_decode = lambda shape, dtype: (
                    ...
                    sd["decoder.conv_in.weight"].shape[1] == 32
                    and sd["decoder.conv_in.weight"].ndim == 5
                ):
                    ddconfig = {
                    ...
                    self.downscale_ratio = (
                        lambda a: max(0, math.floor((a + 3) / 4)),
                        16,
                        16,
                    )
                    ...
                    if "bn.running_mean" in sd:
                        ...
                        self.memory_used_decode = lambda shape, dtype: (
                            old_memory_used_decode(shape, dtype) * 4.0
                        )
                ...
                self.memory_used_encode = lambda shape, dtype: (
                    (1000 * shape[2]) * model_management.dtype_size(dtype)
                )
                self.memory_used_decode = lambda shape, dtype: (
                    (1000 * shape[2] * 2048) * model_management.dtype_size(dtype)
                )
                ...
                self.memory_used_encode = lambda shape, dtype: (
                    (1400 * 9 * shape[-2] * shape[-1])
                    * model_management.dtype_size(dtype)
                )
                self.memory_used_decode = lambda shape, dtype: (
                    (3600 * 4 * shape[-2] * shape[-1] * 16 * 16)
                    * model_management.dtype_size(dtype)
                )
                ...
                self.memory_used_encode = lambda shape, dtype: (
                    (1400 * min(9, shape[2]) * shape[3] * shape[4])
                    * model_management.dtype_size(dtype)
                )
                ...
                self.first_stage_model = (
                    comfy.ldm.cosmos.vae.CausalContinuousVideoTokenizer(**ddconfig)
                )
                ...
                if (
                    ...
                    self.working_dtypes = [torch.bfloat16, torch.float16, torch.float32]
                    self.memory_used_encode = lambda shape, dtype: (
                        3300 * shape[3] * shape[4] * model_management.dtype_size(dtype)
                    )
                    self.memory_used_decode = lambda shape, dtype: (
                        8000
                        * shape[3]
                    ...
                    self.working_dtypes = [torch.bfloat16, torch.float16, torch.float32]
                    self.memory_used_encode = lambda shape, dtype: (
                        (1500 if shape[2] <= 4 else 6000)
                        * shape[3]
                        * shape[4]
                        * model_management.dtype_size(dtype)
                    )
                    self.memory_used_decode = lambda shape, dtype: (
                        (2200 if shape[2] <= 4 else 7000)
                        * shape[3]
                        * shape[4]
                        * (8 * 8)
                        * model_management.dtype_size(dtype)
            ...
            # Hunyuan 3d v2 2.0 & 2.1
                ...
                self.memory_used_encode = (
                    lambda shape, dtype, num_layers=8, kv_cache_multiplier=0: (
                        estimate_memory(shape, dtype, num_layers, kv_cache_multiplier)
                    )
                )

                self.memory_used_decode = (
                    lambda shape, dtype, num_layers=16, kv_cache_multiplier=2: (
                        estimate_memory(shape, dtype, num_layers, kv_cache_multiplier)
                    )
                )
                ...
                self.first_stage_model = (
                    comfy.ldm.ace.vae.music_dcae_pipeline.MusicDCAE(
                        source_sample_rate=44100
                    )
                )
                self.memory_used_encode = lambda shape, dtype: (
                    (shape[2] * 330) * model_management.dtype_size(dtype)
                )
                self.memory_used_decode = lambda shape, dtype: (
                    (shape[2] * shape[3] * 87000) * model_management.dtype_size(dtype)
                )
                ...
                self.first_stage_model = (
                    comfy.pixel_space_convert.PixelspaceConversionVAE()
                )
                self.memory_used_encode = lambda shape, dtype: (
                    (1 * shape[2] * shape[3]) * model_management.dtype_size(dtype)
                )
                self.memory_used_decode = lambda shape, dtype: (
                    (1 * shape[2] * shape[3]) * model_management.dtype_size(dtype)
                )
                ...
                "vocoder.activation_post.downsample.lowpass.filter" in sd
            ):  # MMAudio VAE
                sample_rate = 16000
                ...
                self.first_stage_model = (
                    comfy.ldm.mmaudio.vae.autoencoder.AudioAutoencoder(mode=mode)
                )
                self.memory_used_encode = lambda shape, dtype: (
                    (30 * shape[2]) * model_management.dtype_size(dtype)
                )
                self.memory_used_decode = lambda shape, dtype: (
                    (90 * shape[2] * 1411.2) * model_management.dtype_size(dtype)
                )
                ...
                if self.latent_channels == 48:  # Wan 2.2
                    self.first_stage_model = comfy.taesd.taehv.TAEHV(
                        latent_channels=self.latent_channels, latent_format=None
                    )  # taehv doesn't need scaling
                    self.process_input = lambda image: (_ for _ in ()).throw(
                        NotImplementedError(
                            "This light tae doesn't support encoding currently"
                        )
                    )
                    self.process_output = lambda image: image
                    self.memory_used_decode = lambda shape, dtype: (
                    ...
                    self.latent_channels == 32 and sd["decoder.22.bias"].shape[0] == 12
                ):  # lighttae_hv15
                    self.first_stage_model = comfy.taesd.taehv.TAEHV(
                        latent_channels=self.latent_channels,
                        latent_format=comfy.latent_formats.HunyuanVideo15,
                    )
                    self.process_input = lambda image: (_ for _ in ()).throw(
                        NotImplementedError(
                            "This light tae doesn't support encoding currently"
                    ...
                    if (
                        sd["decoder.1.weight"].dtype == torch.float16
                    ):  # taehv currently only available in float16, so assume it's not lighttaew2_1 as otherwise state dicts are identical
                    ...
                    self.memory_used_encode = lambda shape, dtype: (
                        700
                        * (max(1, (shape[-3] ** 0.66 * 0.11)) * shape[-2] * shape[-1])
                        * model_management.dtype_size(dtype)
                    )
                    self.memory_used_decode = lambda shape, dtype: (
                        50
                        * (
                            max(1, (shape[-3] ** 0.65 * 0.26))
                            * shape[-2]
                            * shape[-1]
                            * 32
                            * 32
        ...
        if pixels.shape[-1] > self.output_channels:
            ...
            if self.pad_channel_value is not None:
                ...
                pixels = torch.nn.functional.pad(
                    pixels,
                    (0, self.output_channels - pixels.shape[-1]),
                    mode=mode,
                    value=value,
                )
        return pixels
...
def load_style_model(ckpt_path):
    ...
    if "style_embedding" in keys:
        model = comfy.t2i_adapter.adapter.StyleAdapter(
...
def load_clip(
    ...
    for p in ckpt_paths:
        sd, metadata = comfy.utils.load_torch_file(
            p, safe_load=True, return_metadata=True
        )
...
def load_text_encoder_state_dicts(
    ...
    if len(clip_data) == 1:
        ...
        if te_model == TEModel.CLIP_G:
            if clip_type == CLIPType.STABLE_CASCADE:
                ...
                clip_target.clip = comfy.text_encoders.sd3_clip.sd3_clip(
                    clip_l=False, clip_g=True, t5=False
                )
                clip_target.tokenizer = comfy.text_encoders.sd3_clip.SD3Tokenizer
            elif clip_type == CLIPType.HIDREAM:
                clip_target.clip = comfy.text_encoders.hidream.hidream_clip(
                    clip_l=False,
                    clip_g=True,
                    t5=False,
                    llama=False,
            ...
            if clip_type == CLIPType.SD3:
                ...
                clip_target.clip = comfy.text_encoders.lt.ltxv_te(
                    **t5xxl_detect(clip_data)
                )
                clip_target.tokenizer = comfy.text_encoders.lt.LTXVT5Tokenizer
            elif clip_type == CLIPType.PIXART or clip_type == CLIPType.CHROMA:
                clip_target.clip = comfy.text_encoders.pixart_t5.pixart_te(
                    **t5xxl_detect(clip_data)
                )
                ...
                clip_target.clip = comfy.text_encoders.genmo.mochi_te(
                    **t5xxl_detect(clip_data)
                )
            ...
            clip_target.clip = comfy.text_encoders.lumina2.te(
                **llama_detect(clip_data), model_type="gemma3_4b"
            )
            ...
            clip_target.clip = comfy.text_encoders.omnigen2.te(
                **llama_detect(clip_data)
            )
            clip_target.tokenizer = comfy.text_encoders.omnigen2.Omnigen2Tokenizer
        elif te_model == TEModel.QWEN25_7B:
            if clip_type == CLIPType.HUNYUAN_IMAGE:
                clip_target.clip = comfy.text_encoders.hunyuan_image.te(
                    byt5=False, **llama_detect(clip_data)
                )
                clip_target.tokenizer = (
                    comfy.text_encoders.hunyuan_image.HunyuanImageTokenizer
                )
            else:
                clip_target.clip = comfy.text_encoders.qwen_image.te(
                    **llama_detect(clip_data)
                )
                clip_target.tokenizer = (
            ...
            te_model == TEModel.MISTRAL3_24B
            or te_model == TEModel.MISTRAL3_24B_PRUNED_FLUX2
        ):
            clip_target.clip = comfy.text_encoders.flux.flux2_te(
                **llama_detect(clip_data),
                pruned=te_model == TEModel.MISTRAL3_24B_PRUNED_FLUX2,
            )
        ...
        if clip_type == CLIPType.SD3:
            ...
            clip_target.clip = comfy.text_encoders.hunyuan_video.hunyuan_video_clip(
                **llama_detect(clip_data)
            )
            clip_target.tokenizer = (
            ...
            clip_target.clip = comfy.text_encoders.kandinsky5.te(
                **llama_detect(clip_data)
            )
            ...
            tokenizer_data["gemma_spiece_model"] = clip_data_gemma.get(
                "spiece_model", None
            )
            tokenizer_data["jina_spiece_model"] = clip_data_jina.get(
                "spiece_model", None
            )
        else:
    ...
    for c in clip_data:
        ...
        tokenizer_data, model_options = (
            comfy.text_encoders.long_clipl.model_options_long_clip(
    ...
    if "parameterization" in model_config_params:
        if model_config_params["parameterization"] == "v":
            ...
            class ModelSamplingAdvanced(
                ...
                comfy.model_sampling.V_PREDICTION,
            ):
                pass
...
def load_state_dict_guess_config(
    ...
    if model_config is None:
        ...
        return (
            ...
            VAE(sd={}),
            None,
        )  # The VAE object is there to throw an exception if it's actually used'
    ...
    if output_clip:
        if te_model_options.get("custom_operations", None) is None:
            ...
            if len(scaled_fp8_list) > 0:
                ...
                for pref in scaled_fp8_list:
                    quant_sd, qmetadata = comfy.utils.convert_old_quants(
                        sd, pref, metadata={}
                    )
                    for k in quant_sd:
                        out_sd[k] = quant_sd[k]
...
def load_unet_state_dict(sd, dtype=None):
    logging.warning(
        "The load_unet_state_dict function has been deprecated and will be removed please switch to: load_diffusion_model_state_dict"
    )
