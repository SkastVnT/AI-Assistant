...
class HiDreamTokenizer:
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        ...
        self.t5xxl = sd3_clip.T5XXLTokenizer(
            embedding_directory=embedding_directory,
            min_length=128,
            max_length=128,
            tokenizer_data=tokenizer_data,
        )
        self.llama = hunyuan_video.LLAMA3Tokenizer(
            embedding_directory=embedding_directory,
            min_length=128,
            pad_token=128009,
            tokenizer_data=tokenizer_data,
...
class HiDreamTEModel(torch.nn.Module):
    def __init__(
        ...
        if llama:
            ...
            self.llama = hunyuan_video.LLAMAModel(
                device=device,
                dtype=dtype_llama,
                model_options=model_options,
                layer="all",
                layer_idx=None,
                special_tokens={"start": 128000, "pad": 128009},
            )
        ...
        logging.debug(
            "Created HiDream text encoder with: clip_l {}, clip_g {}, t5xxl {}:{}, llama {}:{}".format(
                clip_l, clip_g, t5, dtype_t5, llama, dtype_llama
            )
        )
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if t5_out is None:
            t5_out = torch.zeros(
                (1, 128, 4096), device=comfy.model_management.intermediate_device()
            )

        if ll_out is None:
            ll_out = torch.zeros(
                (1, 32, 1, 4096), device=comfy.model_management.intermediate_device()
            )
