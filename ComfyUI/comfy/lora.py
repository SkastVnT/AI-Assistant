...
def model_lora_keys_clip(model, key_map={}):
    ...
    for k in sdk:
        if k.endswith(".weight"):
            key_map["text_encoders.{}".format(k[: -len(".weight")])] = (
        ...
        if k.endswith(".weight"):
            if k.startswith("t5xxl.transformer."):  # OneTrainer SD3 and Flux lora
                ...
                if clip_l_present:
                    ...
                    if t5_index == 2:
                        key_map[
                            "lora_te{}_{}".format(t5_index, l_key.replace(".", "_"))
                        ] = k  # OneTrainer Flux
                        t5_index += 1
...
def model_lora_keys_unet(model, key_map={}):
    ...
    for k in sdk:
        if k.startswith("diffusion_model."):
            if k.endswith(".weight"):
                ...
                key_map["{}".format(k)] = (
                    k  # generic lora format for not .weight without any weird key names
                )
    ...
    if isinstance(model, comfy.model_base.SD3):  # Diffusers lora SD3
        diffusers_keys = comfy.utils.mmdit_to_diffusers(
        ...
        for k in diffusers_keys:
            if k.endswith(".weight"):
                ...
                key_lora = "lycoris_{}".format(
                    k[: -len(".weight")].replace(".", "_")
                )  # simpletuner lycoris format
                key_map[key_lora] = to

    if isinstance(model, comfy.model_base.AuraFlow):  # Diffusers lora AuraFlow
        diffusers_keys = comfy.utils.auraflow_to_diffusers(
            model.model_config.unet_config, output_prefix="diffusion_model."
        )
        for k in diffusers_keys:
            if k.endswith(".weight"):
    ...
    if isinstance(model, comfy.model_base.PixArt):
        diffusers_keys = comfy.utils.pixart_to_diffusers(
            model.model_config.unet_config, output_prefix="diffusion_model."
        )
        for k in diffusers_keys:
            if k.endswith(".weight"):
                ...
                key_lora = "unet.base_model.model.{}".format(
                    k[: -len(".weight")]
                )  # old reference peft script
                key_map[key_lora] = to
    ...
    if isinstance(model, comfy.model_base.Flux):  # Diffusers lora Flux
        diffusers_keys = comfy.utils.flux_to_diffusers(
            model.model_config.unet_config, output_prefix="diffusion_model."
        )
        for k in diffusers_keys:
            if k.endswith(".weight"):
        ...
        for k in sdk:
            ...
            if k.endswith(".weight") and ".linear1." in k:
                key_map["{}".format(k.replace(".linear1.weight", ".linear1_qkv"))] = (
                    k,
                    (0, 0, hidden_size * 3),
                )
    ...
    if isinstance(model, comfy.model_base.GenmoMochi):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(
                ".weight"
            ):  # Official Mochi lora format
                key_lora = k[len("diffusion_model.") : -len(".weight")]
    ...
    if isinstance(model, comfy.model_base.HunyuanVideo):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"):
                ...
                key_lora = k
                key_lora = (
                    key_lora.replace("_mod.lin.", "_mod.linear.")
                    .replace("_attn.qkv.", "_attn_qkv.")
                    .replace("_attn.proj.", "_attn_proj.")
                )
                key_lora = key_lora.replace("mlp.0.", "mlp.fc1.").replace(
                    "mlp.2.", "mlp.fc2."
                )
    ...
    if isinstance(model, comfy.model_base.HiDream):
        for k in sdk:
            if k.startswith("diffusion_model."):
                if k.endswith(".weight"):
                    key_lora = k[len("diffusion_model.") : -len(".weight")]
                    key_map["lycoris_{}".format(key_lora.replace(".", "_"))] = (
                        k  # SimpleTuner lycoris format
                    )
                    key_map["transformer.{}".format(key_lora)] = (
                        k  # SimpleTuner regular format
                    )
    ...
    for p in patches:
                ...
                calculate_weight(
                    ...
                    v[0][1](
                        comfy.model_management.cast_to_device(
                            v[0][0], weight.device, intermediate_dtype, copy=True
                        ),
                        inplace=True,
                    ),
                    key,
        ...
        if patch_type == "diff":
            ...
            if strength != 0.0:
                if diff.shape != weight.shape:
                    ...
                    weight += function(
                        strength
                        * comfy.model_management.cast_to_device(
                            diff, weight.device, weight.dtype
                        )
                    )
        elif patch_type == "set":
            weight.copy_(v[0])
            ...
            diff_weight = comfy.model_management.cast_to_device(
                ...
                original_weights[key][0][0], weight.device, intermediate_dtype
            )
            weight += function(
                strength
                * comfy.model_management.cast_to_device(
                    diff_weight, weight.device, weight.dtype
                )
            )
        else:
