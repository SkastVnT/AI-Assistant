    ...
    def normalize(self, x: torch.Tensor) -> torch.Tensor:
        return (
            ...
            - comfy.model_management.cast_to(
                self.data_mean, dtype=x.dtype, device=x.device
            )
        ) / comfy.model_management.cast_to(
            self.data_std, dtype=x.dtype, device=x.device
        )

    def unnormalize(self, x: torch.Tensor) -> torch.Tensor:
        return x * comfy.model_management.cast_to(
            self.data_std, dtype=x.dtype, device=x.device
        ) + comfy.model_management.cast_to(
            self.data_mean, dtype=x.dtype, device=x.device
        )
