...
class MLPEmbedder(nn.Module):
    def __init__(
        ...
        self.in_layer = operations.Linear(
            in_dim, hidden_dim, bias=bias, dtype=dtype, device=device
        )
        self.silu = nn.SiLU()
...
def build_mlp(
    ...
    if mlp_silu_act:
        return nn.Sequential(
            operations.Linear(
                hidden_size, mlp_hidden_dim * 2, bias=False, dtype=dtype, device=device
            ),
            SiLUActivation(),
            operations.Linear(
                mlp_hidden_dim, hidden_size, bias=False, dtype=dtype, device=device
            ),
        )
    else:
        return nn.Sequential(
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.proj = operations.Linear(
            dim, dim, bias=proj_bias, dtype=dtype, device=device
        )
...
class Modulation(nn.Module):
    def __init__(
        ...
        self.lin = operations.Linear(
            dim, self.multiplier * dim, bias=bias, dtype=dtype, device=device
        )
...
class DoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        if self.flipped_img_txt:
            ...
            attn = attention(
                q, k, v, pe=pe, mask=attn_mask, transformer_options=transformer_options
            )
            del q, k, v
...
class SingleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        if self.yak_mlp:
            mlp = (
                self.mlp_act(mlp[..., self.mlp_hidden_dim_first // 2 :])
                * mlp[..., : self.mlp_hidden_dim_first // 2]
            )
        else:
...
class LastLayer(nn.Module):
    def __init__(
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            operations.Linear(
                hidden_size, 2 * hidden_size, bias=bias, dtype=dtype, device=device
            ),
        )
