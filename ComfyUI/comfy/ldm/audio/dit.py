...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.layer_norm(
            x, x.shape[-1:], weight=comfy.ops.cast_to_input(self.gamma, x), bias=beta
        )
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        freqs = torch.einsum(
            "i , j -> i j", t, comfy.ops.cast_to_input(self.inv_freq, t)
        )
        ...
        ) / self.scale_base  # noqa: F821 seq_len is not defined
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        out = optimized_attention(
            q, k, v, h, skip_reshape=True, transformer_options=transformer_options
        )
        out = self.to_out(out)
        ...
        **kwargs,
        ...
        if self.rotary_pos_emb is not None:
            rotary_pos_emb = self.rotary_pos_emb.forward_from_seq_len(
                x.shape[1], dtype=torch.float, device=x.device
            )
        else:
            rotary_pos_emb = None
