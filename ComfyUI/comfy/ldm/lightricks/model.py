...
class TimestepEmbedding(nn.Module):
    def __init__(
        ...
        self.linear_1 = operations.Linear(
            in_channels, time_embed_dim, sample_proj_bias, dtype=dtype, device=device
        )

        if cond_proj_dim is not None:
            self.cond_proj = operations.Linear(
                cond_proj_dim, in_channels, bias=False, dtype=dtype, device=device
            )
        else:
            self.cond_proj = None
...
class AdaLayerNormSingle(nn.Module):
    ...
    def __init__(
        ...
        self.linear = operations.Linear(
            embedding_dim, 6 * embedding_dim, bias=True, dtype=dtype, device=device
        )
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        self.q_norm = operations.RMSNorm(
            inner_dim, eps=1e-5, dtype=dtype, device=device
        )
        ...
        self.to_q = operations.Linear(
            query_dim, inner_dim, bias=True, dtype=dtype, device=device
        )
        self.to_k = operations.Linear(
            context_dim, inner_dim, bias=True, dtype=dtype, device=device
        )
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        self.scale_shift_table = nn.Parameter(
            torch.empty(6, dim, device=device, dtype=dtype)
        )
    ...
    def forward(
        ...
        attn1_input = self.attn1(
            attn1_input, pe=pe, transformer_options=transformer_options
        )
...
def get_fractional_positions(indices_grid, max_pos):
    fractional_positions = torch.stack(
        [indices_grid[:, i] / max_pos[i] for i in range(3)],
        dim=-1,
    )
...
def precompute_freqs_cis(
    indices_grid, dim, out_dtype, theta=10000.0, max_pos=[20, 2048, 2048]
):
    dtype = torch.float32
    ...
    indices = (
        theta ** torch.linspace(0, 1, dim // 6, device=device, dtype=dtype)
        * math.pi
        / 2
    )
    ...
    if dim % 6 != 0:
        padding_size = dim % 6
        cos_vals = torch.cat(
            [torch.ones_like(cos_vals[:, :, :padding_size]), cos_vals], dim=-1
        )
        sin_vals = torch.cat(
            [torch.zeros_like(sin_vals[:, :, :padding_size]), sin_vals], dim=-1
        )

    # Reshape and extract one value per pair (since repeat_interleave duplicates each value)
    cos_vals = cos_vals.reshape(*cos_vals.shape[:2], -1, 2)[..., 0].to(
        out_dtype
    )  # [B, N, dim//2]
    sin_vals = sin_vals.reshape(*sin_vals.shape[:2], -1, 2)[..., 0].to(
        out_dtype
    )  # [B, N, dim//2]
...
class LTXVModel(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.patchify_proj = operations.Linear(
            in_channels, self.inner_dim, bias=True, dtype=dtype, device=device
        )
        ...
        self.scale_shift_table = nn.Parameter(
            torch.empty(2, self.inner_dim, dtype=dtype, device=device)
        )
        self.norm_out = operations.LayerNorm(
            self.inner_dim,
        ...
        self.proj_out = operations.Linear(
            self.inner_dim, self.out_channels, dtype=dtype, device=device
        )
