...
class ContextHandlerABC(ABC):
    ...
    @abstractmethod
    def get_resized_cond(
        self,
        cond_in: list[dict],
        x_in: torch.Tensor,
        window: ContextWindowABC,
        device=None,
    ) -> list:
        raise NotImplementedError("Not implemented.")
...
class IndexListContextWindow(ContextWindowABC):
    ...
    def get_tensor(
        self, full: torch.Tensor, device=None, dim=None, retain_index_list=[]
    ) -> torch.Tensor:
        if dim is None:
            dim = self.dim
    ...
    def add_window(
        self, full: torch.Tensor, to_add: torch.Tensor, dim=None
    ) -> torch.Tensor:
        if dim is None:
            dim = self.dim
        idx = tuple([slice(None)] * dim + [self.index_list])
...
ContextResults = collections.namedtuple(
    "ContextResults", ["window_idx", "sub_conds_out", "sub_conds", "window"]
)
...
class IndexListContextHandler(ContextHandlerABC):
    def __init__(
        ...
        self.cond_retain_index_list = (
            [int(x.strip()) for x in cond_retain_index_list.split(",")]
            if cond_retain_index_list
            else []
        )
    ...
    def should_use_context(
        ...
        if x_in.size(self.dim) > self.context_length:
            logging.info(
                f"Using context windows {self.context_length} with overlap {self.context_overlap} for {x_in.size(self.dim)} frames."
            )
            if self.cond_retain_index_list:
                logging.info(
                    f"Retaining original cond for indexes: {self.cond_retain_index_list}"
                )
            return True
        return False
    ...
    def get_resized_cond(
        ...
        if self.split_conds_to_windows and len(cond_in) > 1:
            ...
            logging.info(
                f"Splitting conds to windows; using region {region} for window {window.index_list[0]}-{window.index_list[-1]} with center ratio {window.center_ratio:.3f}"
            )
            ...
            for key in actual_cond:
                try:
                    ...
                    if isinstance(cond_item, torch.Tensor):
                        ...
                        if self.dim < cond_item.ndim and cond_item.size(
                            self.dim
                        ) == x_in.size(self.dim):
                    ...
                    # look for control
                        ...
                        resized_actual_cond[key] = self.prepare_control_objects(
                            cond_item, device
                        )
                        ...
                        for cond_key, cond_value in new_cond_item.items():
                            ...
                            if isinstance(cond_value, torch.Tensor):
                                if (
                                    self.dim < cond_value.ndim
                                    and cond_value(self.dim) == x_in.size(self.dim)
                                ) or (
                                    cond_value.ndim < self.dim
                                    and cond_value.size(0) == x_in.size(self.dim)
                                ):
                            ...
                            # Handle audio_embed (temporal dim is 1)
                                ...
                                cond_key == "audio_embed"
                                and hasattr(cond_value, "cond")
                                and isinstance(cond_value.cond, torch.Tensor)
                            ):
                                audio_cond = cond_value.cond
                                if audio_cond.ndim > 1 and audio_cond.size(
                                    1
                                ) == x_in.size(self.dim):
                                    new_cond_item[cond_key] = cond_value._copy_with(
                                        window.get_tensor(audio_cond, device, dim=1)
                                    )
                            # Handle vace_context (temporal dim is 3)
                            elif (
                                cond_key == "vace_context"
                                and hasattr(cond_value, "cond")
                                and isinstance(cond_value.cond, torch.Tensor)
                            ):
                                vace_cond = cond_value.cond
                                if vace_cond.ndim >= 4 and vace_cond.size(
                                    3
                                ) == x_in.size(self.dim):
                                    sliced_vace = window.get_tensor(
                                        vace_cond,
                                        device,
                                        dim=3,
                            ...
                            # if has cond that is a Tensor, check if needs to be subset
                                ...
                                if (
                                    self.dim < cond_value.cond.ndim
                                    and cond_value.cond.size(self.dim)
                                    == x_in.size(self.dim)
                                ) or (
                                    cond_value.cond.ndim < self.dim
                                    and cond_value.cond.size(0) == x_in.size(self.dim)
                                ):
                                    new_cond_item[cond_key] = cond_value._copy_with(
                                        window.get_tensor(
                                            cond_value.cond,
    ...
    def set_step(self, timestep: torch.Tensor, model_options: dict[str]):
        mask = torch.isclose(
            model_options["transformer_options"]["sample_sigmas"],
            timestep[0],
            rtol=0.0001,
        )
    ...
    def get_context_windows(
        self, model: BaseModel, x_in: torch.Tensor, model_options: dict[str]
    ) -> list[IndexListContextWindow]:
        full_length = x_in.size(self.dim)  # TODO: choose dim based on model
        context_windows = self.context_schedule.func(full_length, self, model_options)
        context_windows = [
            IndexListContextWindow(window, dim=self.dim, total_frames=full_length)
    ...
    def execute(
        ...
        if self.fuse_method.name == ContextFuseMethods.RELATIVE:
            counts_final = [
                torch.ones(get_shape_for_dim(x_in, self.dim), device=x_in.device)
                for _ in conds
            ]
        else:
            counts_final = [
                torch.zeros(get_shape_for_dim(x_in, self.dim), device=x_in.device)
                for _ in conds
            ]
        ...
        for enum_window in enumerated_context_windows:
            ...
            for result in results:
                self.combine_context_window_results(
                    x_in,
                    result.sub_conds_out,
                    result.sub_conds,
                    result.window,
                    result.window_idx,
                    len(enumerated_context_windows),
                    timestep,
                    conds_final,
                    counts_final,
                    biases_final,
    ...
    def evaluate_context_windows(
        ...
        for window_idx, window in enumerated_context_windows:
            ...
            for callback in comfy.patcher_extension.get_all_callbacks(
                IndexListCallbacks.EVALUATE_CONTEXT_WINDOWS, self.callbacks
            ):
                callback(
                    self,
                    model,
            ...
            sub_conds = [
                self.get_resized_cond(cond, x_in, window, device) for cond in conds
            ]

            sub_conds_out = calc_cond_batch(
    ...
    def combine_context_window_results(
        ...
        if self.fuse_method.name == ContextFuseMethods.RELATIVE:
            ...
            weights_tensor = match_weights_to_dim(
                weights, x_in, self.dim, device=x_in.device
            )
            for i in range(len(sub_conds_out)):
...
def _prepare_sampling_wrapper(
    executor, model, noise_shape: torch.Tensor, *args, **kwargs
):
    # limit noise_shape length to context_length for more accurate vram use estimation
    model_options = kwargs.get("model_options", None)
    if model_options is None:
        raise Exception(
            "model_options not found in prepare_sampling_wrapper; this should never happen, something went wrong."
        )
...
def _sampler_sample_wrapper(
    executor, guider, sigmas, extra_args, callback, noise, *args, **kwargs
):
    model_options = extra_args.get("model_options", None)
    if model_options is None:
        raise Exception(
            "model_options not found in sampler_sample_wrapper; this should never happen, something went wrong."
        )
    handler: IndexListContextHandler = model_options.get("context_handler", None)
    if handler is None:
        raise Exception(
            "context_handler not found in sampler_sample_wrapper; this should never happen, something went wrong."
        )
    ...
    noise = apply_freenoise(
        noise,
        handler.dim,
        handler.context_length,
        handler.context_overlap,
        extra_args["seed"],
    )
...
def match_weights_to_dim(
    weights: list[float], x_in: torch.Tensor, dim: int, device=None
) -> torch.Tensor:
...
def create_windows_uniform_looped(
    num_frames: int, handler: IndexListContextHandler, model_options: dict[str]
):
    windows = []
    ...
    context_stride = min(
        handler.context_stride,
        int(np.ceil(np.log2(num_frames / handler.context_length))) + 1,
    )
        ...
        for j in range(
            ...
            windows.append(
                [
                    e % num_frames
                    for e in range(
                        j, j + handler.context_length * context_step, context_step
                    )
                ]
            )
...
def create_windows_uniform_standard(
    num_frames: int, handler: IndexListContextHandler, model_options: dict[str]
):
    ...
    while win_i < len(windows):
        ...
        if is_roll:
            ...
            if roll_val not in windows[(win_i + 1) % len(windows)]:
                ...
                windows.insert(
                    win_i + 1, list(range(roll_val, roll_val + handler.context_length))
                )
...
def create_windows_static_standard(
    num_frames: int, handler: IndexListContextHandler, model_options: dict[str]
):
    windows = []
    if num_frames <= handler.context_length:
    ...
    for start_idx in range(0, num_frames, delta):
        ...
        if ending >= num_frames:
            ...
            windows.append(
                list(range(final_start_idx, final_start_idx + handler.context_length))
            )
            break
...
def create_windows_batched(
    ...
    for start_idx in range(0, num_frames, handler.context_length):
        windows.append(
            list(range(start_idx, min(start_idx + handler.context_length, num_frames)))
        )
    return windows
...
def get_context_weights(
    ...
    return handler.fuse_method.func(
        length, sigma=sigma, handler=handler, full_length=full_length, idxs=idxs
    )
...
def create_weights_overlap_linear(
    length: int,
    full_length: int,
    idxs: list[int],
    handler: IndexListContextHandler,
...
def apply_freenoise(
    noise: torch.Tensor, dim: int, context_length: int, context_overlap: int, seed: int
):
    ...
    for start_idx in range(0, latent_video_length - context_length, delta):
        ...
        list_idx = (
            torch.randperm(actual_delta, generator=generator, device="cpu") + start_idx
        )
