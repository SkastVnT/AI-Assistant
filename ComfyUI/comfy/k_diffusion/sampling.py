...
def get_sigmas_laplace(n, sigma_min, sigma_max, mu=0.0, beta=0.5, device="cpu"):
    ...
    lmb = mu - beta * torch.sign(0.5 - x) * torch.log(
        1 - 2 * torch.abs(0.5 - x) + epsilon
    )
...
def sample_euler_ancestral(
    ...
    if isinstance(
        ...
        return sample_euler_ancestral_RF(
            model, x, sigmas, extra_args, callback, disable, eta, s_noise, noise_sampler
        )
...
def sample_dpm_2_ancestral(
    ...
    if isinstance(
        ...
        return sample_dpm_2_ancestral_RF(
            model, x, sigmas, extra_args, callback, disable, eta, s_noise, noise_sampler
        )
...
class DPMSolver(nn.Module):
    ...
    def dpm_solver_fast(
        ...
        noise_sampler = (
            default_noise_sampler(x, seed=self.extra_args.get("seed", None))
...
def sample_dpmpp_2s_ancestral(
    ...
    if isinstance(
        ...
        return sample_dpmpp_2s_ancestral_RF(
            model, x, sigmas, extra_args, callback, disable, eta, s_noise, noise_sampler
        )
...
@torch.no_grad()
def sample_dpmpp_2s_ancestral_RF(
    model,
    x,
    sigmas,
    extra_args=None,
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            sd, su = get_ancestral_step(
                lambda_s.neg().exp(), lambda_s_1.neg().exp(), eta
            )
            ...
            if eta > 0 and s_noise > 0:
                x_2 = (
                    x_2 + alpha_s_1 * noise_sampler(sigmas[i], sigma_s_1) * s_noise * su
                )
...
def sample_dpmpp_2m_sde(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            x = (
                sigmas[i + 1] / sigmas[i] * (-h * eta).exp() * x
...
@torch.no_grad()
def sample_dpmpp_2m_sde_heun_gpu(
    model,
    x,
    sigmas,
    extra_args=None,
    ...
    return sample_dpmpp_2m_sde_heun(
        model,
        x,
        sigmas,
        extra_args=extra_args,
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            sigma_down, sigma_up = get_ancestral_step(
                sigmas[i] / alpha_s, sigmas[i + 1] / alpha_t, eta=eta
            )
...
def sample_dpmpp_2s_ancestral_cfg_pp(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigma_down == 0:
            ...
            h = t_next - t
            s = t + r * h
            x_2 = (sigma_fn(s) / sigma_fn(t)) * (x + (denoised - temp[0])) - (
                -h * r
            ).expm1() * denoised
            denoised_2 = model(x_2, sigma_fn(s) * s_in, **extra_args)
            x = (sigma_fn(t_next) / sigma_fn(t)) * (x + (denoised - temp[0])) - (
                -h
            ).expm1() * denoised_2
        # Noise addition
        if sigmas[i + 1] > 0:
...
def sample_dpmpp_2m_cfg_pp(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if old_uncond_denoised is None or sigmas[i + 1] == 0:
            ...
            h_last = t - t_fn(sigmas[i - 1])
            r = h_last / h
            denoised_mix = -torch.exp(-h) * uncond_denoised - torch.expm1(-h) * (
                1 / (2 * r)
...
def sample_gradient_estimation_cfg_pp(
    model, x, sigmas, extra_args=None, callback=None, disable=None, ge_gamma=2.0
):
...
def sample_er_sde(
    ...
    point_indice = torch.arange(
        0, num_integration_points, dtype=torch.float32, device=x.device
    )
...
def sample_seeds_2(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        # Step 1
        x_2 = (
            sigma_s_1 / sigmas[i] * (-r * h * eta).exp() * x
            - alpha_s_1 * ei_h_phi_1(-r * h_eta) * denoised
        )
        if inject_noise:
            sde_noise = (-2 * r * h * eta).expm1().neg().sqrt() * noise_sampler(
                sigmas[i], sigma_s_1
            )
        ...
        if solver_type == "phi_1":
            ...
            x = sigmas[i + 1] / sigmas[i] * (-h * eta).exp() * x - alpha_t * (
                b1 * denoised + b2 * denoised_2
            )

        if inject_noise:
            segment_factor = (r - 1) * h * eta
            sde_noise = sde_noise * segment_factor.exp()
            sde_noise = sde_noise + segment_factor.mul(
                2
            ).expm1().neg().sqrt() * noise_sampler(sigma_s_1, sigmas[i + 1])
...
def sample_exp_heun_2_x0(
    model, x, sigmas, extra_args=None, callback=None, disable=None, solver_type="phi_2"
):
    """Deterministic exponential Heun second order method in data prediction (x0) and logSNR time."""
    return sample_seeds_2(
        model,
        x,
        sigmas,
        extra_args=extra_args,
...
def sample_seeds_3(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        # Step 1
        x_2 = (
            sigma_s_1 / sigmas[i] * (-r_1 * h * eta).exp() * x
            - alpha_s_1 * ei_h_phi_1(-r_1 * h_eta) * denoised
        )
        if inject_noise:
            sde_noise = (-2 * r_1 * h * eta).expm1().neg().sqrt() * noise_sampler(
                sigmas[i], sigma_s_1
            )
            x_2 = x_2 + sde_noise * sigma_s_1 * s_noise
        denoised_2 = model(x_2, sigma_s_1 * s_in, **extra_args)
        ...
        x_3 = sigma_s_2 / sigmas[i] * (-r_2 * h * eta).exp() * x - alpha_s_2 * (
            a3_1 * denoised + a3_2 * denoised_2
        )
        if inject_noise:
            ...
            sde_noise = sde_noise + segment_factor.mul(
                2
            ).expm1().neg().sqrt() * noise_sampler(sigma_s_1, sigma_s_2)
        ...
        if inject_noise:
            ...
            sde_noise = sde_noise + segment_factor.mul(
                2
            ).expm1().neg().sqrt() * noise_sampler(sigma_s_2, sigmas[i + 1])
            x = x + sde_noise * sigmas[i + 1] * s_noise
    return x
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if corrector_order_used == 0:
            ...
            pred_mat = torch.stack(
                pred_list[-corrector_order_used:], dim=1
            )  # (B, K, ...)
        ...
        if sigmas[i + 1] == 0:
            ...
            pred_mat = torch.stack(
                pred_list[-predictor_order_used:], dim=1
            )  # (B, K, ...)
