...
class ModelPatcher:
    ...
    def patch_weight_to_device(self, key, device_to=None, inplace_update=False):
        ...
        if set_func is None:
            ...
            set_func(
                out_weight, inplace_update=inplace_update, seed=string_to_seed(key)
            )
    ...
    def load(
        ...
        with self.use_ejected():
            ...
            if lowvram_counter > 0:
                logging.info(
                    "loaded partially; {:.2f} MB usable, {:.2f} MB loaded, {:.2f} MB offloaded, {:.2f} MB buffer reserved, lowvram patches: {}".format(
                ...
                logging.info(
                    "loaded completely; {:.2f} MB usable, {:.2f} MB loaded, full load: {}".format(
                        lowvram_model_memory / (1024 * 1024),
                        mem_counter / (1024 * 1024),
                        full_load,
                    )
