    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "rho", default=7.0, min=0.0, max=100.0, step=0.01, round=False
                ),
            ],
            outputs=[io.Sigmas.Output()],
        )
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "rho", default=1.0, min=0.0, max=100.0, step=0.01, round=False
                ),
            ],
            outputs=[io.Sigmas.Output()],
        )
...
class LaplaceScheduler(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "mu", default=0.0, min=-10.0, max=10.0, step=0.1, round=False
                ),
                io.Float.Input(
                    "beta", default=0.5, min=0.0, max=10.0, step=0.1, round=False
                ),
            ],
            outputs=[io.Sigmas.Output()],
        )
    ...
    def execute(cls, steps, sigma_max, sigma_min, mu, beta) -> io.NodeOutput:
        sigmas = k_diffusion_sampling.get_sigmas_laplace(
            n=steps, sigma_min=sigma_min, sigma_max=sigma_max, mu=mu, beta=beta
        )
        return io.NodeOutput(sigmas)
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input("steps", default=20, min=1, max=10000),
                io.Float.Input(
                    "alpha", default=0.6, min=0.0, max=50.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "beta", default=0.6, min=0.0, max=50.0, step=0.01, round=False
                ),
            ],
            outputs=[io.Sigmas.Output()],
        )
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Int.Input("steps", default=20, min=1, max=10000),
                io.Float.Input(
                    "beta_d", default=19.9, min=0.0, max=5000.0, step=0.01, round=False
                ),  # TODO: fix default values
                io.Float.Input(
                    "beta_min", default=0.1, min=0.0, max=5000.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "eps_s", default=0.001, min=0.0, max=1.0, step=0.0001, round=False
                ),
            ],
            outputs=[io.Sigmas.Output()],
        )
    ...
    def execute(
        ...
        for i in range(len(sigmas) - 1):
            ...
            if end_at_sigma <= sigma_current <= start_at_sigma:
                interpolated_steps = (
                    computed_spacing * (sigma_next - sigma_current) + sigma_current
                )
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Model.Input("model"),
                io.Float.Input(
                    "sampling_percent", default=0.0, min=0.0, max=1.0, step=0.0001
                ),
                io.Boolean.Input(
                    "return_actual_sigma",
                    default=False,
...
class SamplerDPMPP_3M_SDE(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Float.Input(
                    "eta", default=1.0, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "s_noise", default=1.0, min=0.0, max=100.0, step=0.01, round=False
                ),
    ...
    def execute(cls, eta, s_noise, noise_device) -> io.NodeOutput:
        ...
        sampler = comfy.samplers.ksampler(
            sampler_name, {"eta": eta, "s_noise": s_noise}
        )
        return io.NodeOutput(sampler)
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "r", default=0.5, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Combo.Input("noise_device", options=["gpu", "cpu"]),
            ],
            outputs=[io.Sampler.Output()],
...
class SamplerEulerAncestral(io.ComfyNode):
    ...
    def execute(cls, eta, s_noise) -> io.NodeOutput:
        sampler = comfy.samplers.ksampler(
            "euler_ancestral", {"eta": eta, "s_noise": s_noise}
        )
        return io.NodeOutput(sampler)
...
class SamplerEulerAncestralCFGPP(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Float.Input(
                    "eta", default=1.0, min=0.0, max=1.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "s_noise", default=1.0, min=0.0, max=10.0, step=0.01, round=False
                ),
            ],
            outputs=[io.Sampler.Output()],
        )
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "rtol", default=0.05, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "atol", default=0.0078, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "h_init", default=0.05, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "pcoeff", default=0.0, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "icoeff", default=1.0, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "dcoeff", default=0.0, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "accept_safety",
                    default=0.81,
                ...
                io.Float.Input(
                    "eta", default=0.0, min=0.0, max=100.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "s_noise", default=1.0, min=0.0, max=100.0, step=0.01, round=False
                ),
...
class SamplerER_SDE(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Combo.Input(
                    "solver_type", options=["ER-SDE", "Reverse-time SDE", "ODE"]
                ),
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Model.Input("model"),
                io.Float.Input(
                    "eta", default=1.0, min=0.0, max=10.0, step=0.01, round=False
                ),
                io.Float.Input(
                    "sde_start_percent", default=0.2, min=0.0, max=1.0, step=0.001
                ),
                io.Float.Input(
                    "sde_end_percent", default=0.8, min=0.0, max=1.0, step=0.001
                ),
                io.Float.Input(
                    "s_noise", default=1.0, min=0.0, max=100.0, step=0.01, round=False
                ),
    ...
    def predict_noise(self, x, timestep, model_options={}, seed=None):
        ...
        if self.nested:
            out = comfy.samplers.calc_cond_batch(
                self.inner_model,
                [negative_cond, middle_cond, positive_cond],
                x,
                timestep,
                model_options,
            )
...
class DualCFGGuider(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Conditioning.Input("negative"),
                io.Float.Input(
                    "cfg_conds", default=8.0, min=0.0, max=100.0, step=0.1, round=0.01
                ),
                io.Float.Input(
                    "cfg_cond2_negative",
                    default=8.0,
    ...
    @classmethod
    def execute(
        cls, model, cond1, cond2, negative, cfg_conds, cfg_cond2_negative, style
    ) -> io.NodeOutput:
...
class ManualSigmas(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            is_experimental=True,
            inputs=[io.String.Input("sigmas", default="1, 0.5", multiline=False)],
            outputs=[io.Sigmas.Output()],
        )
