...
class QwenImageBlockWiseControlNet(torch.nn.Module):
    def __init__(
        ...
        self.img_in = operations.Linear(
            in_dim + additional_in_dim, dim, device=device, dtype=dtype
        )
        self.controlnet_blocks = torch.nn.ModuleList(
            [
                BlockWiseControlBlock(
                    dim, device=device, dtype=dtype, operations=operations
                )
    ...
    def process_input_latent_image(self, latent_image):
        latent_image[:, :16] = comfy.latent_formats.Wan21().process_in(
            latent_image[:, :16]
        )
        patch_size = 2
        hidden_states = comfy.ldm.common_dit.pad_to_patch_size(
            latent_image, (1, patch_size, patch_size)
        )
        orig_shape = hidden_states.shape
        hidden_states = hidden_states.view(
            orig_shape[0], orig_shape[1], orig_shape[-2] // 2, 2, orig_shape[-1] // 2, 2
        )
...
class SigLIPMultiFeatProjModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.high_embedding_linear = nn.Sequential(
            operations.Linear(siglip_token_nums, style_token_nums), nn.SiLU()
        )
        self.high_layer_norm = (
            operations.LayerNorm(siglip_token_dims)
            if context_layer_norm
            else nn.Identity()
        )
        self.high_projection = operations.Linear(
            siglip_token_dims, hidden_size, bias=True
        )
        ...
        self.mid_projection = operations.Linear(
            siglip_token_dims, hidden_size, bias=True
        )
        ...
        self.low_projection = operations.Linear(
            siglip_token_dims, hidden_size, bias=True
        )
    ...
    def _process_layer_features(
        ...
        embedding = embedding_linear(hidden_states.to(dtype).transpose(1, 2)).transpose(
            1, 2
        )
    ...
    def encode_latent_cond(self, image):
        ...
        if self.model_patch.model.additional_in_dim > 0:
            if self.mask is None:
                mask_ = torch.ones_like(latent_image)[
                    :, : self.model_patch.model.additional_in_dim // 4
                ]
            else:
...
class ZImageControlPatch:
    def __init__(
        self, model_patch, vae, image, strength, inpaint_image=None, mask=None
    ):
        self.model_patch = model_patch
        self.vae = vae
        self.image = image
        ...
        if skip_encoding:
            ...
            if self.image is None:
                self.encoded_image_size = (
                    self.inpaint_image.shape[1],
                    self.inpaint_image.shape[2],
                )
            else:
    ...
    def encode_latent_cond(self, control_image=None, inpaint_image=None):
        ...
        if control_image is not None:
            latent_image = comfy.latent_formats.Flux().process_in(
        ...
        if self.is_inpaint:
            ...
            if self.mask is not None:
                mask_inpaint = comfy.utils.common_upscale(
                    self.mask.view(
                        self.mask.shape[0], -1, self.mask.shape[-2], self.mask.shape[-1]
                    ).mean(dim=1, keepdim=True),
                    inpaint_image.shape[-2],
                    inpaint_image.shape[-3],
                    "bilinear",
                    "center",
                )
                inpaint_image = (
                    (inpaint_image - 0.5) * mask_inpaint.movedim(1, -1).round()
                ) + 0.5

            inpaint_image_latent = comfy.latent_formats.Flux().process_in(
                self.vae.encode(inpaint_image)
            )
            ...
            if self.mask is None:
                ...
                mask_ = comfy.utils.common_upscale(
                    ...
                    .mean(dim=1, keepdim=True)
                    .to(device=inpaint_image_latent.device),
                    inpaint_image_latent.shape[-1],
                    inpaint_image_latent.shape[-2],
                    "nearest",
                    "center",
                )
            ...
            if latent_image is None:
                latent_image = comfy.latent_formats.Flux().process_in(
                    self.vae.encode(torch.ones_like(inpaint_image) * 0.5)
                )
    ...
    def __call__(self, kwargs):
        ...
        if self.encoded_image is None or self.encoded_image_size != (
            ...
            if self.image is not None:
                ...
                self.encoded_image_size = (
                    image_scaled.shape[-3],
                    image_scaled.shape[-2],
                )
            ...
            if self.inpaint_image is not None:
                ...
                self.encoded_image_size = (
                    inpaint_scaled.shape[-3],
                    inpaint_scaled.shape[-2],
                )
        ...
        if self.temp_data is None or self.temp_data[0] > cnet_index:
            if block_type == "noise_refiner":
                self.temp_data = (
                    ...
                    (
                        None,
                        self.model_patch.model(
                            txt, self.encoded_image.to(img.dtype), pe, vec
                        ),
                    ),
                )
            else:
        ...
        if block_type == "noise_refiner":
            ...
            if self.temp_data[1][0] is not None:
                img[:, : self.temp_data[1][0].shape[1]] += (
                    self.temp_data[1][0] * self.strength
                )
        else:
            while (
                self.temp_data[0] < cnet_index and (self.temp_data[0] + 1) < cnet_blocks
            ):
                next_layer = self.temp_data[0] + 1
                self.temp_data = (
                    next_layer,
...
class UsoStyleProjectorPatch:
    ...
    def __call__(self, kwargs):
        ...
        txt = kwargs.get("txt")
        siglip_embedding = self.model_patch.model(self.encoded_image.to(txt.dtype)).to(
            txt.dtype
        )
        txt = torch.cat([siglip_embedding, txt], dim=1)
        kwargs["txt"] = txt
        kwargs["txt_ids"] = torch.cat(
            [
                torch.zeros(
                    siglip_embedding.shape[0],
                    siglip_embedding.shape[1],
                    3,
                    dtype=txt_ids.dtype,
...
class USOStyleReference:
    ...
    def apply_patch(self, model, model_patch, clip_vision_output):
        encoded_image = torch.stack(
            (
                clip_vision_output.all_hidden_states[:, -20],
                clip_vision_output.all_hidden_states[:, -11],
                clip_vision_output.penultimate_hidden_states,
            )
        )
        model_patched = model.clone()
        model_patched.set_model_post_input_patch(
            UsoStyleProjectorPatch(model_patch, encoded_image)
        )
        return (model_patched,)
