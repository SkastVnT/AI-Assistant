    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Vae.Input("vae"),
                io.Int.Input(
                    "width", default=768, min=16, max=nodes.MAX_RESOLUTION, step=16
                ),
                io.Int.Input(
                    "height", default=512, min=16, max=nodes.MAX_RESOLUTION, step=16
                ),
                io.Int.Input(
                    "length", default=121, min=1, max=nodes.MAX_RESOLUTION, step=4
                ),
    ...
    def execute(
        ...
        if start_image is not None:
            ...
            positive = node_helpers.conditioning_set_values(
                positive, {"time_dim_replace": encoded, "concat_mask": mask}
            )
            negative = node_helpers.conditioning_set_values(
                negative, {"time_dim_replace": encoded, "concat_mask": mask}
            )
...
def adaptive_mean_std_normalization(
    ...
    reference_mean = torch.clamp(
        reference.mean(), source_mean - clump_mean_low, source_mean + clump_mean_high
    )
...
class NormalizeVideoLatentStart(io.ComfyNode):
    ...
    def execute(cls, latent, start_frame_count, reference_frame_count) -> io.NodeOutput:
        ...
        normalized_first_frames = adaptive_mean_std_normalization(
