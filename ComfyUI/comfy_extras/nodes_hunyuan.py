...
class EmptyHunyuanLatentVideo(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Int.Input(
                    "width", default=848, min=16, max=nodes.MAX_RESOLUTION, step=16
                ),
                io.Int.Input(
                    "height", default=480, min=16, max=nodes.MAX_RESOLUTION, step=16
                ),
                io.Int.Input(
                    "length", default=25, min=1, max=nodes.MAX_RESOLUTION, step=4
                ),
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input(
                    "length", default=33, min=1, max=nodes.MAX_RESOLUTION, step=4
                ),
                io.Int.Input("batch_size", default=1, min=1, max=4096),
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "noise_augmentation", default=0.70, min=0.0, max=1.0, step=0.01
                ),
            ],
            outputs=[
                io.Conditioning.Output(display_name="positive"),
    ...
    def execute(
        ...
        if start_image is not None:
            start_image = comfy.utils.common_upscale(
                ...
                in_latent.shape[-1] * 16,
                in_latent.shape[-2] * 16,
                "bilinear",
                "center",
            ).movedim(1, -1)
            encoded = vae.encode(start_image[:, :, :, :3])
        ...
        positive = node_helpers.conditioning_set_values(
            positive,
            {
                "concat_latent_image": cond_latent,
...
class HunyuanVideo15LatentUpscaleWithModel(io.ComfyNode):
    ...
    @classmethod
    def execute(
        cls, model, samples, upscale_method, width, height, crop
    ) -> io.NodeOutput:
        if width == 0 and height == 0:
            ...
            s = comfy.utils.common_upscale(
                samples["samples"], width // 16, height // 16, upscale_method, crop
            )
...
class TextEncodeHunyuanVideo_ImageToVideo(io.ComfyNode):
    ...
    @classmethod
    def execute(
        cls, clip, clip_vision_output, prompt, image_interleave
    ) -> io.NodeOutput:
...
class HunyuanImageToVideo(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input(
                    "length", default=53, min=1, max=nodes.MAX_RESOLUTION, step=4
                ),
                io.Int.Input("batch_size", default=1, min=1, max=4096),
                io.Combo.Input(
                    "guidance_type", options=["v1 (concat)", "v2 (replace)", "custom"]
                ),
                io.Image.Input("start_image", optional=True),
            ],
            outputs=[
    ...
    def execute(
        ...
        if start_image is not None:
            start_image = comfy.utils.common_upscale(
                start_image[:length, :, :, :3].movedim(-1, 1),
                width,
                height,
                "bilinear",
                "center",
...
class EmptyHunyuanImageLatent(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Int.Input(
                    "width", default=2048, min=64, max=nodes.MAX_RESOLUTION, step=32
                ),
                io.Int.Input(
                    "height", default=2048, min=64, max=nodes.MAX_RESOLUTION, step=32
                ),
                io.Int.Input("batch_size", default=1, min=1, max=4096),
            ],
            outputs=[
    ...
    def execute(cls, width, height, batch_size=1) -> io.NodeOutput:
        latent = torch.zeros(
            [batch_size, 64, height // 32, width // 32],
            device=comfy.model_management.intermediate_device(),
        )
        return io.NodeOutput({"samples": latent})
...
class HunyuanRefinerLatent(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Latent.Input("latent"),
                io.Float.Input(
                    "noise_augmentation", default=0.10, min=0.0, max=1.0, step=0.01
                ),
            ],
            outputs=[
                io.Conditioning.Output(display_name="positive"),
    ...
    def execute(cls, positive, negative, latent, noise_augmentation) -> io.NodeOutput:
        ...
        positive = node_helpers.conditioning_set_values(
            positive,
            {"concat_latent_image": latent, "noise_augmentation": noise_augmentation},
        )
        negative = node_helpers.conditioning_set_values(
            negative,
            {"concat_latent_image": latent, "noise_augmentation": noise_augmentation},
        ...
        out_latent["samples"] = torch.zeros(
            [latent.shape[0], 32, latent.shape[-3], latent.shape[-2], latent.shape[-1]],
            device=comfy.model_management.intermediate_device(),
        )
        return io.NodeOutput(positive, negative, out_latent)
