...
class ComfyClient:
    ...
    def get_jobs(
        self, status=None, limit=None, offset=None, sort_by=None, sort_order=None
    ):
...
class TestExecution:
    ...
    def test_full_cache(self, client: ComfyClient, builder: GraphBuilder, server):
        ...
        input2 = g.node(
            "StubImage", content="NOISE", height=512, width=512, batch_size=1
        )
    ...
    def test_error(self, client: ComfyClient, builder: GraphBuilder):
        ...
        input2 = g.node(
            "StubImage", content="NOISE", height=256, width=256, batch_size=1
        )
        mask = g.node("StubMask", value=0.5, height=512, width=512, batch_size=1)
    ...
    def test_validation_error_literal(
        self, test_value, expect_error, client: ComfyClient, builder: GraphBuilder
    ):
        g = builder
    ...
    @pytest.mark.parametrize(
        "test_type, test_value", [("StubInt", 5), ("StubMask", 5.0)]
    )
    def test_validation_error_edge1(
        self, test_type, test_value, client: ComfyClient, builder: GraphBuilder
    ):
        g = builder
    ...
    @pytest.mark.parametrize(
        "test_type, test_value, expect_error",
        [("StubInt", 5, True), ("StubFloat", 5.0, False)],
    )
    def test_validation_error_edge2(
        self,
        test_type,
        test_value,
        expect_error,
    ...
    def test_validation_error_edge3(
        self,
        test_type,
        test_value,
        expect_error,
    ...
    @pytest.mark.parametrize(
        "test_value1, test_value2, expect_error",
        [(0.0, 0.5, False), (0.0, 5.0, False), (0.0, 7.0, True)],
    )
    def test_validation_error_kwargs(
        self,
        test_value1,
        test_value2,
        expect_error,
        ...
        g = builder
        validation5 = g.node(
            "TestCustomValidation5", input1=test_value1, input2=test_value2
        )
    ...
    def test_dynamic_cycle_error(self, client: ComfyClient, builder: GraphBuilder):
        ...
        generator = g.node(
            "TestDynamicDependencyCycle", input1=input1.out(0), input2=input2.out(0)
        )
    ...
    def test_custom_is_changed(
        self, client: ComfyClient, builder: GraphBuilder, server
    ):
        g = builder
    ...
    def test_undeclared_inputs(self, client: ComfyClient, builder: GraphBuilder):
        ...
        expected = 255 // 4
        assert (
            numpy.array(result_image).min() == expected
            and numpy.array(result_image).max() == expected
    ...
    def test_for_loop(self, client: ComfyClient, builder: GraphBuilder):
        ...
        is_changed = g.node(
            "TestCustomIsChanged", should_change=True, image=input2.out(0)
        )
        for_open = g.node(
            "TestForLoopOpen", remaining=iterations, initial_value1=is_changed.out(0)
        )
        average = g.node(
            "TestVariadicAverage", input1=input1.out(0), input2=for_open.out(2)
        )
        for_close = g.node(
            "TestForLoopClose",
            flow_control=for_open.out(0),
            initial_value1=average.out(0),
        )
    ...
    def test_mixed_expansion_returns(self, client: ComfyClient, builder: GraphBuilder):
        ...
        assert (
            numpy.array(images_dynamic[0]).min() == 25
            and numpy.array(images_dynamic[0]).max() == 25
        ), "First image should be 0.1"
        assert (
            numpy.array(images_dynamic[1]).min() == 51
            and numpy.array(images_dynamic[1]).max() == 51
        ), "Second image should be 0.2"
        assert (
            numpy.array(images_dynamic[2]).min() == 76
            and numpy.array(images_dynamic[2]).max() == 76
        ), "Third image should be 0.3"

        images_literal = result.get_images(output_literal)
        assert len(images_literal) == 3, "Should have 2 images"
        for i in range(3):
            assert (
                numpy.array(images_literal[i]).min() == 255
                and numpy.array(images_literal[i]).max() == 255
            ), "All images should be white"
    ...
    def test_mixed_lazy_results(self, client: ComfyClient, builder: GraphBuilder):
        ...
        mask = g.node(
            "StubMask", value=val_list.out(0), height=512, width=512, batch_size=1
        )
        input1 = g.node(
            "StubImage", content="BLACK", height=512, width=512, batch_size=1
        )
        ...
        assert (
            numpy.array(images[0]).min() == 0 and numpy.array(images[0]).max() == 0
        ), "First image should be 0.0"
        assert (
            numpy.array(images[1]).min() == 127 and numpy.array(images[1]).max() == 127
        ), "Second image should be 0.5"
        assert (
            numpy.array(images[2]).min() == 255 and numpy.array(images[2]).max() == 255
        ), "Third image should be 1.0"
    ...
    def test_is_changed_with_outputs(
        self, client: ComfyClient, builder: GraphBuilder, server
    ):
        g = builder
        input1 = g.node(
            "StubConstantImage", value=0.5, height=512, width=512, batch_size=1
        )
    ...
    def test_parallel_sleep_expansion(
        self, client: ComfyClient, builder: GraphBuilder, skip_timing_checks
    ):
        # Warmup execution to ensure server is fully initialized
        run_warmup(client)
    ...
    def test_execution_block_list_output(
        ...
        assert (
            numpy.array(images[0]).min() == 0 and numpy.array(images[0]).max() == 0
        ), "First image should be black"
        assert (
            numpy.array(images[1]).min() == 0 and numpy.array(images[1]).max() == 0
        ), "Second image should also be black"

    # Output nodes included in the partial execution list are executed
    def test_partial_execution_included_outputs(
        self, client: ComfyClient, builder: GraphBuilder
    ):
        g = builder
        input1 = g.node(
    ...
    def test_partial_execution_dependencies(
        self, client: ComfyClient, builder: GraphBuilder
    ):
        g = builder
        input1 = g.node(
        ...
        output_with_socket = g.node(
            "TestOutputNodeWithSocketOutput", image=input1.out(0), value=2.0
        )

        # Create another node that depends on the output_with_socket
        dependent_node = g.node(
            "TestLazyMixImages",
            image1=output_with_socket.out(0),
            image2=input1.out(0),
            mask=g.node("StubMask", value=0.5, height=512, width=512, batch_size=1).out(
                0
            ),
        )
    ...
    def test_partial_execution_with_lazy_nodes(
        ...
        mask1 = g.node(
            "StubMask", value=0.0, height=512, width=512, batch_size=1
        )  # Will only need image1
        mask2 = g.node(
            "StubMask", value=0.5, height=512, width=512, batch_size=1
        )  # Will need both images
    ...
    def test_partial_execution_multiple_output_nodes(
        self, client: ComfyClient, builder: GraphBuilder
    ):
        g = builder
        input1 = g.node(
        ...
        output_node1 = g.node(
            "TestOutputNodeWithSocketOutput", image=input1.out(0), value=1.5
        )
        output_node2 = g.node(
            "TestOutputNodeWithSocketOutput", image=output_node1.out(0), value=2.0
        )
    ...
    def test_partial_execution_empty_list(
        self, client: ComfyClient, builder: GraphBuilder
    ):
        g = builder
        input1 = g.node(
    ...
    # Jobs API tests
    def test_jobs_api_job_structure(self, client: ComfyClient, builder: GraphBuilder):
    ...
    def test_jobs_api_pagination(self, client: ComfyClient, builder: GraphBuilder):
    ...
    def test_jobs_api_sorting(self, client: ComfyClient, builder: GraphBuilder):
        ...
        if len(desc_jobs["jobs"]) >= 2:
            desc_times = [
                j["create_time"] for j in desc_jobs["jobs"] if j["create_time"]
            ]
    ...
    def test_jobs_api_status_filter(self, client: ComfyClient, builder: GraphBuilder):
    ...
    def test_get_job_by_id(self, client: ComfyClient, builder: GraphBuilder):
    ...
    def test_get_job_not_found(self, client: ComfyClient, builder: GraphBuilder):
