...
class Combo(ComfyTypeIO):
    ...
    class Input(WidgetInput):
        ...
        def __init__(
            ...
            super().__init__(
                id, display_name, optional, tooltip, lazy, default, socketless
            )
...
class DynamicInput(Input, ABC):
    ...
    def expand_schema_for_dynamic(
        self, d: dict[str, Any], live_inputs: dict[str, Any], curr_prefix=""
    ):
        pass
        ...
        def expand_schema_for_dynamic(
            ...
            if self.id in live_inputs:
                ...
                if selected_option is not None:
                    add_to_input_dict_v1(
                        d, selected_option.inputs, live_inputs, curr_prefix
                    )
...
def add_dynamic_id_mapping(
    d: dict[str, Any], inputs: list[Input], curr_prefix: str, self: DynamicInput = None
):
...
class Schema:
    ...
    def validate(self):
        ...
        if len(intersection) > 0:
            issues.append(
                f"Ids must be unique between inputs and outputs, but {intersection} are not."
            )
        if len(issues) > 0:
            raise ValueError("\n".join(issues))
...
def create_input_dict_v1(
    inputs: list[Input], live_inputs: dict[str, Any] = None
) -> dict:
    input = {"required": {}}
...
class _ComfyNodeBaseInternal(_ComfyNodeInternal):
    ...
    def PREPARE_CLASS_CLONE(cls, v3_data: V3Data | None) -> type[ComfyNode]:
        ...
        # set hidden
        type_clone.hidden = HiddenHolder.from_dict(
            v3_data["hidden_inputs"] if v3_data else None
        )
        return type_clone
    ...
    @classmethod
    def INPUT_TYPES(
        cls, include_hidden=True, return_schema=False, live_inputs=None
    ) -> dict[str, dict] | tuple[dict[str, dict], Schema, V3Data]:
