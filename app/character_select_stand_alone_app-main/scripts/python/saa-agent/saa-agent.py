...
class GenerateDataPacker:
    ...
    def pack(self, regional: bool = False, **kwargs) -> Dict:  # noqa S3776
        ...
        if self.api_interface == "ComfyUI" and sampler not in SAMPLER_COMFYUI:
            self.logger.warning(
                f"Sampler '{sampler}' not supported in ComfyUI, using 'euler_ancestral'"
            )
            sampler = "euler_ancestral"
        elif self.api_interface == "WebUI" and sampler not in SAMPLER_WEBUI:
            self.logger.warning(
                f"Sampler '{sampler}' not supported in WebUI, using 'Euler a'"
            )
            sampler = "Euler a"
        ...
        if self.api_interface == "ComfyUI" and scheduler not in SCHEDULER_COMFYUI:
            self.logger.warning(
                f"Scheduler '{scheduler}' not supported in ComfyUI, using 'normal'"
            )
            scheduler = "normal"
        elif self.api_interface == "WebUI" and scheduler not in SCHEDULER_WEBUI:
            self.logger.warning(
                f"Scheduler '{scheduler}' not supported in WebUI, using 'Automatic'"
            )
            scheduler = "Automatic"
        ...
        hifix_random_flag = kwargs.get(
            "hifix_random_seed", kwargs.get("hifix_randomSeed", None)
        )
        ...
        if hifix_seed_provided:
            ...
            if isinstance(hifix_seed_user, int) and hifix_seed_user == -1:
                ...
                hifix_random = True
                self.logger.info(
                    f"hifix: user provided -1 -> generated hifix seed: {hifix_seed_to_use}"
                )
            else:
                hifix_seed_to_use = (
                    int(hifix_seed_user) if hifix_seed_user is not None else seed
                )
                hifix_random = False
                self.logger.info(
                    f"hifix: using user-provided hifix seed: {hifix_seed_to_use}"
                )
        else:
            ...
            if hifix_random_flag:
                ...
                hifix_random = True
                self.logger.info(
                    f"hifix: random seed enabled -> generated hifix seed: {hifix_seed_to_use}"
                )
            else:
                # reuse main seed
                ...
                hifix_random = False
                self.logger.info(
                    f"hifix: reusing main seed for hifix: {hifix_seed_to_use}"
                )
        ...
        if regional:
            ...
            data["regional"] = kwargs.get(
                "regional_params", self.create_regional_params()
            )
        else:
...
class SAAClient:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if register_response and "value" in register_response:
                self.logger.debug(
                    f"Register successfully! {register_response['value']}"
                )
            else:
            ...
            version_response = await self.send_message(
                {"type": "API", "method": "getAppVersion", "params": []}
            )
    ...
    async def send_message(
        self, message: Dict, allow_reconnect: bool = True
    ) -> Optional[Dict]:  # noqa S3776
        """
        ...
        if not self.ws or self.ws.close_code is not None:
            if allow_reconnect and not self.reconnecting:
                ...
                if not await self.connect():
                    raise RuntimeError(
                        "WebSocket connection failed and reconnection unsuccessful"
                    )
            else:
        ...
        try:
            ...
            self.logger.debug(
                f"Sent message: {message.get('type')} - {message.get('method')} (ID: {msg_id})"
            )
            ...
            self.logger.error(
                f"Message timeout [{message_with_id.get('type')} - {method}] ID={msg_id}"
            )
            self.logger.error(
                f"  Wait time: {self.timeout}s | WebSocket status: {ws_status}"
            )
            raise RuntimeError(
                f"Message timeout (ID: {msg_id}, method: {method}, wait: {self.timeout}s)"
            )
        except Exception as e:
            if msg_id in self.pending_messages:
                del self.pending_messages[msg_id]
...
class ProgressCallback:
    ...
    def __call__(self, args: List):  # noqa S3776
        ...
        if len(args) == 2:
            ...
            level_map = {"warn": "WARNING", "error": "ERROR", "log": "INFO"}
...
async def generate_image(config: GenerationConfig, logger: logging.Logger) -> int:
    ...
    """
    client = SAAClient(
        config.ws_address, config.username, config.password, config.timeout, logger
    )
    ...
    try:
        ...
        callback = ProgressCallback(
            client, config.output_file, config.output_base64, logger
        )
        ...
        if config.regional:
            ...
            params["regional_params"] = packer.create_regional_params(
                overlap_ratio=config.overlap_ratio, image_ratio=config.image_ratio
            )
        else:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    conn_group.add_argument(
        "--password", type=str, default="", help="Login password for WSS connections"
    )
    conn_group.add_argument(
        "--timeout",
        type=int,
    ...
    gen_group.add_argument(
        "--positive", type=str, default="", help="Positive prompt (what you want)"
    )
    gen_group.add_argument(
        "--negative", type=str, default="", help="Negative prompt (what you don't want)"
    )
    gen_group.add_argument(
    ...
    gen_group.add_argument(
        "--steps", type=int, default=28, help="Number of sampling steps (default: 28)"
    )
    gen_group.add_argument(
        "--seed", type=int, default=-1, help="Random seed (-1 for random) (default: -1)"
    )
    gen_group.add_argument(
        "--sampler",
        type=str,
    ...
    regional_group.add_argument(
        "--positive-left", type=str, default="", help="Positive prompt for left region"
    )
    regional_group.add_argument(
        "--positive-right",
        type=str,
    ...
    enhance_group.add_argument(
        "--hifix", action="store_true", help="Enable high-resolution fix"
    )
    enhance_group.add_argument(
        "--hifix-scale",
        type=float,
    ...
    enhance_group.add_argument(
        "--refiner", action="store_true", help="Enable refiner model"
    )
    enhance_group.add_argument(
        "--refiner-model",
        type=str,
...
def main():
    ...
    if not args.regional and not args.positive:
        print(
            "Error: --positive is required for non-regional generation", file=sys.stderr
        )
        return ExitCode.INVALID_PARAMS.value

    if args.regional and (not args.positive_left or not args.positive_right):
        print(
            "Error: --positive-left and --positive-right are required for regional generation",
            file=sys.stderr,
        )
        return ExitCode.INVALID_PARAMS.value
