...
from image_pipeline.paths import (
    COMFYUI_DIR,
    CONFIGS_DIR,
    LORA_DIR,
    REPO_ROOT,
...
_COMFYUI_LORA_ROOT = COMFYUI_DIR / "models" / "loras"
...
_LORA_REGISTRY_YAML = CONFIGS_DIR / "lora_registry.yaml"
...
_DOWNLOAD_MANIFEST_DIR = LORA_DIR
...
def _resolve_local_lora_path(filename: str, location: str) -> Optional[Path]:
    ...
    if location == "lora_dir" or not location:
        candidates += [
            LORA_DIR / filename,
            LORA_DIR / "new_2" / "character" / filename,
        ]
    for c in candidates:
        try:
            if c.is_file():
