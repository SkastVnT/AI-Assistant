...
class ServiceManager:
    ...
    def start_service(self, service_id: str) -> bool:
        ...
        if is_port_in_use(port):
            print(
                f"{Colors.YELLOW}   Killing existing process on port {port}...{Colors.END}"
            )
            kill_port(port)
            time.sleep(1)

        print(
            f"{Colors.BLUE}🚀 Starting {config['name']} on port {port}...{Colors.END}"
        )
        ...
        try:
            ...
            for _ in range(30):  # 30 seconds timeout
                ...
                if is_port_in_use(port):
                    print(
                        f"{Colors.GREEN}   ✓ {config['name']} started successfully{Colors.END}"
                    )
                    return True
                if process.poll() is not None:
                    print(
                        f"{Colors.RED}   ✗ {config['name']} exited unexpectedly{Colors.END}"
                    )
                    return False

            print(
                f"{Colors.YELLOW}   ⚠️  {config['name']} may still be starting...{Colors.END}"
            )
            return True
    ...
    def create_tunnel(self, service_id: str) -> str | None:
        ...
        if not is_port_in_use(port):
            print(
                f"{Colors.YELLOW}   ⚠️  Port {port} not listening, skipping tunnel{Colors.END}"
            )
            return None

        try:
            print(
                f"{Colors.CYAN}🔗 Creating tunnel for {config['name']}...{Colors.END}"
            )
    ...
    def print_status(self):
        ...
        print(
            f"{Colors.BOLD}{'SERVICE':<25} {'PORT':<8} {'STATUS':<12} {'PUBLIC URL'}{Colors.END}"
        )
    ...
    def save_urls(self):
        ...
        data = {"generated_at": time.strftime("%Y-%m-%d %H:%M:%S"), "services": {}}
    ...
    def wait_forever(self):
        ...
        print(
            f"{Colors.GREEN}🎉 All services are running and publicly accessible!{Colors.END}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Deploy AI-Assistant services publicly"
    )
    parser.add_argument(
        "--services",
        "-s",
    ...
    parser.add_argument(
        "--list", action="store_true", help="List available services and exit"
    )
    ...
    if args.list:
        ...
        for service_id, config in SERVICES.items():
            print(
                f"  {Colors.CYAN}{service_id:<25}{Colors.END} Port {config['port']:<6} - {config['description']}"
            )
        print()
        return
