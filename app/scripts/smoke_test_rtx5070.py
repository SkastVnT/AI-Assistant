...
generation â€” that needs ComfyUI booted with models loaded. This script
...
Checks:
  ...
  4. ComfyUI ``/system_stats`` reachable (optional â€” soft warning only).

Run from the repo root inside ``venv-image``:

    & .\\venv-image\\Scripts\\Activate.ps1
    python app/scripts/smoke_test_rtx5070.py
    python app/scripts/smoke_test_rtx5070.py --comfyui-url http://127.0.0.1:8188
...
CONFIG_REL = "app/configs_vps/anime_pipeline_rtx5070.yaml"

# Make app/ importable so `image_pipeline.*` resolves regardless
...
def check_torch_cuda() -> bool:
    ...
    if cap == (12, 0):
        print(
            "  Blackwell sm_120 detected â€” confirm torch is built with CUDA 12.8 kernels."
        )
    try:
...
def check_comfyui(url: str | None) -> bool:
    ...
    try:
        ...
        print("  WARN: httpx not installed in this venv â€” skipping.")
        return True
    try:
        ...
        # Soft failure â€” pipeline can boot ComfyUI on demand.
        return True
    return True
...
def check_loras_dir() -> bool:
    ...
    if count == 0:
        print("  WARN: no LoRAs on disk; run app/scripts/validate_lora_registry.py.")
    return True
