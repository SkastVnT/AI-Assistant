...
| last30days | â€” (subprocess) | `tools` | No | `vendor/last30days/repo/scripts/last30days.py` |
...
These variables are loaded by `services/shared_env.py` â†’ `load_shared_env()` for all services.
...
These are loaded by `run.py` **without override** â€” they supplement shared env.
...
last30days reads its own config from `~/.config/last30days/.env` (or the repo's `.env`). The chatbot does NOT pass API keys to it â€” last30days manages its own credentials.
...
### Step 1 â€” Clone and activate venv
...
### Step 5 â€” Set up last30days (optional)
...
### Step 6 â€” Set up Hermes Agent (optional)
...
last30days runs as a subprocess â€” no persistent health endpoint. Verify via:
...
| Symptom | Cause | Fix |
|---|---|---|
| `"âŒ last30days tool is not enabled"` | `LAST30DAYS_ENABLED=false` | Set to `true` in env |
| `"âŒ last30days script not found"` | Missing repo clone | Run `vendor/last30days/setup.ps1` or `setup.sh` |
...
# Remove volumes (destructive â€” loses data)
...
- Clean failure boundary â€” subprocess errors don't crash the chatbot

```
chatbot (venv-core, Python 3.10+)
  â””â”€ subprocess.run(["python3.12", "last30days.py", ...])
       â””â”€ last30days (own deps, Python 3.12+)
            â””â”€ optional: Node.js (Bird client for X)
```
...
```
chatbot (port 5000)
  â””â”€ HTTP proxy â†’ Hermes Gateway API (port 8080)
                    â””â”€ AIAgent â†’ tools, memory, subagents
```

### What is NOT included

- **Stable Diffusion** (port 7861) â€” requires `venv-image` + GPU
- **Edit Image / ComfyUI** (port 8100) â€” requires `venv-image` + GPU
- **MCP Server** â€” uses stdio transport, runs alongside chatbot process
- **RAG subsystem** â€” has its own `app/rag/docker-compose.yml`
...
â”‚  â”‚ MongoDB  â”‚â—„â”€â”€â”€â”‚    ChatBot      â”‚    â”‚    host:5000
â”‚  â”‚  :27017  â”‚    â”‚     :5000       â”‚â”€â”€â”€â”€â”¼â”€â”€â–º http://localhost:5000
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜    â””â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”˜    â”‚
â”‚                           â”‚             â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”€â”€â”    â”‚
â”‚  â”‚  Redis   â”‚    â”‚  Hermes (opt)   â”‚    â”‚    host:8080
â”‚  â”‚  :6379   â”‚    â”‚     :8080       â”‚â”€â”€â”€â”€â”¼â”€â”€â–º http://localhost:8080
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜    â”‚
...
- [last30days Integration](../../services/chatbot/docs/last30days_integration.md) â€” detailed tool integration docs
- [Integration Plan](./integration_plan_hermes_last30days.md) â€” master plan with phases and risk matrix
- [Chatbot README](../../services/chatbot/docs/README.md) â€” chatbot service documentation
- [Troubleshooting](../../services/chatbot/docs/TROUBLESHOOTING.md) â€” general chatbot troubleshooting
