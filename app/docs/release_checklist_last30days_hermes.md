...
## Pre-merge â€” Integration Points

### last30days
- [ ] `core/last30days_tool.py` â€” subprocess wrapper with timeout, input validation, max topic length
- [ ] `routes/last30days.py` â€” Flask blueprint registered in `chatbot_main.py`
- [ ] `core/skills/builtins/social_research.yaml` â€” skill definition loadable by SkillRegistry
...
### Hermes
- [ ] `core/hermes_adapter.py` â€” HTTP proxy with timeout, auth header, error handling
- [ ] `routes/hermes.py` â€” Flask blueprint registered in `chatbot_main.py`
- [ ] `core/config.py` has `HERMES_ENABLED`, `HERMES_API_URL`, `HERMES_API_KEY`, `HERMES_TIMEOUT`

## Pre-merge â€” Regression Safety

- [ ] `test_regression_safety.py` passes â€” verifies:
...
## Pre-merge â€” Environment & Config
...
| Hermes dependency conflicts with venv-core | N/A | Sidecar pattern â€” separate process |
...
### Full removal
See [deployment docs â€” rollback section](./deployment_last30days_hermes.md#rollback-procedures).
