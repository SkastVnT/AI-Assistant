...
```
 ...
 ┌────────────────────────────────────────────────────────────────────────┐
 │ Storage tiers:                                                         │
 │   app/storage/character_db/         (registry seed JSON)                   │
 │   app/storage/character_refs/<tag>/ (downloaded references + seen_urls)    │
 │   app/storage/character_loras/<tag>/(LoRA cache + meta)                    │
 │   app/storage/metadata/<job_id>.json(ResultStore manifests — anime path)   │
 │   ComfyUI/output/               (raw images from local ComfyUI)        │
 │   services/chatbot/app/storage/Image_Gen/ (multi-provider gallery)         │
 │   app/storage/{outputs,intermediate,references,prompts}/  (reserved, empty)│
 └────────────────────────────────────────────────────────────────────────┘
 ...
 └──────────────┬───────────────┘         └─────────────────┬──────────────────┘
                ...
                │ app/image_pipeline/anime_pipeline/             │ HTTP proxy
                │   saa_character_db.py                      ▼
...
ComfyUI. Cycles 1–6 of `app/image_pipeline/reasoning/`.
...
- Path B — `app/image_pipeline/anime_pipeline/comfy_client.py` used by
...
|---|---|---|---|
| Registry seed | `app/storage/character_db/` | git (manual) | Source of truth for §4 |
...
`app/storage/references/`, `app/storage/prompts/` — only `.gitkeep`. These were
...
- Index: `app/storage/index/<asset_id>.json` (planned location — verify
  before adopting).
