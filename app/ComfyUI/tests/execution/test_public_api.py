...
class TestPublicAPI:
    ...
    def test_sync_progress_update_executes(
        ...
        g = builder
        image = g.node(
            "StubImage", content="BLACK", height=256, width=256, batch_size=1
        )

        # Use TestSyncProgressUpdate with short sleep
        progress_node = g.node(
            "TestSyncProgressUpdate", value=image.out(0), sleep_seconds=0.5
        )
    ...
    def test_async_progress_update_executes(
        self, client: ComfyClient, builder: GraphBuilder
    ):
        ...
        g = builder
        image = g.node(
            "StubImage", content="WHITE", height=256, width=256, batch_size=1
        )

        # Use TestAsyncProgressUpdate with short sleep
        progress_node = g.node(
            "TestAsyncProgressUpdate", value=image.out(0), sleep_seconds=0.5
        )
        output = g.node("SaveImage", images=progress_node.out(0))
    ...
    def test_sync_and_async_progress_together(
        self, client: ComfyClient, builder: GraphBuilder
    ):
        ...
        sync_progress = g.node(
            "TestSyncProgressUpdate", value=image1.out(0), sleep_seconds=0.3
        )
        async_progress = g.node(
            "TestAsyncProgressUpdate", value=image2.out(0), sleep_seconds=0.3
        )
