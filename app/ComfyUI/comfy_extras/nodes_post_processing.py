    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "blend_factor", default=0.5, min=0.0, max=1.0, step=0.01
                ),
                io.Combo.Input(
                    "blend_mode",
                    options=[
...
class Blur(io.ComfyNode):
    ...
    @classmethod
    def execute(
        cls, image: torch.Tensor, blur_radius: int, sigma: float
    ) -> io.NodeOutput:
        if blur_radius == 0:
...
class Sharpen(io.ComfyNode):
    ...
    @classmethod
    def execute(
        cls, image: torch.Tensor, sharpen_radius: int, sigma: float, alpha: float
    ) -> io.NodeOutput:
        if sharpen_radius == 0:
            return io.NodeOutput(image)
...
class ImageScaleToTotalPixels(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "megapixels", default=1.0, min=0.01, max=16.0, step=0.01
                ),
    ...
    @classmethod
    def execute(
        cls, image, upscale_method, megapixels, resolution_steps
    ) -> io.NodeOutput:
