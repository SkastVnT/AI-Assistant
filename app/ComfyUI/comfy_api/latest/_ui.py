...
class ImageSaveHelper:
        ...
        if cls.hidden.extra_pnginfo:
            for x in cls.hidden.extra_pnginfo:
                metadata.add(
                    ...
                    + b"\0"
                    + json.dumps(cls.hidden.extra_pnginfo[x]).encode(
                        "latin-1", "strict"
                    ),
                    after_idat=True,
                )
    ...
    @staticmethod
    def _create_webp_metadata(
        pil_image: PILImage.Image, cls: type[ComfyNode] | None
    ) -> PILImage.Exif:
        ...
        if cls.hidden.prompt is not None:
            exif_data[0x0110] = "prompt:{}".format(
    ...
    def save_images(
        ...
        full_output_folder, filename, counter, subfolder, _ = (
            folder_paths.get_save_image_path(
                filename_prefix,
                _get_directory_by_folder_type(folder_type),
                images[0].shape[1],
                images[0].shape[0],
            )
        )
    ...
    @staticmethod
    def get_save_images_ui(
        images, filename_prefix: str, cls: type[ComfyNode] | None, compress_level=4
    ) -> SavedImages:
                ...
                if sample_rate > 48000:
                    ...
                    for rate in sorted(AudioSaveHelper._OPUS_RATES):
                        if rate > sample_rate:
                            ...
                            break
                    if (
                        sample_rate not in AudioSaveHelper._OPUS_RATES
                    ):  # Fallback if still not supported
                        sample_rate = 48000
...
class PreviewImage(_UIOutput):
    ...
    def as_dict(self):
        return {"images": self.values, "animated": (self.animated,)}


class PreviewMask(PreviewImage):
    def __init__(
        self,
        mask: PreviewMask.Type,
        animated: bool = False,
        cls: ComfyNode = None,
        **kwargs,
    ):
