    ...
    if is_amd():
        ...
        if not (any((a in arch) for a in AMD_RDNA2_AND_OLDER_ARCH)):
            if os.getenv(AMD_ENABLE_MIOPEN_ENV) != "1":
                torch.backends.cudnn.enabled = (
                    False  # Seems to improve things a lot on AMD
                )
                logging.info(
        ...
        if (
            ...
            if (
                ...
                if torch_version_numeric >= (
                    2,
                    7,
                ):  # works on 2.6 but doesn't actually seem to improve much
                    if any(
                        (a in arch)
                        for a in ["gfx90a", "gfx942", "gfx1100", "gfx1101", "gfx1151"]
                    ):  # TODO: more arches, TODO: gfx950
                        ENABLE_PYTORCH_ATTENTION = True
                if rocm_version >= (7, 0):
        ...
        if torch_version_numeric >= (2, 7) and rocm_version >= (6, 4):
            if any(
                (a in arch) for a in ["gfx1200", "gfx1201", "gfx950"]
            ):  # TODO: more arches, "gfx942" gives error on pytorch nightly 2.10 1013 rocm7.0
...
try:
    if (is_nvidia() or is_amd()) and PerformanceFeature.Fp16Accumulation in args.fast:
        ...
        PRIORITIZE_FP16 = (
            True  # TODO: limit to cards where it actually boosts performance
        )
...
if (
    ...
    and PerformanceFeature.AutoTune in args.fast
):
...
if not args.disable_pinned_memory:
    if is_nvidia() or is_amd():
        if WINDOWS:
            MAX_PINNED_MEMORY = (
                get_total_memory(torch.device("cpu")) * 0.45
            )  # Windows limit is apparently 50%
        else:
            MAX_PINNED_MEMORY = get_total_memory(torch.device("cpu")) * 0.95
        logging.info(
            "Enabled pinned memory {}".format(MAX_PINNED_MEMORY // (1024 * 1024))
        )
...
def should_use_bf16(
    ...
    if is_amd():
        arch = torch.cuda.get_device_properties(device).gcnArchName
        if any(
            (a in arch) for a in AMD_RDNA2_AND_OLDER_ARCH
