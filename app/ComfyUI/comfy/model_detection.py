...
def detect_unet_config(state_dict, key_prefix, metadata=None):
    ...
    if (
        ...
        for key in state_dict_keys:
            if key.startswith("{}joint_blocks.".format(key_prefix)) and key.endswith(
                ...
                layer = key[
                    len("{}joint_blocks.".format(key_prefix)) : -len(
                        ".x_block.attn2.qkv.weight"
                    )
                ]
    ...
    if (
        ...
        dit_config["out_channels"] = out_w.shape[0] // math.prod(
            dit_config["patch_size"]
        )
        if any(
            s.startswith("{}vector_in.".format(key_prefix)) for s in state_dict_keys
        ):
        ...
        if any(
            s.startswith("{}time_r_in.".format(key_prefix)) for s in state_dict_keys
        ):
        ...
        guidance_keys = list(
            filter(
                lambda a: a.startswith("{}guidance_in.".format(key_prefix)),
                state_dict_keys,
            )
        )
        ...
        if "{}vision_in.proj.0.weight".format(key_prefix) in state_dict_keys:
            dit_config["vision_in_dim"] = state_dict[
                "{}vision_in.proj.0.weight".format(key_prefix)
            ].shape[0]
    ...
    if "{}double_blocks.0.img_attn.norm.key_norm.scale".format(
        ...
        dit_config = {}
        if (
            "{}double_stream_modulation_img.lin.weight".format(key_prefix)
            in state_dict_keys
        ):
        ...
        if (
            "{}distilled_guidance_layer.0.norms.0.scale".format(key_prefix)
            in state_dict_keys
            ...
            if (
                f"{key_prefix}nerf_blocks.0.norm.scale" in state_dict_keys
            ):  # Chroma Radiance
                ...
                dit_config["nerf_final_head_type"] = (
                    "conv"
                    if f"{key_prefix}nerf_final_layer_conv.norm.scale"
                    in state_dict_keys
                    else "linear"
                )
            ...
            dit_config["yak_mlp"] = (
                "{}double_blocks.0.img_mlp.gate_proj.weight".format(key_prefix)
                in state_dict_keys
            )
            dit_config["txt_norm"] = (
    ...
    if (
        "{}genre_embedder.weight".format(key_prefix) in state_dict_keys
    ):  # ACE-Step model
        dit_config = {}
    ...
    if "{}t_block.1.weight".format(key_prefix) in state_dict_keys:  # PixArt
        ...
        if pe_key in state_dict_keys:
            ...
            dit_config["pe_interpolation"] = dit_config["input_size"] // (
                512 // 8
            )  # guess
    ...
    if "{}cap_embedder.1.weight".format(key_prefix) in state_dict_keys:  # Lumina 2
        ...
        if dit_config["dim"] == 2304:  # Original Lumina 2
            ...
            ctd_weight = state_dict.get(
                "{}clip_text_pooled_proj.0.weight".format(key_prefix), None
            )
    ...
    if "{}head.modulation".format(key_prefix) in state_dict_keys:  # Wan 2.1
        ...
        if "{}vace_patch_embedding.weight".format(key_prefix) in state_dict_keys:
            ...
            "{}audio_proj.audio_proj_glob_1.layer.bias".format(key_prefix)
            in state_dict_keys
        ):
    ...
    if f"{key_prefix}t_embedder.mlp.2.weight" in state_dict_keys:  # Hunyuan 3D 2.1
        ...
        dit_config["in_channels"] = state_dict[f"{key_prefix}x_embedder.weight"].shape[
            1
        ]
        dit_config["context_dim"] = 1024
        dit_config["hidden_size"] = state_dict[f"{key_prefix}x_embedder.weight"].shape[
            0
        ]
        dit_config["mlp_ratio"] = 4.0
        dit_config["num_heads"] = 16
    ...
    if "{}txt_norm.weight".format(key_prefix) in state_dict_keys:  # Qwen Image
        ...
        dit_config["in_channels"] = state_dict[
            "{}img_in.weight".format(key_prefix)
        ].shape[1]
        dit_config["num_layers"] = count_blocks(
            state_dict_keys, "{}transformer_blocks.".format(key_prefix) + "{}."
        )
        ...
        if (
            "{}time_text_embed.addition_t_embedding.weight".format(key_prefix)
            in state_dict_keys
        ):  # Layered
    ...
    if (
        "{}visual_transformer_blocks.0.cross_attention.key_norm.weight".format(
            key_prefix
        )
        in state_dict_keys
    ):  # Kandinsky 5
        dit_config = {}
        model_dim = state_dict[
            "{}visual_embeddings.in_layer.bias".format(key_prefix)
        ].shape[0]
        ...
        dit_config["time_dim"] = state_dict[
            "{}time_embeddings.in_layer.bias".format(key_prefix)
        ].shape[0]
        dit_config["image_model"] = "kandinsky5"
        dit_config["ff_dim"] = state_dict[
            "{}visual_transformer_blocks.0.feed_forward.in_layer.weight".format(
                key_prefix
            )
        ].shape[0]
        dit_config["visual_embed_dim"] = state_dict[
            "{}visual_embeddings.in_layer.weight".format(key_prefix)
        ].shape[1]
        dit_config["num_text_blocks"] = count_blocks(
            state_dict_keys, "{}text_transformer_blocks.".format(key_prefix) + "{}."
        )
        dit_config["num_visual_blocks"] = count_blocks(
            state_dict_keys, "{}visual_transformer_blocks.".format(key_prefix) + "{}."
        )
        return dit_config
...
def convert_diffusers_mmdit(state_dict, output_prefix=""):
    ...
    if "joint_transformer_blocks.0.attn.add_k_proj.weight" in state_dict:  # AuraFlow
        ...
        sd_map = comfy.utils.auraflow_to_diffusers(
            {"n_double_layers": num_joint, "n_layers": num_joint + num_single},
            output_prefix=output_prefix,
        )
    elif (
        "adaln_single.emb.timestep_embedder.linear_1.bias" in state_dict
        and "pos_embed.proj.bias" in state_dict
    ):  # PixArt
        num_blocks = count_blocks(state_dict, "transformer_blocks.{}.")
        sd_map = comfy.utils.pixart_to_diffusers(
            {"depth": num_blocks}, output_prefix=output_prefix
        )
    elif "x_embedder.weight" in state_dict:  # Flux
        depth = count_blocks(state_dict, "transformer_blocks.{}.")
        depth_single_blocks = count_blocks(state_dict, "single_transformer_blocks.{}.")
        hidden_size = state_dict["x_embedder.bias"].shape[0]
        sd_map = comfy.utils.flux_to_diffusers(
            {
                "depth": depth,
        ...
        "transformer_blocks.0.attn.add_q_proj.weight" in state_dict
        and "pos_embed.proj.weight" in state_dict
    ):  # SD3
        num_blocks = count_blocks(state_dict, "transformer_blocks.{}.")
        depth = state_dict["pos_embed.proj.weight"].shape[0] // 64
        sd_map = comfy.utils.mmdit_to_diffusers(
