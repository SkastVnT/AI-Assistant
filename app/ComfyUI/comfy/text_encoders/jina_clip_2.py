...
class JinaClip2Tokenizer(sd1_clip.SDTokenizer):
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        ...
        super().__init__(
            ...
            pad_token=1,
            end_token=2,
            tokenizer_args={"add_bos": True, "add_eos": True},
...
class XLMRobertaEmbeddings(nn.Module):
    def __init__(self, config, device=None, dtype=None, ops=None):
        ...
        self.word_embeddings = ops.Embedding(
            config.vocab_size,
            embed_dim,
        ...
        self.token_type_embeddings = ops.Embedding(
            config.type_vocab_size, embed_dim, device=device, dtype=dtype
        )
    ...
    def forward(self, input_ids=None, embeddings=None):
        ...
        if embeddings is not None:
            token_type_ids = torch.zeros(
                embeddings.shape[1], device=embeddings.device, dtype=torch.int32
            )
...
class MHA(nn.Module):
    def __init__(self, config, device=None, dtype=None, ops=None):
        ...
        self.rotary_emb = RotaryEmbedding(
            self.head_dim, config.rotary_emb_base, device=device
        )
    ...
    def forward(self, x, mask=None, optimized_attention=None):
        ...
        out = optimized_attention(
            q, k, v, heads=self.num_heads, mask=mask, skip_reshape=True
        )
...
class MLP(nn.Module):
    def __init__(self, config, device=None, dtype=None, ops=None):
        super().__init__()
        self.fc1 = ops.Linear(
            config.hidden_size, config.intermediate_size, device=device, dtype=dtype
        )
        self.activation = F.gelu
        self.fc2 = ops.Linear(
            config.intermediate_size, config.hidden_size, device=device, dtype=dtype
        )
...
class Block(nn.Module):
    def __init__(self, config, device=None, dtype=None, ops=None):
        ...
        self.norm1 = ops.LayerNorm(
            config.hidden_size, eps=config.layer_norm_eps, device=device, dtype=dtype
        )
    ...
    def forward(self, hidden_states, mask=None, optimized_attention=None):
        mixer_out = self.mixer(
            hidden_states, mask=mask, optimized_attention=optimized_attention
        )
...
class XLMRobertaEncoder(nn.Module):
    def __init__(self, config, device=None, dtype=None, ops=None):
        ...
        self.layers = nn.ModuleList(
            [
                Block(config, device=device, dtype=dtype, ops=ops)
    ...
    def forward(self, hidden_states, attention_mask=None):
        optimized_attention = (
            comfy.ldm.modules.attention.optimized_attention_for_device(
                hidden_states.device, mask=attention_mask is not None, small_input=True
            )
        )
...
class XLMRobertaModel(nn.Module):
    def __init__(self, config_dict, dtype, device, operations):
        ...
        self.model = XLMRobertaModel_(
            self.config, device=device, dtype=dtype, ops=operations
        )
...
class JinaClip2TextModel(sd1_clip.SDClipModel):
    def __init__(self, device="cpu", dtype=None, model_options={}):
        super().__init__(
            ...
            special_tokens={"start": 0, "end": 2, "pad": 1},
