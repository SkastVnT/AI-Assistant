...
def precompute_freqs_cis(
    head_dim, position_ids, theta, rope_scale=None, rope_dims=None, device=None
):
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        output = optimized_attention(
            xq, xk, xv, self.num_heads, mask=attention_mask, skip_reshape=True
        )
...
class TransformerBlock(nn.Module):
    def __init__(
        self, config: Llama2Config, index, device=None, dtype=None, ops: Any = None
    ):
        super().__init__()
...
class Llama2_(nn.Module):
    def __init__(self, config, device=None, dtype=None, ops=None):
        ...
        if (
            self.config.transformer_type == "gemma2"
            or self.config.transformer_type == "gemma3"
        ):
    ...
    def forward(
        ...
        if (
            ...
            and final_layer_norm_intermediate
            ...
            if e.get("type") == "image":
                ...
                if position_ids is None:
                    position_ids = torch.zeros(
                        (3, embeds.shape[1]), device=embeds.device
                    )
                    position_ids[:, :start] = torch.arange(
                        0, start, device=embeds.device
                    )
                ...
                position_ids[:, end:] = torch.arange(
                    start_next + offset,
                    start_next + (embeds.shape[1] - end) + offset,
