...
class ByT5SmallTokenizer(sd1_clip.SDTokenizer):
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        tokenizer_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "byt5_tokenizer"
        )
        super().__init__(
            tokenizer_path,
            pad_with_end=False,
            embedding_size=1472,
            embedding_key="byt5_small",
...
class HunyuanImageTokenizer(QwenImageTokenizer):
    ...
    def tokenize_with_weights(self, text: str, return_word_ids=False, **kwargs):
        ...
        if len(text_prompt_texts) > 0:
            out["byt5"] = self.byt5.tokenize_with_weights(
                "".join(map(lambda a: 'Text "{}". '.format(a), text_prompt_texts)),
                return_word_ids,
                **kwargs,
            )
        return out
...
class Qwen25_7BVLIModel(sd1_clip.SDClipModel):
    def __init__(
        ...
        llama_quantization_metadata = model_options.get(
...
class ByT5SmallModel(sd1_clip.SDClipModel):
    def __init__(
        ...
        textmodel_json_config = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "byt5_config_small_glyph.json"
        )
        super().__init__(
            device=device,
            layer=layer,
...
class HunyuanImageTEModel(QwenImageTEModel):
    def __init__(self, byt5=True, device="cpu", dtype=None, model_options={}):
        super(QwenImageTEModel, self).__init__(
            device=device,
            dtype=dtype,
            name="qwen25_7b",
        ...
        if byt5:
            self.byt5_small = ByT5SmallModel(
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        cond, p, extra = super().encode_token_weights(
            token_weight_pairs, template_end=template_end
        )
    ...
    class QwenImageTEModel_(HunyuanImageTEModel):
        def __init__(self, device="cpu", dtype=None, model_options={}):
            ...
            if dtype_llama is not None:
                dtype = dtype_llama
            super().__init__(
                byt5=byt5, device=device, dtype=dtype, model_options=model_options
            )
