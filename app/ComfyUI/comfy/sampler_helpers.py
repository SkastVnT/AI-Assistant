...
def estimate_memory(model, noise_shape, conds):
    ...
    memory_required = model.model.memory_required(
        [noise_shape[0] * 2] + list(noise_shape[1:]), cond_shapes=cond_shapes
    )
    minimum_memory_required = model.model.memory_required(
        [noise_shape[0]] + list(noise_shape[1:]), cond_shapes=cond_shapes_min
    )
    return memory_required, minimum_memory_required


def prepare_sampling(
    model: ModelPatcher, noise_shape, conds, model_options=None, force_full_load=False
):
...
def _prepare_sampling(
    ...
    models += (
        model.get_nested_additional_models()
    )  # TODO: does this require inference_memory update?
...
def prepare_model_patcher(model: ModelPatcher, conds, model_options: dict):
    ...
    for wc_name in ["wrappers", "callbacks"]:
        comfy.patcher_extension.merge_nested_dicts(
            to_load_options.setdefault(wc_name, {}),
