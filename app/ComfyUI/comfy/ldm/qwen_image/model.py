...
class GELU(nn.Module):
    def __init__(
        ...
        self.proj = operations.Linear(
            dim_in, dim_out, bias=bias, dtype=dtype, device=device
        )
...
class FeedForward(nn.Module):
    def __init__(
        ...
        self.net.append(
            operations.Linear(inner_dim, dim_out, bias=bias, dtype=dtype, device=device)
        )
...
class QwenTimestepProjEmbeddings(nn.Module):
    def __init__(
        ...
        if self.use_additional_t_cond:
            self.addition_t_embedding = operations.Embedding(
                2, embedding_dim, device=device, dtype=dtype
            )
    ...
    def forward(self, timestep, hidden_states, addition_t_cond=None):
        ...
        if self.use_additional_t_cond:
            if addition_t_cond is None:
                addition_t_cond = torch.zeros(
                    (timesteps_emb.shape[0]),
                    device=timesteps_emb.device,
                    dtype=torch.long,
                )
            timesteps_emb += self.addition_t_embedding(
                addition_t_cond, out_dtype=timesteps_emb.dtype
            )
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_q = operations.Linear(
            query_dim, self.inner_dim, bias=bias, dtype=dtype, device=device
        )
        self.to_k = operations.Linear(
            query_dim, self.inner_kv_dim, bias=bias, dtype=dtype, device=device
        )
    ...
    def forward(
        ...
        img_query = (
            ...
            .view(batch_size, seq_img, self.heads, -1)
            .transpose(1, 2)
            .contiguous()
        )
        img_key = (
        ...
        txt_query = (
            ...
            .view(batch_size, seq_txt, self.heads, -1)
            .transpose(1, 2)
            .contiguous()
        )
        txt_key = (
...
class QwenImageTransformerBlock(nn.Module):
    def __init__(
        ...
        self.img_mlp = FeedForward(
            dim=dim, dim_out=dim, dtype=dtype, device=device, operations=operations
        )
    ...
    def _apply_gate(self, x, y, gate, timestep_zero_index=None):
        if timestep_zero_index is not None:
            return y + torch.cat(
                (
                    x[:, :timestep_zero_index] * gate[0],
                    x[:, timestep_zero_index:] * gate[1],
                ),
                dim=1,
            )
        else:
    ...
    def _modulate(
        ...
        if timestep_zero_index is not None:
            ...
            reg = torch.addcmul(
                shift.unsqueeze(1), x[:, :timestep_zero_index], 1 + scale.unsqueeze(1)
            )
            zero = torch.addcmul(
                shift_0.unsqueeze(1),
                x[:, timestep_zero_index:],
                1 + scale_0.unsqueeze(1),
            )
            return torch.cat((reg, zero), dim=1), (
                gate.unsqueeze(1),
                gate_0.unsqueeze(1),
            )
        else:
            return torch.addcmul(
                shift.unsqueeze(1), x, 1 + scale.unsqueeze(1)
            ), gate.unsqueeze(1)
    ...
    def forward(
        ...
        img_modulated, img_gate1 = self._modulate(
            self.img_norm1(hidden_states), img_mod1, timestep_zero_index
        )
        del img_mod1
        ...
        hidden_states = self._apply_gate(
        ...
        img_modulated2, img_gate2 = self._modulate(
            self.img_norm2(hidden_states), img_mod2, timestep_zero_index
        )
        hidden_states = self._apply_gate(
            self.img_mlp(img_modulated2), hidden_states, img_gate2, timestep_zero_index
        )
...
class QwenImageTransformer2DModel(nn.Module):
    ...
    def process_img(self, x, index=0, h_offset=0, w_offset=0):
        ...
        return (
            hidden_states,
            repeat(img_ids, "t h w c -> b (t h w) c", b=bs),
            orig_shape,
        )
