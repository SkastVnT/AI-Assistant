...
class TimestepEmbedding(nn.Module):
    def __init__(
        ...
        self.linear_1 = operations.Linear(
            in_channels, time_embed_dim, dtype=dtype, device=device
        )
        self.act = nn.SiLU()
        self.linear_2 = operations.Linear(
            time_embed_dim, time_embed_dim, dtype=dtype, device=device
        )
...
class LuminaRMSNormZero(nn.Module):
    def __init__(
        ...
        self.linear = operations.Linear(
            min(embedding_dim, 1024), 4 * embedding_dim, dtype=dtype, device=device
        )
        self.norm = operations.RMSNorm(
            embedding_dim, eps=norm_eps, dtype=dtype, device=device
        )
...
class LuminaLayerNormContinuous(nn.Module):
    def __init__(
        ...
        self.silu = nn.SiLU()
        self.linear_1 = operations.Linear(
            conditioning_embedding_dim, embedding_dim, dtype=dtype, device=device
        )
        self.norm = operations.LayerNorm(
            embedding_dim, eps, elementwise_affine, dtype=dtype, device=device
        )
        self.linear_2 = (
            operations.Linear(
                embedding_dim, out_dim, bias=True, dtype=dtype, device=device
            )
...
class LuminaFeedForward(nn.Module):
    def __init__(
        ...
        self.linear_1 = operations.Linear(
            dim, inner_dim, bias=False, dtype=dtype, device=device
        )
        self.linear_2 = operations.Linear(
            inner_dim, dim, bias=False, dtype=dtype, device=device
        )
        self.linear_3 = operations.Linear(
            dim, inner_dim, bias=False, dtype=dtype, device=device
        )
...
class Lumina2CombinedTimestepCaptionEmbedding(nn.Module):
    def __init__(
        ...
        self.caption_embedder = nn.Sequential(
            ...
            operations.Linear(
                text_feat_dim, hidden_size, bias=True, dtype=dtype, device=device
            ),
        )
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_q = operations.Linear(
            query_dim, heads * dim_head, bias=bias, dtype=dtype, device=device
        )
        self.to_k = operations.Linear(
            query_dim, kv_heads * dim_head, bias=bias, dtype=dtype, device=device
        )
        ...
        self.to_out = nn.Sequential(
            operations.Linear(
                heads * dim_head, query_dim, bias=bias, dtype=dtype, device=device
            ),
            nn.Dropout(0.0),
        )
...
class OmniGen2TransformerBlock(nn.Module):
    def __init__(
        ...
        if modulation:
            ...
            self.norm1 = operations.RMSNorm(
                dim, eps=norm_eps, dtype=dtype, device=device
            )
...
class OmniGen2RotaryPosEmbed(nn.Module):
    def __init__(
        ...
        self.rope_embedder = EmbedND(
            dim=sum(axes_dim), theta=self.theta, axes_dim=axes_dim
        )
    ...
    def forward(
        ...
        ref_img_freqs_cis = torch.zeros(
            *ref_img_freqs_cis_shape, device=device, dtype=freqs_cis.dtype
        )
...
class OmniGen2Transformer2DModel(nn.Module):
    def __init__(
        ...
        self.ref_image_patch_embedder = operations.Linear(
            patch_size * patch_size * in_channels,
            hidden_size,
            dtype=dtype,
            device=device,
        ...
        self.image_index_embedding = nn.Parameter(
            torch.empty(5, hidden_size, device=device, dtype=dtype)
        )
    ...
    def flat_and_pad_to_seq(self, hidden_states, ref_image_hidden_states):
        ...
        if ref_image_hidden_states is not None:
            ref_image_hidden_states = list(
                map(
                    lambda ref: comfy.ldm.common_dit.pad_to_patch_size(ref, (p, p)),
                    ref_image_hidden_states,
                )
            )
            ref_img_sizes = [
                [
                    (imgs.size(2), imgs.size(3)) if imgs is not None else None
        ...
        if ref_image_hidden_states is not None:
            ...
            for ref_img in ref_image_hidden_states:
                ...
                ref_img = rearrange(
                    ref_img, "b c (h p1) (w p2) -> b (h w) (p1 p2 c)", p1=p, p2=p
                )
                imgs.append(ref_img)
    ...
    def img_patch_embed_and_refine(
        ...
        if ref_image_hidden_states is not None:
            ref_image_hidden_states = self.ref_image_patch_embedder(
                ref_image_hidden_states
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = torch.cat(
            [text_hidden_states, combined_img_hidden_states], dim=1
        )
        attention_mask = None
