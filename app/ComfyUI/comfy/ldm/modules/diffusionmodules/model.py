    ...
    if isinstance(op, CarriedConv3d):
        if conv_carry_in is None:
            ...
            x = torch.nn.functional.pad(
                x, (1, 1, 1, 1, 2 - carry_len, 0), mode="replicate"
            )
    ...
    def forward(self, x, conv_carry_in=None, conv_carry_out=None):
        ...
        if x.ndim == 5 and scale_factor[0] > 1.0:
            ...
            if conv_carry_in is None:
                ...
                results.append(
                    interpolate_up(
                        first.squeeze(2), scale_factor=scale_factor[1:]
                    ).unsqueeze(2)
                )
                x = x[:, :, 1:, :, :]
            if x.shape[2] > 0:
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, temb=None, conv_carry_in=None, conv_carry_out=None):
        ...
        h = conv_carry_causal_3d(
            h, self.conv1, conv_carry_in=conv_carry_in, conv_carry_out=conv_carry_out
        )
        ...
        h = [self.dropout(h)]
        h = conv_carry_causal_3d(
            h, self.conv2, conv_carry_in=conv_carry_in, conv_carry_out=conv_carry_out
        )

        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                x = conv_carry_causal_3d(
                    [x],
                    self.conv_shortcut,
                    conv_carry_in=conv_carry_in,
...
class Encoder(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
        ...
        # downsampling
        self.conv_in = conv_op(in_channels, self.ch, kernel_size=3, stride=1, padding=1)
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                if curr_res in attn_resolutions:
                    attn.append(
                        make_attn(block_in, attn_type=attn_type, conv_op=conv_op)
                    )
            down = nn.Module()
            down.block = block
            down.attn = attn
            if i_level != self.num_resolutions - 1:
                ...
                down.downsample = Downsample(
                    block_in, resamp_with_conv, stride=stride, conv_op=conv_op
                )
                curr_res = curr_res // 2
            self.down.append(down)
        ...
        self.mid.attn_1 = make_attn(
            block_in, attn_type=attn_type, conv_op=mid_attn_conv_op
        )
        self.mid.block_2 = ResnetBlock(
            in_channels=block_in,
            out_channels=block_in,
    ...
    def forward(self, x):
        ...
        if self.carried:
            ...
            if x.shape[2] > self.time_compress:
                ...
                xl += torch.split(
                    x[:, :, 1 : 1 + ((x.shape[2] - 1) // tc) * tc, :, :], tc * 2, dim=2
                )
            x = xl
        else:
            x = [x]
        ...
        for i, x1 in enumerate(x):
            ...
            for i_level in range(self.num_resolutions):
                for i_block in range(self.num_res_blocks):
                    h1 = self.down[i_level].block[i_block](
                        h1, temb, conv_carry_in, conv_carry_out
                    )
                    if len(self.down[i_level].attn) > 0:
                        assert i == 0  # carried should not happen if attn exists
                        h1 = self.down[i_level].attn[i_block](h1)
                if i_level != self.num_resolutions - 1:
                    h1 = self.down[i_level].downsample(
                        h1, conv_carry_in, conv_carry_out
                    )
...
class Decoder(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        # z to block_in
        self.conv_in = conv_op(z_channels, block_in, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, z, **kwargs):
        ...
        for i, h1 in enumerate(h):
            ...
            for i_level in reversed(range(self.num_resolutions)):
                for i_block in range(self.num_res_blocks + 1):
                    h1 = self.up[i_level].block[i_block](
                        h1, temb, conv_carry_in, conv_carry_out, **kwargs
                    )
                    if len(self.up[i_level].attn) > 0:
                        assert i == 0  # carried should not happen if attn exists
