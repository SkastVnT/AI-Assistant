...
class Flux(nn.Module):
        ...
        **kwargs,
        ...
        if params.txt_norm:
            self.txt_norm = RMSNorm(
                params.context_in_dim, dtype=dtype, device=device, operations=operations
            )
        else:
            self.txt_norm = None
    ...
    def process_img(self, x, index=0, h_offset=0, w_offset=0, transformer_options={}):
        ...
        img_ids = torch.zeros(
            (steps_h, steps_w, len(self.params.axes_dim)),
            device=x.device,
            dtype=torch.float32,
        )
        ...
        **kwargs,
        ...
        if ref_latents is not None:
            ...
            ref_latents_method = kwargs.get(
                "ref_latents_method", self.params.default_ref_method
            )
