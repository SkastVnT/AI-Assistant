...
class TimeEmbeddings(nn.Module):
    def __init__(
        self, model_dim, time_dim, max_period=10000.0, operation_settings=None
    ):
        super().__init__()
...
class Modulation(nn.Module):
    def __init__(self, time_dim, model_dim, num_params, operation_settings=None):
        super().__init__()
        self.activation = nn.SiLU()
        self.out_layer = operation_settings.get("operations").Linear(
            time_dim,
...
class SelfAttention(nn.Module):
    def __init__(self, num_channels, head_dim, operation_settings=None):
        ...
        operations = operation_settings.get("operations")
        self.to_query = operations.Linear(
            num_channels,
            num_channels,
            bias=True,
            device=operation_settings.get("device"),
        ...
        self.to_value = operations.Linear(
            num_channels,
            num_channels,
            bias=True,
            device=operation_settings.get("device"),
        ...
        self.query_norm = operations.RMSNorm(
            head_dim,
            device=operation_settings.get("device"),
            dtype=operation_settings.get("dtype"),
        )
        self.key_norm = operations.RMSNorm(
            head_dim,
            device=operation_settings.get("device"),
            dtype=operation_settings.get("dtype"),
        )
    ...
    def _forward(self, x, freqs, transformer_options={}):
        ...
        out = attention(
            q, k, v, self.num_heads, transformer_options=transformer_options
        )
    ...
    def forward(self, x, freqs, transformer_options={}):
        if x.shape[1] > 8192:
            return self._forward_chunked(
                x, freqs, transformer_options=transformer_options
            )
        else:
...
class OutLayer(nn.Module):
    def __init__(
        self, model_dim, time_dim, visual_dim, patch_size, operation_settings=None
    ):
        super().__init__()
        self.patch_size = patch_size
        self.modulation = Modulation(
            time_dim, model_dim, 2, operation_settings=operation_settings
        )
        operations = operation_settings.get("operations")
        self.norm = operations.LayerNorm(
            model_dim,
    ...
    def forward(self, visual_embed, time_embed):
        ...
        return (
            x.permute(0, 4, 1, 5, 2, 6, 3, 7).flatten(2, 3).flatten(3, 4).flatten(4, 5)
        )
...
class TransformerEncoderBlock(nn.Module):
    def __init__(self, model_dim, time_dim, ff_dim, head_dim, operation_settings=None):
        super().__init__()
        self.text_modulation = Modulation(
            time_dim, model_dim, 6, operation_settings=operation_settings
        )
        operations = operation_settings.get("operations")

        self.self_attention_norm = operations.LayerNorm(
            model_dim,
            elementwise_affine=False,
            device=operation_settings.get("device"),
            dtype=operation_settings.get("dtype"),
        ...
        self.self_attention = SelfAttention(
            model_dim, head_dim, operation_settings=operation_settings
        )

        self.feed_forward_norm = operations.LayerNorm(
            model_dim,
            elementwise_affine=False,
            device=operation_settings.get("device"),
            dtype=operation_settings.get("dtype"),
        ...
        self.feed_forward = FeedForward(
            model_dim, ff_dim, operation_settings=operation_settings
        )
...
class TransformerDecoderBlock(nn.Module):
    def __init__(self, model_dim, time_dim, ff_dim, head_dim, operation_settings=None):
        super().__init__()
        self.visual_modulation = Modulation(
            time_dim, model_dim, 9, operation_settings=operation_settings
        )
        ...
        self.cross_attention_norm = operations.LayerNorm(
            model_dim,
            elementwise_affine=False,
            device=operation_settings.get("device"),
            dtype=operation_settings.get("dtype"),
    ...
    def forward(
        self, visual_embed, text_embed, time_embed, freqs, transformer_options={}
    ):
        ...
        visual_out = self.self_attention(
            visual_out, freqs, transformer_options=transformer_options
        )
        ...
        visual_out = self.cross_attention(
            visual_out, text_embed, transformer_options=transformer_options
        )
        visual_embed = apply_gate_sum(visual_embed, visual_out, gate)
        # feed forward
        shift, scale, gate = get_shift_scale_gate(ff_params)
...
class Kandinsky5(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.time_embeddings = TimeEmbeddings(
            model_dim, time_dim, operation_settings=operation_settings
        )
        self.text_embeddings = TextEmbeddings(
            in_text_dim, model_dim, operation_settings=operation_settings
        )
        self.pooled_text_embeddings = TextEmbeddings(
            in_text_dim2, time_dim, operation_settings=operation_settings
        )
        ...
        self.rope_embedder_1d = EmbedND(
            dim=head_dim, theta=10000.0, axes_dim=[head_dim]
        )
    ...
    def rope_encode_3d(
        ...
        if rope_options is not None:
            ...
            if (
                self.model_dim == 4096
            ):  # pro video model uses different rope scaling at higher resolutions
                if h * w >= 14080:
    ...
    def forward_orig(
        ...
        **kwargs,
        ...
        time_embed = self.time_embeddings(
            timestep, x.dtype
        ) + self.pooled_text_embeddings(y)

        for block in self.text_transformer_blocks:
            context = block(
                context, time_embed, freqs_text, transformer_options=transformer_options
            )
    ...
    def _forward(
        ...
        **kwargs,
        ...
        if time_dim_replace is not None:
            time_dim_replace = comfy.ldm.common_dit.pad_to_patch_size(
                time_dim_replace, self.patch_size
            )
            x[:, : time_dim_replace.shape[1], : time_dim_replace.shape[2]] = (
                time_dim_replace
            )

        freqs = self.rope_encode_3d(
            t_len,
            h,
            w,
            device=x.device,
        ...
        freqs_text = self.rope_encode_1d(
            context.shape[1],
            device=x.device,
            dtype=x.dtype,
            transformer_options=transformer_options,
    ...
    def forward(
        ...
        **kwargs,
        ...
        return comfy.patcher_extension.WrapperExecutor.new_class_executor(
            ...
            context,
            y,
            time_dim_replace=time_dim_replace,
            transformer_options=transformer_options,
            **kwargs,
        )
