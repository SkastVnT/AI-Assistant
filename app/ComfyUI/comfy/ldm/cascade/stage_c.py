...
class StageC(nn.Module):
    ...
    def _up_decode(
        self, level_outputs, r_embed, clip, cnet=None, transformer_options={}
    ):
        x = level_outputs[0]
    ...
    def forward(
        ...
        **kwargs,
        ...
        level_outputs = self._down_encode(
            x, r_embed, clip, cnet, transformer_options=transformer_options
        )
        x = self._up_decode(
            level_outputs, r_embed, clip, cnet, transformer_options=transformer_options
        )
        return self.clf(x)
