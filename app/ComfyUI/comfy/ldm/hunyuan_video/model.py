...
class SelfAttentionRef(nn.Module):
    def __init__(
        self, dim: int, qkv_bias: bool = False, dtype=None, device=None, operations=None
    ):
        super().__init__()
...
class TokenRefinerBlock(nn.Module):
    def __init__(self, hidden_size, heads, dtype=None, device=None, operations=None):
        ...
        self.norm1 = operations.LayerNorm(
            hidden_size, elementwise_affine=True, eps=1e-6, dtype=dtype, device=device
        )
        self.self_attn = SelfAttentionRef(
            hidden_size, True, dtype=dtype, device=device, operations=operations
        )
    ...
    def forward(self, x, c, mask, transformer_options={}):
        ...
        q, k, v = qkv.reshape(qkv.shape[0], qkv.shape[1], 3, self.heads, -1).permute(
            2, 0, 3, 1, 4
        )
...
class IndividualTokenRefiner(nn.Module):
    def __init__(
        self, hidden_size, heads, num_blocks, dtype=None, device=None, operations=None
    ):
        super().__init__()
    ...
    def forward(self, x, c, mask, transformer_options={}):
        m = None
        if mask is not None:
            m = mask.view(mask.shape[0], 1, 1, mask.shape[1]).repeat(
                1, 1, mask.shape[1], 1
            )
            m = m + m.transpose(2, 3)
...
class TokenRefiner(nn.Module):
    def __init__(
        ...
        self.input_embedder = operations.Linear(
            text_dim, hidden_size, bias=True, dtype=dtype, device=device
        )
        self.t_embedder = MLPEmbedder(
            256, hidden_size, dtype=dtype, device=device, operations=operations
        )
        self.c_embedder = MLPEmbedder(
            text_dim, hidden_size, dtype=dtype, device=device, operations=operations
        )
    ...
    def forward(
        ...
        t = self.t_embedder(
            timestep_embedding(timesteps, 256, time_factor=1.0).to(x.dtype)
        )
        ...
        x = self.individual_token_refiner(
            x, c, mask, transformer_options=transformer_options
        )
        return x
...
class HunyuanVideo(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.img_in = comfy.ldm.modules.diffusionmodules.mmdit.PatchEmbed(
            None,
            self.patch_size,
            self.in_channels,
            self.hidden_size,
            conv3d=len(self.patch_size) == 3,
            dtype=dtype,
            device=device,
            operations=operations,
        )
        ...
        if self.vision_in_dim is not None:
            ...
            self.vision_in = MLPProj(
                in_dim=self.vision_in_dim,
    ...
    def forward_orig(
        ...
        if (self.time_r_in is not None) and (not disable_time_r):
            ...
            if len(w) > 0:
                ...
                vec_r = self.time_r_in(
                    timestep_embedding(timesteps_r, 256, time_factor=1000.0).to(
                        img.dtype
                    )
                )
        ...
        if guiding_frame_index is not None:
            ...
            if self.vector_in is not None:
                ...
                vec = torch.cat(
                    [
                        (vec_ + token_replace_vec).unsqueeze(1),
                        (vec_ + vec).unsqueeze(1),
                    ],
                    dim=1,
                )
            else:
        ...
        txt = self.txt_in(
            txt, timesteps, txt_mask, transformer_options=transformer_options
        )
        ...
        if self.byt5_in is not None and txt_byt5 is not None:
            ...
            if self.cond_type_embedding is not None:
                cond_emb = self.cond_type_embedding(
                    torch.ones_like(
                        txt_byt5[:, :, 0], device=txt_byt5.device, dtype=torch.long
                    )
                )
                txt_byt5 = txt_byt5 + cond_emb.to(txt_byt5.dtype)
                txt = torch.cat(
                    (txt_byt5, txt), dim=1
                )  # byt5 first for HunyuanVideo1.5
            else:
            ...
            if self.cond_type_embedding is not None:
                cond_emb = self.cond_type_embedding(
                    ...
                    * torch.ones_like(
                        txt_vision_states[:, :, 0],
                        dtype=torch.long,
                        device=txt_vision_states.device,
                    )
                )
                txt_vision_states = txt_vision_states + cond_emb
            txt = torch.cat((txt_vision_states.to(txt.dtype), txt), dim=1)
            extra_txt_ids = torch.zeros(
                (txt_ids.shape[0], txt_vision_states.shape[1], txt_ids.shape[-1]),
                device=txt_ids.device,
                dtype=txt_ids.dtype,
            )
        ...
        img = img.reshape(
            [img.shape[0]] + shape + [self.out_channels] + self.patch_size
        )
        if img.ndim == 8:
    ...
    def img_ids(self, x):
        ...
        img_ids[:, :, :, 0] = img_ids[:, :, :, 0] + torch.linspace(
            0, t_len - 1, steps=t_len, device=x.device, dtype=x.dtype
        ).reshape(-1, 1, 1)
    ...
    def _forward(
        ...
        **kwargs,
        ...
        if len(self.patch_size) == 3:
            ...
            txt_ids = torch.zeros(
                (bs, context.shape[1], 3), device=x.device, dtype=x.dtype
            )
        else:
            img_ids = self.img_ids_2d(x)
            txt_ids = torch.zeros(
                (bs, context.shape[1], 2), device=x.device, dtype=x.dtype
            )
