...
class PointCrossAttention(nn.Module):
    ...
    def sample_points_and_latents(
        self, point_cloud: torch.Tensor, features: torch.Tensor
    ):
        """
        ...
        random_pc, sharpedge_pc = torch.split(
            point_cloud, [self.pc_size, self.pc_sharpedge_size], dim=1
        )
        ...
        random_query_pc, random_input_pc, random_idx_pc, random_idx_query = (
            self.subsample(
                pc=random_pc,
                num_query=num_random_query,
                input_pc_size=input_random_pc_size,
            )
        )
        ...
        if input_sharpedge_pc_size == 0:
            sharpedge_input_pc = torch.zeros(B, 0, D, dtype=random_input_pc.dtype).to(
                point_cloud.device
            )
            sharpedge_query_pc = torch.zeros(B, 0, D, dtype=random_query_pc.dtype).to(
                point_cloud.device
            )
            ...
            (
                ...
                pc=sharpedge_pc,
                num_query=num_sharpedge_query,
                input_pc_size=input_sharpedge_pc_size,
            )
        ...
        if self.point_feats > 0:
            random_surface_features, sharpedge_surface_features = torch.split(
                features, [self.pc_size, self.pc_sharpedge_size], dim=1
            )

            input_random_surface_features, query_random_features = self.handle_features(
                features=random_surface_features,
                idx_pc=random_idx_pc,
                batch_size=B,
                input_pc_size=input_random_pc_size,
                idx_query=random_idx_query,
            )

            if input_sharpedge_pc_size == 0:
                input_sharpedge_surface_features = torch.zeros(
                    B,
                    0,
                    self.point_feats,
                    dtype=input_random_surface_features.dtype,
                    device=point_cloud.device,
                )

                query_sharpedge_features = torch.zeros(
                    B,
                    0,
                    self.point_feats,
                    dtype=query_random_features.dtype,
                    device=point_cloud.device,
                )
            else:
                input_sharpedge_surface_features, query_sharpedge_features = (
                    self.handle_features(
                        idx_pc=sharpedge_idx_pc,
                        features=sharpedge_surface_features,
                        batch_size=B,
                        idx_query=sharpedge_idx_query,
                        input_pc_size=input_sharpedge_pc_size,
                    )
                )

            query_features = torch.cat(
                [query_random_features, query_sharpedge_features], dim=1
            )
            input_features = torch.cat(
                [input_random_surface_features, input_sharpedge_surface_features], dim=1
            )
            ...
            if self.normal_pe:
                ...
                input_features = torch.cat(
                    [input_features_pe, input_features[..., :3]], dim=-1
                )
                query_features = torch.cat(
                    [query_features_pe, query_features[..., :3]], dim=-1
                )
    ...
    def handle_features(
        self, features, idx_pc, input_pc_size, batch_size: int, idx_query
    ):
        ...
        query_features = flattent_input_features[idx_query].view(
            B, -1, flattent_input_features.shape[-1]
        )
...
def sample_pointcloud(mesh, num=200000):
    ...
    return torch.from_numpy(points.astype(np.float32)), torch.from_numpy(
...
def load_surface_sharpedge(
    mesh, num_points=4096, num_sharp_points=4096, sharpedge_flag=True, device="cuda"
):
    ...
    fill_pts, fill_normals = (
        (torch.zeros(0, 3), torch.zeros(0, 3))
        if num_fill == 0
        else sample_pointcloud(mesh_fill, num_fill)
    )
    ...
    def assemble_tensor(points, normals, label=None):
        ...
        if label is not None:
            label_tensor = torch.full(
                (data.shape[0], 1), float(label), dtype=torch.float16
            ).to(device)
            data = torch.cat([data, label_tensor], dim=1)

        return data

    surface = assemble_tensor(
        torch.cat([surf_pts.to(device), fill_pts.to(device)], dim=0),
        torch.cat([surf_normals.to(device), fill_normals.to(device)], dim=0),
        label=0 if sharpedge_flag else None,
    )

    sharp_surface = assemble_tensor(
        torch.from_numpy(sharp_pts),
        torch.from_numpy(sharp_normals),
    ...
    sharp_surface = sharp_surface[
        rng.choice(sharp_surface.shape[0], num_sharp_points, replace=False)
    ]
...
class SharpEdgeSurfaceLoader:
    ...
    def __call__(self, mesh_input, device="cuda"):
        ...
        return load_surface_sharpedge(
            mesh, self.num_uniform_points, self.num_sharp_points, device=device
        )
...
class VanillaVolumeDecoder:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        x = torch.linspace(
            bbox_min[0], bbox_max[0], int(octree_resolution) + 1, dtype=torch.float32
        )
        y = torch.linspace(
            bbox_min[1], bbox_max[1], int(octree_resolution) + 1, dtype=torch.float32
        )
        z = torch.linspace(
            bbox_min[2], bbox_max[2], int(octree_resolution) + 1, dtype=torch.float32
        )

        [xs, ys, zs] = torch.meshgrid(x, y, z, indexing="ij")
        xyz = (
            torch.stack((xs, ys, zs), axis=-1)
        ...
        batch_logits = []
        for start in tqdm(
            range(0, xyz.shape[0], num_chunks),
...
class ShapeVAE(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder = PointCrossAttention(
