...
class MemBlock(nn.Module):
    def __init__(self, n_in, n_out, act_func):
        ...
        self.skip = (
            operations.Conv2d(n_in, n_out, 1, bias=False)
            if n_in != n_out
            else nn.Identity()
        )
        self.act = act_func
...
def apply_model_with_memblocks(model, x, parallel, show_progress_bar):
    ...
    if parallel:
        ...
        work_queue = deque(
            [
                TWorkItem(xt, 0)
                for t, xt in enumerate(x.reshape(B, T * C, H, W).chunk(T, dim=1))
            ]
        )
            ...
            if i == len(model):
                ...
                if isinstance(b, MemBlock):
                    ...
                    for xt_next in reversed(
                        xt.view(B, b.stride * C, H, W).chunk(b.stride, 1)
                    ):
...
class TAEHV(nn.Module):
    def __init__(
        ...
        self.process_in = (
            latent_format().process_in if latent_format is not None else (lambda x: x)
        )
        self.process_out = (
            latent_format().process_out if latent_format is not None else (lambda x: x)
        )
    ...
    def encode(self, x, **kwargs):
        ...
        x = apply_model_with_memblocks(
            self.encoder, x, self.parallel, self.show_progress_bar
        ).movedim(2, 1)
    ...
    def decode(self, x, **kwargs):
        ...
        x = apply_model_with_memblocks(
            self.decoder, x, self.parallel, self.show_progress_bar
        )
        if self.patch_size > 1:
