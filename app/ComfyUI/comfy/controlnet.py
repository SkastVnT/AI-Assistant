...
class ControlBase:
    ...
    def set_cond_hint(
        ...
        cond_hint,
        strength=1.0,
        timestep_percent_range=(0.0, 1.0),
        vae=None,
        extra_concat=[],
    ):
        ...
        if self.latent_format is not None:
            if vae is None:
                logging.warning(
                    "WARNING: no VAE provided to the controlnet apply node when this controlnet requires one."
                )
            self.vae = vae
            ...
            if len(self.extra_concat_orig) > 0:
                ...
                for c in self.extra_concat_orig:
                    ...
                    c = comfy.utils.common_upscale(
                        c,
                        self.cond_hint.shape[-1],
                        self.cond_hint.shape[-2],
                        self.upscale_algorithm,
                        "center",
                    )
                    if c.ndim < self.cond_hint.ndim:
                        c = c.unsqueeze(2)
                        c = comfy.utils.repeat_to_batch_size(
                            c, self.cond_hint.shape[2], dim=2
                        )
                    to_concat.append(
                        comfy.utils.repeat_to_batch_size(c, self.cond_hint.shape[0])
                    )
...
class ControlLora(ControlNet):
    def __init__(
        self, control_weights, global_average_pooling=False, model_options={}
    ):  # TODO? model_options
    ...
    def inference_memory_requirements(self, dtype):
        return comfy.utils.calculate_parameters(
...
def controlnet_config(sd, model_options={}):
    ...
    if operations is None:
        operations = comfy.ops.pick_operations(
            unet_dtype, manual_cast_dtype, disable_fast_fp8=True
        )
...
def load_controlnet_mmdit(sd, model_options={}):
    ...
    (
        ...
        manual_cast_dtype,
        offload_device,
    ) = controlnet_config(new_sd, model_options=model_options)
    ...
    control_model = comfy.cldm.mmdit.ControlNet(
...
class ControlNetSD35(ControlNet):
    def pre_run(self, model, percent_to_timestep_function):
        if self.control_model.double_y_emb:
            missing, unexpected = self.control_model.orig_y_embedder.load_state_dict(
                model.diffusion_model.y_embedder.state_dict(), strict=False
            )
        else:
            missing, unexpected = self.control_model.x_embedder.load_state_dict(
                model.diffusion_model.x_embedder.state_dict(), strict=False
            )
        super().pre_run(model, percent_to_timestep_function)
...
def load_controlnet_sd35(sd, model_options={}):
    ...
    control_model = comfy.cldm.dit_embedder.ControlNetEmbedder(
        img_size=None,
        patch_size=2,
        in_chans=16,
...
def load_controlnet_hunyuandit(controlnet_data, model_options={}):
    (
        ...
        manual_cast_dtype,
        offload_device,
    ) = controlnet_config(controlnet_data, model_options=model_options)

    control_model = comfy.ldm.hydit.controlnet.HunYuanControlNet(
        operations=operations, device=offload_device, dtype=unet_dtype
    )
...
def load_controlnet_flux_xlabs_mistoline(sd, mistoline=False, model_options={}):
    (
        ...
        manual_cast_dtype,
        offload_device,
    ) = controlnet_config(sd, model_options=model_options)
...
def load_controlnet_flux_instantx(sd, model_options={}):
    ...
    control_model = comfy.ldm.flux.controlnet.ControlNetFlux(
        latent_input=True,
        num_union_modes=num_union_modes,
        control_latent_channels=control_latent_channels,
        operations=operations,
        device=offload_device,
        dtype=unet_dtype,
...
def load_controlnet_qwen_instantx(sd, model_options={}):
    ...
    if control_latent_channels == 68:  # inpaint controlnet
        ...
        concat_mask = True
    control_model = comfy.ldm.qwen_image.controlnet.QwenImageControlNetModel(
...
def convert_mistoline(sd):
    return comfy.utils.state_dict_prefix_replace(
        sd, {"single_controlnet_blocks.": "controlnet_single_blocks."}
    )
...
def load_controlnet_state_dict(state_dict, model=None, model_options={}):
    ...
    if (
        ...
        controlnet_config = comfy.model_detection.unet_config_from_diffusers_unet(
            controlnet_data
        )
        ...
        if "double_blocks.0.img_attn.norm.key_norm.scale" in controlnet_data:
            return load_controlnet_flux_xlabs_mistoline(
                controlnet_data, model_options=model_options
            )
        elif "pos_embed_input.proj.weight" in controlnet_data:
            if "transformer_blocks.0.adaLN_modulation.1.bias" in controlnet_data:
                return load_controlnet_sd35(
                    controlnet_data, model_options=model_options
                )  # Stability sd3.5 format
            else:
                return load_controlnet_mmdit(
                    controlnet_data, model_options=model_options
                )  # SD3 diffusers controlnet
        elif "transformer_blocks.0.img_mlp.net.0.proj.weight" in controlnet_data:
            return load_controlnet_qwen_instantx(
                controlnet_data, model_options=model_options
            )
        elif "controlnet_x_embedder.weight" in controlnet_data:
            return load_controlnet_flux_instantx(
                controlnet_data, model_options=model_options
            )

    elif "controlnet_blocks.0.linear.weight" in controlnet_data:  # mistoline flux
        return load_controlnet_flux_xlabs_mistoline(
            convert_mistoline(controlnet_data),
            mistoline=True,
...
def load_controlnet(ckpt_path, model=None, model_options={}):
    ...
    cnet = load_controlnet_state_dict(
        comfy.utils.load_torch_file(ckpt_path, safe_load=True),
        model=model,
        model_options=model_options,
    )
    if cnet is None:
...
def load_t2i_adapter(t2i_data, model_options={}):  # TODO: model_options
    ...
    if "body.0.in_conv.weight" in keys:
        ...
        model_ad = comfy.ldm.cascade.controlnet.ControlNet(
