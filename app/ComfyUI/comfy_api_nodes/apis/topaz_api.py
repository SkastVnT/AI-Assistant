...
class ImageEnhanceRequest(BaseModel):
    ...
    face_enhancement_creativity: float = Field(
        0, description="Is ignored if face_enhancement is false"
    )
    face_enhancement_strength: float = Field(
        0.8, description="Is ignored if face_enhancement is false"
    )
    ...
    prompt: Optional[str] = Field(
        None, description="Text prompt for creative upscaling guidance"
    )
    creativity: int = Field(3, description="Creativity settings range from 1 to 9")
    face_preservation: str = Field(
        "true", description="To preserve the identity of characters"
    )
    color_preservation: str = Field(
        "true", description="To preserve the original color"
    )
...
class VideoEnhancementFilter(BaseModel):
    ...
    focusFixLevel: Optional[str] = Field(
        None, description="Downscales video input for correction of blurred subjects"
    )
    compression: Optional[float] = Field(
        None, description="Strength of compression recovery"
    )
    details: Optional[float] = Field(
        None, description="Amount of detail reconstruction"
    )
    prenoise: Optional[float] = Field(
        None, description="Amount of noise to add to input to reduce over-smoothing"
    )
    ...
    preblur: Optional[float] = Field(
        None, description="Anti-aliasing and deblurring strength"
    )
    ...
    recoverOriginalDetailValue: Optional[float] = Field(
        None, description="Source details into the output video"
    )
    creativity: Optional[str] = Field(
        None, description="Creativity level(high, low) for slc-1 only"
    )
    isOptimizedMode: Optional[bool] = Field(
...
class OutputInformationVideo(BaseModel):
    ...
    audioCodec: Optional[str] = Field(
        ..., description="Required if audioTransfer is Copy or Convert"
    )
