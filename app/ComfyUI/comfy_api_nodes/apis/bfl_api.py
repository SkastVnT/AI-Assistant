...
class BFLFluxExpandImageRequest(BaseModel):
    ...
    top: conint(ge=0, le=2048) = Field(
        ..., description="Number of pixels to expand at the top of the image"
    )
    bottom: conint(ge=0, le=2048) = Field(
        ..., description="Number of pixels to expand at the bottom of the image"
    )
    left: conint(ge=0, le=2048) = Field(
        ..., description="Number of pixels to expand at the left side of the image"
    )
    right: conint(ge=0, le=2048) = Field(
        ..., description="Number of pixels to expand at the right side of the image"
    )
    steps: conint(ge=15, le=50) = Field(
        ..., description="Number of steps for the image generation process"
    )
    guidance: confloat(ge=1.5, le=100) = Field(
        ..., description="Guidance strength for the image generation process"
    )
    safety_tolerance: Optional[conint(ge=0, le=6)] = Field(
        6,
        description="Tolerance level for input and output moderation. Between 0 and 6, 0 being most strict, 6 being least strict. Defaults to 2.",
    )
...
class BFLFluxFillImageRequest(BaseModel):
    ...
    mask: str = Field(
        None,
        description="A Base64-encoded string representing the mask of the areas you with to modify.",
    )
...
class BFLFluxProGenerateRequest(BaseModel):
    ...
    image_prompt: Optional[str] = Field(
        None, description="Optional image to remix in base64 format"
    )
...
class Flux2ProGenerateRequest(BaseModel):
    ...
    safety_tolerance: int | None = Field(
        5,
        description="Tolerance level for input and output moderation. Value 0 being most strict.",
        ge=0,
        le=5,
    )
...
class BFLFluxKontextProGenerateRequest(BaseModel):
    ...
    input_image: Optional[str] = Field(
        None, description="Image to edit in base64 format"
    )
    seed: Optional[int] = Field(None, description="The seed value for reproducibility.")
    guidance: confloat(ge=0.1, le=99.0) = Field(
        ..., description="Guidance strength for the image generation process"
    )
    steps: conint(ge=1, le=150) = Field(
        ..., description="Number of steps for the image generation process"
    )
    safety_tolerance: Optional[conint(ge=0, le=2)] = Field(
        2,
        description="Tolerance level for input and output moderation. Between 0 and 2, 0 being most strict, 6 being least strict. Defaults to 2.",
    )
    output_format: Optional[BFLOutputFormat] = Field(
        BFLOutputFormat.png,
        description="Output format for the generated image. Can be 'jpeg' or 'png'.",
    ...
    aspect_ratio: Optional[str] = Field(
        None, description="Aspect ratio of the image between 21:9 and 9:21."
    )
    prompt_upsampling: Optional[bool] = Field(
        None,
        description="Whether to perform upsampling on the prompt. If active, automatically modifies the prompt for more creative generation.",
...
class BFLFluxProUltraGenerateRequest(BaseModel):
    ...
    raw: Optional[bool] = Field(
        None, description="Generate less processed, more natural-looking images."
    )
    image_prompt: Optional[str] = Field(
        None, description="Optional image to remix in base64 format"
    )
...
class BFLFluxStatusResponse(BaseModel):
    ...
    progress: Optional[float] = Field(
        None, description="The progress of the task (0.0 to 1.0).", ge=0.0, le=1.0
    )
