...
class EnvironmentValidator:
    ...
    def _validate_var(self, env_var: EnvVar) -> Any | None:
        ...
        if env_var.allowed_values and value not in env_var.allowed_values:
            self._errors.append(
                ValidationError(
                    env_var.name, f"Value must be one of: {env_var.allowed_values}"
                )
            )
            return None
        ...
        if env_var.var_type == EnvVarType.PATH and not os.path.exists(parsed):
            self._warnings.append(
                ValidationError(
                    env_var.name, f"Path does not exist: {parsed}", "warning"
                )
            )
    ...
    def validate(self, exit_on_error: bool = True) -> dict[str, Any]:
        ...
        if self._errors:
            error_msg = (
                f"Environment validation failed with {len(self._errors)} error(s)"
            )
        ...
        logger.info(
            f"Environment validation passed ({len(self._vars)} variables checked)"
        )
        return values
...
def create_validator(
    ...
    additional_vars: list[EnvVar] = None,
