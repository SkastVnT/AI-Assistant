...
Scans ``storage/outputs/`` and ``storage/intermediate/`` for image files,
...
emits a curation report under ``storage/metadata/curation/``.
...
Usage::
    ...
    python scripts/curate_image_storage.py \
        --root storage \
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--root",
        default="storage",
        help="Storage root containing outputs/ and intermediate/ (default: storage).",
    )
    p.add_argument(
        "--output-dir",
        default="storage/metadata/curation",
        help="Directory for JSONL + summary outputs (default: storage/metadata/curation).",
    )
    p.add_argument(
        "--avg-vision-sec",
        type=float,
