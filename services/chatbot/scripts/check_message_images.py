...
def check_messages():
    ...
    messages_with_images = db.messages.count_documents(
    ...
    for idx, msg in enumerate(recent_messages, 1):
        ...
        if images:
            ...
            print("      âš ï¸ No images in this message")

        # Check metadata
        metadata = msg.get("metadata", {})
        if metadata:
            print(
                f"   Metadata: model={metadata.get('model')}, cloud_service={metadata.get('cloud_service')}"
            )
