    ...
    addMessage(chatContainer, content, isUser, model, context, timestamp, thinkingProcess = null, customPromptUsed = false, agentConfig = null, attachedFiles = null) {
        ...
        if (isUser && attachedFiles && attachedFiles.length > 0) {
            ...
            attachedFiles.forEach(file => {
                ...
                if (file.preview) {
                    card.innerHTML = `<div class="file-message-preview"><img src="${this.escapeHtml(file.preview)}" alt="" width="80" height="80" style="object-fit:cover;"></div>`;
                } else {
            ...
            if (file.preview) {
                fileCard.innerHTML = `
                    <div class="file-message-preview">
                        <img src="${file.preview}" alt="${this.escapeHtml(file.name)}" width="80" height="80" style="object-fit:cover;">
                    </div>
                `;
            } else {
                const icon = this.getFileIcon(file.type, file.name);
