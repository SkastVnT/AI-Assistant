...
class ConversationManager:
    ...
    def __init__(self, storage_path: str = None, enable_embeddings: bool = True):
        """
        ...
        logger.info(
            f"âœ… ConversationManager initialized with {len(self.conversations_index)} conversations"
        )
    ...
    def _load_embedding_model(self):
        ...
        try:
            ...
            logger.warning(
                "âš ï¸ sentence-transformers not installed. Semantic search disabled."
            )
    ...
    def semantic_search(
        self, query: str, top_k: int = 10, filter_tags: list[str] = None
    ) -> list[dict]:
        """
    ...
    def full_text_search(
        self, query: str, limit: int = 10, filter_tags: list[str] = None
    ) -> list[dict]:
        """
    ...
    def auto_tag_conversation(
        self, conversation_id: str, gemini_model=None
    ) -> list[str]:
        """
...
"""
    ...
    def find_related_conversations(
        self, conversation_id: str, top_k: int = 5
    ) -> list[dict]:
        """
        ...
        query_text = " ".join([msg["content"] for msg in conversation["messages"][:3]])
    ...
    def get_statistics(self) -> dict:
        ...
        total_messages = sum(
            conv.get("message_count", 0) for conv in self.conversations_index.values()
        )
...
if __name__ == "__main__":
    ...
    conv_id = manager.create_conversation(
        title="Python Tutorial", tags=["python", "tutorial", "programming"]
    )
    ...
    manager.add_message(
        conv_id, "assistant", "List comprehensions provide a concise way..."
    )
