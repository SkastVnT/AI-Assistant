...
class StableDiffusionClient:
    ...
    def _calculate_timeout(self, width: int, height: int, steps: int) -> int:
        ...
        timeout = max(
            60, int(base_time * 2) + 30
        )  # At least 60s, with 2x safety margin + 30s overhead
        ...
        try:
            ...
            try:
                response = requests.get(
                    f"{self.api_url}/sdapi/v1/sd-models", timeout=10
                )
                if response.status_code == 200:
    ...
    def get_models(self) -> list[dict]:
        ...
        try:
            ...
            try:
                response = requests.get(
                    f"{self.api_url}/object_info/CheckpointLoaderSimple", timeout=10
                )
                if response.status_code == 200:
                    data = response.json()
                    models = (
    ...
    def get_current_model(self) -> dict:
        ...
        try:
            ...
            if models:
                return {
                    "model": models[0].get("title", "animagine-xl-3.1"),
                    "vae": "Automatic",
                }
    ...
    def change_model(self, model_name: str) -> bool:
        ...
        try:
            ...
            response = requests.post(
                f"{self.api_url}/sdapi/v1/options", json=payload, timeout=60
            )
            response.raise_for_status()
            return True
        except Exception as e:
    ...
    def txt2img(
        ...
        print(
            f"[INFO] Generating {width}x{height} image with {steps} steps (timeout: {timeout}s)"
        )

        # Check if SD WebUI is running
        if not self.check_health():
            return {
                "error": "Stable Diffusion WebUI is not running. Please start it first."
            }

        try:
            response = requests.post(
                f"{self.api_url}/sdapi/v1/txt2img", json=payload, timeout=timeout
            )
            response.raise_for_status()
            ...
            return {
                "error": "Image generation timed out. Try reducing image size or steps, or check if the image service is responding."
            }
            ...
            return {
                "error": "A network error occurred while generating the image. Please try again later."
            }
        except Exception as e:
    ...
    def img2img(
        ...
        timeout = self._calculate_timeout(width, height, steps)
        print(
            f"[INFO] Img2Img: {width}x{height}, {steps} steps, denoising={denoising_strength}, timeout={timeout}s"
        )

        try:
            response = requests.post(
                f"{self.api_url}/sdapi/v1/img2img", json=payload, timeout=timeout
            )
            response.raise_for_status()
            print("[SUCCESS] Img2Img generated successfully!")
            return response.json()
        except requests.exceptions.RequestException as e:
...
def get_sd_client(api_url: str = None) -> StableDiffusionClient:
    ...
    if _sd_client is None or getattr(_sd_client, "api_url", "").rstrip(
        "/"
    ) != api_url.rstrip("/"):
