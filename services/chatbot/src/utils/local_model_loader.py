...
class LocalModelLoader:
    ...
    def load_model(self, model_key):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to load AutoTokenizer, trying PreTrainedTokenizerFast: {e}"
                )
            ...
            if device == "cuda":
                ...
                if config.get("quantize", False):
                    logger.info("Loading with 8-bit quantization to save VRAM...")
                ...
                # CPU mode
                logger.warning("No GPU detected, loading on CPU (will be slow)")
                model = AutoModelForCausalLM.from_pretrained(  # nosec B615  # local_files_only=True prevents remote downloads
                    model_path,
                    device_map="cpu",
                    trust_remote_code=True,
    ...
    def generate(
        self, model_key, messages, system_prompt="", max_tokens=None, temperature=0.7
    ):
