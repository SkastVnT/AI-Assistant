...
class MultimodalHandler:
    ...
    def __init__(
        ...
        speech2text_url: str | None = None,
        document_intelligence_url: str | None = None,
    ):
        """
    ...
    def _analyze_with_gemini(
        self, image_path: str, prompt: str, language: str = "vi"
    ) -> dict:
...
"""
        ...
        response = self.gemini_client.models.generate_content(
            model="grok-3", contents=[enhanced_prompt, image]
        )
    ...
    def _analyze_with_gpt4_vision(
        ...
        # Create prompt
        lang_instruction = (
            "Respond in Vietnamese." if language == "vi" else "Respond in English."
        )
    ...
    def analyze_document(
        self, document_path: str, analysis_type: str = "full", language: str = "vi"
    ) -> dict:
        """
        ...
        try:
            ...
            with open(document_path, "rb") as f:
                files = {"file": f}
                data = {"analysis_type": analysis_type, "language": language}
            ...
            logger.warning(
                "Document Intelligence service not available, using fallback"
            )
    ...
    def transcribe_audio(
        ...
        try:
            ...
            with open(audio_path, "rb") as f:
                ...
                response = requests.post(
                    f"{self.speech2text_url}/upload", files=files, timeout=30
                )
            ...
            while elapsed < max_wait:
                response = requests.get(
                    f"{self.speech2text_url}/status/{job_id}", timeout=10
                )
    ...
    def analyze_multimodal(
        self, inputs: list[dict], query: str, language: str = "vi"
    ) -> dict:
        """
        Combined multimodal analysis
            ...
            for input_item in inputs:
                ...
                if input_type == "image":
                    result = self.analyze_image(
                        input_item["path"], prompt=query, language=language
                    )
                    processed_inputs.append({"type": "image", "result": result})

                elif input_type == "audio":
                    result = self.transcribe_audio(
                        input_item["path"], language=language
                    )
                    processed_inputs.append({"type": "audio", "result": result})

                elif input_type == "document":
                    result = self.analyze_document(
                        input_item["path"], language=language
                    )
                    processed_inputs.append({"type": "document", "result": result})

                elif input_type == "text":
                    processed_inputs.append(
                        {"type": "text", "content": input_item["content"]}
                    )

            # Combine insights with AI
            combined_response = self._combine_multimodal_insights(
                processed_inputs, query, language
            )
    ...
    def _combine_multimodal_insights(
        self, processed_inputs: list[dict], query: str, language: str = "vi"
    ) -> dict:
...
"""
    ...
    def get_capabilities(self) -> dict:
        ...
        return {
            ...
            "audio": {"enabled": True, "service_url": self.speech2text_url},
            "document": {
                "enabled": True,
                "service_url": self.document_intelligence_url,
            },
            "multimodal": {"enabled": True, "max_inputs": 10},
        }
