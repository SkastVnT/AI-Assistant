...
class OCRIntegration:
    ...
    def extract_text_from_image(
        ...
        if openai_key:
            try:
                return self._ocr_openai(
                    base64_image, media_type, filename, openai_key, ocr_prompt
                )
        ...
        if gemini_key:
            try:
                return self._ocr_gemini(
                    base64_image, media_type, filename, gemini_key, ocr_prompt
                )
            except Exception as e:
        ...
        if grok_key:
            try:
                return self._ocr_grok(
                    base64_image, media_type, filename, grok_key, ocr_prompt
                )
            except Exception as e:
    ...
    def extract_text_from_pdf(
        self, pdf_data: bytes, filename: str = "document.pdf"
    ) -> dict[str, Any]:
        ...
        try:
            ...
            with pdfplumber.open(io.BytesIO(pdf_data)) as pdf:
                ...
                if combined:
                    ...
                    logger.info(
                        f"[OCR] pdfplumber extracted {len(combined)} chars from {filename}"
                    )
                    return result
                result["pages"] = len(pdf.pages)
        except ImportError:
        ...
        for lib_name, import_path in [("pypdf", "pypdf"), ("PyPDF2", "PyPDF2")]:
            try:
                ...
                if combined:
                    ...
                    logger.info(
                        f"[OCR] {lib_name} extracted {len(combined)} chars from {filename}"
                    )
                    return result
                result["pages"] = len(reader.pages)
        ...
        logger.info(
            f"[OCR] Text extraction empty for {filename} ({page_count} pages), trying Vision OCR..."
        )
        try:
            ...
            if ocr_texts:
                ...
                if combined:
                    ...
                    logger.info(
                        f"[OCR] Vision OCR extracted {len(combined)} chars from {filename}"
                    )
                    return result
        except Exception as e:
    ...
    def _ocr_pdf_pages(
        self, pdf_data: bytes, filename: str, max_pages: int = 10
    ) -> list:
        ...
        try:
            ...
            images = convert_from_bytes(
                pdf_data, first_page=1, last_page=max_pages, dpi=200
            )
        except ImportError:
        ...
        for i, img in enumerate(images):
            ...
            result = self.extract_text_from_image(
                img_bytes, f"{filename}_page{i + 1}.png"
            )
    ...
    def process_file(
        self, file_data: bytes, filename: str, content_type: str = None
    ) -> dict[str, Any]:
    ...
    def _extract_via_markitdown(
        ...
        try:
            ...
            if text:
                logger.info(
                    f"[OCR] markitdown extracted {len(text)} chars from {filename}"
                )
                return {
                    "success": True,
                    "text": text,
            ...
            logger.warning(
                "[OCR] markitdown not installed — run: pip install 'markitdown[all]'"
            )
            return {
                "success": False,
                "text": "",
