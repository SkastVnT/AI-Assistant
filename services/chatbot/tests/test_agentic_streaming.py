...
class TestCouncilEvent:
    ...
    def test_all_stages(self):
        for stage in EventStage:
            e = CouncilEvent(
                run_id="r", stage=stage, role="x", status=EventStatus.started
            )
    ...
    def test_all_statuses(self):
        for status in EventStatus:
            e = CouncilEvent(
                run_id="r", stage=EventStage.planning, role="x", status=status
            )
...
class TestCouncilEventEmitter:
    ...
    async def test_emit_and_consume(self):
        ...
        await emitter.emit(
            stage=EventStage.planning, role="planner", status=EventStatus.started
        )
        await emitter.emit(
            stage=EventStage.planning,
            role="planner",
    ...
    async def test_emit_returns_event(self):
        ...
        event = await emitter.emit(
            stage=EventStage.failed, role="orch", status=EventStatus.completed
        )
        await emitter.close()
...
class TestOrchestratorEmitsEvents:
    ...
    async def test_full_pipeline_emits_events(self):
        ...
        research = ResearcherOutput(
            evidence=[EvidenceItem(source="llm", content="data")], summary="ok"
        )
        critique = CriticOutput(quality_score=9, verdict="pass")
        synth = SynthesizerOutput(
            answer=FinalAnswer(content="Final answer", confidence=0.9)
        )
        ...
        with (
            patch.object(
                orch._planner, "execute", side_effect=_make_agent_mock(plan, "planner")
            ),
            patch.object(
                orch._researcher,
                "execute",
                side_effect=_make_agent_mock(research, "researcher"),
            ),
            patch.object(
                orch._critic,
                "execute",
                side_effect=_make_agent_mock(critique, "critic"),
            ),
            patch.object(
                orch._synthesizer,
                "execute",
                side_effect=_make_agent_mock(synth, "synthesizer"),
            ),
        ):
