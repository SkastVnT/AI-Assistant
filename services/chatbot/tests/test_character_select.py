...
def _enable(monkeypatch):
    ...
    monkeypatch.setattr(
        "core.character_select_adapter.CHARACTER_SELECT_URL", "http://fake-saa:51028"
    )
...
class TestAdapter:
    ...
    def test_enabled_port_closed(self, _enable, monkeypatch):
        monkeypatch.setattr(
            "core.character_select_adapter._port_is_open", lambda *a, **kw: False
        )
    ...
    def test_enabled_port_open_http_ok(self, _enable, monkeypatch):
        monkeypatch.setattr(
            "core.character_select_adapter._port_is_open", lambda *a, **kw: True
        )
