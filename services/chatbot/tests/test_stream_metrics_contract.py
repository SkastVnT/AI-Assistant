...
def test_stream_metrics_records_lifecycle_events():
    ...
    assert (
        after["totals"]["completed_requests"]
        >= before["totals"]["completed_requests"] + 1
    )
    assert (
        after["totals"]["errored_requests"] >= before["totals"]["errored_requests"] + 1
    )
    assert (
        after["totals"]["fallback_to_standard"]
        >= before["totals"]["fallback_to_standard"] + 1
    )
    assert (
        after["totals"]["near_token_limit"] >= before["totals"]["near_token_limit"] + 1
    )
