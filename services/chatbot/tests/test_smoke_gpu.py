...
def _fake_local_result(*, url="http://127.0.0.1:8188/view?filename=out.png", seed=100):
...
def _fake_remote_result(
    *, url="https://fal.run/test/fallback.png", provider="fal", seed=200
):
    from core.image_gen.providers.base import ImageResult
...
class TestSmokeGPU(unittest.TestCase):
    ...
    def test_04_generate_uses_local(self):
        ...
        print(
            f"\n✅ 4: Local provider used = {result.provider}, cost=${result.cost_usd}"
        )
    ...
    def test_05_local_images_are_localhost_urls(self):
        ...
        self.assertTrue(
            any("127.0.0.1" in u for u in result.images_url),
    ...
    def test_06_remote_fallback_when_local_down(self):
        ...
        self.assertGreater(
            len(names), 0, "Remote providers should be available as fallback"
        )
    ...
    def test_07_generate_remote_when_local_down(self):
        ...
        print(f"\n✅ 7: Remote fallback → {result.provider}, cost=${result.cost_usd}")
    ...
    def test_08_multi_turn_local_stays_local(self):
        ...
        with patch(
            ...
            gen = _fake_local_result(
                url="http://127.0.0.1:8188/view?f=gen.png", seed=10
            )
            ed1 = _fake_local_result(
                url="http://127.0.0.1:8188/view?f=ed1.png", seed=11
            )
            ed2 = _fake_local_result(
                url="http://127.0.0.1:8188/view?f=ed2.png", seed=12
            )
    ...
    def test_09_quality_mode_prefers_remote(self):
        ...
        if names:
            self.assertNotEqual(
                names[0], "comfyui", f"ComfyUI should not be first in QUALITY: {names}"
            )
