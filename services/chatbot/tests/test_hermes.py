...
class TestHermesChat:
    ...
    def test_non_200_response(self, _enable_hermes, monkeypatch):
        ...
        monkeypatch.setattr(
            "core.hermes_adapter.requests.post", lambda *a, **kw: mock_resp
        )
        from core.hermes_adapter import hermes_chat
...
class TestHermesReasoningBridge:
    ...
    def test_keyword_helper_rejects_normal_chat(self):
        ...
        assert not _has_image_keyword(
            "ảnh hưởng của thuế quan là gì"
        )  # 'ảnh' as substring
    ...
    def test_normal_chat_does_not_redirect(self, client, hermes_called, monkeypatch):
        ...
        monkeypatch.setattr(
            "core.image_intent.is_reasoning_pipeline_enabled", lambda: True
        )
        # Classifier should never even be called; if it is, return None.
        monkeypatch.setattr(
            "core.image_intent.detect_image_intent", lambda *a, **kw: None
        )
    ...
    def test_image_request_redirects_to_pipeline(
        self, client, hermes_called, monkeypatch
    ):
    ...
    def test_pipeline_exception_falls_back_to_hermes(
        self, client, hermes_called, monkeypatch
    ):
    ...
    def test_pipeline_disabled_no_classifier_call(
        self, client, hermes_called, monkeypatch
    ):
        """When REASONING_PIPELINE=false, classifier must not run."""
        monkeypatch.setattr(
            "core.image_intent.is_reasoning_pipeline_enabled", lambda: False
        )
