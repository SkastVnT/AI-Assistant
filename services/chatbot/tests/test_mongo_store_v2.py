...
class TestActivityLogging:
    def test_save_conversation_inserts_with_schema_v2(self, fake_db):
        ...
        assert (
            call.args[1]["$setOnInsert"] == {"created_at": body["updated_at"]}
            or "created_at" in call.args[1]["$setOnInsert"]
        )
    ...
    def test_save_tool_call_then_update_lifecycle(self, fake_db):
        ...
        out2 = mongo_store.update_tool_call(
            "tc-1",
            {
                "status": "completed",
...
class TestStreamingFailSafe:
    def test_stream_module_imports_with_mongo_store_disabled(self, monkeypatch):
        ...
        out_conv = mongo_store.save_conversation(
            {"conversation_id": "c1-failsafe-test"}
        )
