    ...
    def mock_db_with_data(self):
        ...
        def slow_find(*args, **kwargs):
            ...
            cursor.__iter__ = lambda self: iter(
                [{"_id": f"conv_{i}", "title": f"Chat {i}"} for i in range(10)]
            )
            return cursor
    ...
    def test_cache_memory_usage(self):
        ...
        for i in range(1000):
            ChatbotCache.set_conversation(
                f"mem_test_{i}", {"_id": f"mem_test_{i}", "data": "x" * 100}
            )
...
class TestConcurrentPerformance:
    ...
    def test_concurrent_repository_operations(self):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(create_and_read, i) for i in range(5)]
            for future in as_completed(futures):
                future.result()
...
class TestBulkOperations:
    ...
    def test_bulk_memory_search_performance(self):
        ...
        mock_cursor.__iter__ = lambda self: iter(
            [{"_id": f"mem_{i}", "content": f"Memory {i}"} for i in range(10)]
        )
