...
class TestDecideRefineAction:
    ...
    def test_pose_failure_boosts_control(self, config):
        ...
        critique = CritiqueReport(
            ...
            background_score=7,
            pose_score=2,
            pose_issues=["wrong pose direction"],
        )
...
class TestPatchPlan:
    def test_denoise_adjustment_clamped(self, config, bad_critique, beauty_pass_config):
        ...
        decision = decide_refine_action(bad_critique, round_num=0, config=config)
        new_pc = patch_plan_from_critique(
            beauty_pass_config, bad_critique, decision, config
        )
        assert (
            config.refine_denoise_floor
            <= new_pc.denoise
            <= config.refine_denoise_ceiling
        )
    ...
    def test_negative_patching(self, config, bad_critique, beauty_pass_config):
        ...
        assert (
            "bad anatomy" in new_pc.negative_prompt
            or "extra fingers" in new_pc.negative_prompt
        )
    ...
    def test_prompt_patch_merged(self, config, beauty_pass_config):
        ...
        new_pc = patch_plan_from_critique(
            beauty_pass_config, critique, decision, config
        )
