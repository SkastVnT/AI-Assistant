...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "_mongodb_config", mongodb_config_path
    )
...
class ConversationDB:
    ...
    def get_user_conversations(
        user_id: str, include_archived: bool = False, limit: int = 20
    ) -> list[dict]:
        ...
        conversations = (
            db.conversations.find(query).sort("updated_at", DESCENDING).limit(limit)
        )
    ...
    @staticmethod
    def update_conversation(conversation_id: str, update_data: dict) -> bool:
        """Update conversation"""
        db = get_db()

        update_data["updated_at"] = datetime.utcnow()
        result = db.conversations.update_one(
            {"_id": _parse_id(conversation_id)}, {"$set": update_data}
        )
    ...
    @staticmethod
    def increment_message_count(conversation_id: str, tokens: int = 0) -> bool:
        ...
        result = db.conversations.update_one(
            ...
            {
                "$inc": {"total_messages": 1, "total_tokens": tokens},
    ...
    def archive_conversation(conversation_id: str) -> bool:
        ...
        return ConversationDB.update_conversation(
            conversation_id, {"is_archived": True}
        )
...
class MessageDB:
    ...
    def get_conversation_messages(
        ...
        query = db.messages.find({"conversation_id": _parse_id(conversation_id)}).sort(
    ...
    def mark_stopped(message_id: str) -> bool:
        ...
        result = db.messages.update_one(
            {"_id": _parse_id(message_id)}, {"$set": {"is_stopped": True}}
        )
    ...
    def get_message_versions(message_id: str) -> list[dict]:
        ...
        versions = db.messages.find({"parent_message_id": _parse_id(message_id)}).sort(
            "version", ASCENDING
        )
...
class MemoryDB:
    ...
    def get_user_memories(
        user_id: str, tags: list[str] | None = None, limit: int = 50
    ) -> list[dict]:
        ...
        memories = (
            db.chatbot_memory.find(query).sort("created_at", DESCENDING).limit(limit)
        )

        return list(memories)

    @staticmethod
    def search_memories(user_id: str, search_text: str, limit: int = 20) -> list[dict]:
        ...
        memories = (
            db.chatbot_memory.find(
                {"user_id": user_id, "$text": {"$search": search_text}},
    ...
    def rate_memory(memory_id: str, rating: int) -> bool:
        ...
        result = db.chatbot_memory.update_one(
            {"_id": _parse_id(memory_id)}, {"$set": {"rating": rating}}
        )
    ...
    def get_public_memories(
        ...
        memories = (
            ...
            .sort([("rating", DESCENDING), ("created_at", DESCENDING)])
            .limit(limit)
        )
...
class FileDB:
    ...
    @staticmethod
    def get_user_files(
        user_id: str, file_type: str | None = None, limit: int = 50
    ) -> list[dict]:
        ...
        files = (
            db.uploaded_files.find(query).sort("created_at", DESCENDING).limit(limit)
        )
...
class UserSettingsDB:
    ...
    def update_settings(user_id: str, settings_update: dict) -> bool:
        ...
        result = db.user_settings.update_one(
            {"user_id": user_id}, {"$set": settings_update}, upsert=True
        )
...
def cleanup_old_data(days: int = 90):
    ...
    old_conversations = db.conversations.find(
        {"is_archived": True, "updated_at": {"$lt": cutoff_date}}
    )
    ...
    return {"deleted_conversations": deleted_conv, "deleted_messages": deleted_msg}
