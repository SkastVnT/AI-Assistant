...
# Note: load_shared_env re-reads the env file each time it is called, but
# does NOT override variables that are already set in the environment.  This
# call ensures that scripts importing mongodb_config directly (without going
# through chatbot_main) still receive env vars from app/config/.env.
# MongoDB Atlas URI
...
class MongoDBClient:
    ...
    def connect(self):
        ...
        if self._client is None:
            uri = (
                MONGODB_X509_URI
                if (MONGODB_X509_ENABLED and MONGODB_X509_URI)
                else MONGODB_URI
            )
            ...
            try:
                ...
                self._client = MongoClient(uri, **connect_kwargs)
                # Test connection
                self._client.admin.command("ping")
                self._db = self._client[DATABASE_NAME]
                print(
                    f"âœ… Successfully connected to MongoDB Atlas - Database: {DATABASE_NAME}"
                )
                return True
            except Exception as e:
                print(
                    f"âš ï¸ MongoDB connection failed (app will run without DB): {str(e)[:100]}"
                )
                self._client = None
                self._db = None
                return False
