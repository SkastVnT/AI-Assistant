...
MODEL_FALLBACKS = {
    ...
    "gemma4": ["deepseek", "step-flash", "gemini"],
...
def get_user_friendly_error(
    error: Exception, model: str = "unknown", language: str = "vi"
) -> str:
...
def get_error_stats() -> dict:
    ...
    return {
        **_error_stats,
        "most_failing_model": (
            max(_error_stats["by_model"].items(), key=lambda x: x[1])[0]
            if _error_stats["by_model"]
            else None
        ),
        "most_common_error": (
            max(_error_stats["by_type"].items(), key=lambda x: x[1])[0]
            if _error_stats["by_type"]
            else None
        ),
    }
                ...
                jsonify(
                    {
                        ...
                        "model": model,
                        "fallbacks": MODEL_FALLBACKS.get(
                            model, ["gemini", "step-flash"]
                        ),
                        "timestamp": datetime.now().isoformat(),
                    }
                ),
