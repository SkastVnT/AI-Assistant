        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    logger.warning(
                        f"[Retry] Attempt {attempt + 1} failed: {e}. Retrying in {delay:.1f}s..."
                    )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        raise Exception(
                            "BloomVN is loading, try again in 20-30 seconds"
                        )
...
class AsyncChatbotAgent:
    ...
    def _build_messages(self, ctx: ChatContext, system_prompt: str) -> list[dict]:
    ...
    async def chat(self, model: str, ctx: ChatContext, prompts_getter) -> ChatResponse:
        """Execute async chat"""
        ...
        try:
            ...
            temperature = (
                config.temperature_deep if ctx.deep_thinking else config.temperature
            )
            max_tokens = (
                config.max_tokens_deep if ctx.deep_thinking else config.max_tokens
            )
    ...
    async def chat_stream(
        self, model: str, ctx: ChatContext, prompts_getter
    ) -> AsyncGenerator[str, None]:
        """Execute async streaming chat"""
        handler = self._handlers.get(model)
        config = self.config_map.get(model)
...
async def run_multiple_chats(
    tasks: list[dict[str, Any]], agent: AsyncChatbotAgent, prompts_getter
) -> list[ChatResponse]:
    """
    ...
    """
    coroutines = [
        agent.chat(task["model"], task["ctx"], prompts_getter) for task in tasks
    ]
