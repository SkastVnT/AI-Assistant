...
class ProviderKeys:
    ...
    google_search_1: str | None = field(
        default_factory=lambda: os.getenv("GOOGLE_SEARCH_API_KEY_1")
    )
    google_search_2: str | None = field(
        default_factory=lambda: os.getenv("GOOGLE_SEARCH_API_KEY_2")
    )
...
class HermesSettings:
    ...
    api_url: str = field(
        default_factory=lambda: os.getenv("HERMES_API_URL", "http://localhost:8080")
    )
...
class ReasoningPipelineSettings:
    ...
    max_panels: int = field(
        default_factory=lambda: _int("REASONING_PIPELINE_MAX_PANELS", 9)
    )
...
class Last30DaysSettings:
    ...
    script_path: str = field(
        default_factory=lambda: os.getenv("LAST30DAYS_SCRIPT_PATH", "")
    )
    python_path: str = field(
        default_factory=lambda: os.getenv("LAST30DAYS_PYTHON_PATH", "")
    )
...
class StableDiffusionSettings:
    api_url: str = field(
        default_factory=lambda: os.getenv("SD_API_URL", "http://127.0.0.1:7861")
    )
...
class Settings:
    ...
    character_select: CharacterSelectSettings = field(
        default_factory=CharacterSelectSettings
    )
    reasoning_pipeline: ReasoningPipelineSettings = field(
        default_factory=ReasoningPipelineSettings
    )
    last30days: Last30DaysSettings = field(default_factory=Last30DaysSettings)
    stable_diffusion: StableDiffusionSettings = field(
        default_factory=StableDiffusionSettings
    )
