    ...
    def create_sse_response(generator: Generator[str, None, None]) -> Response:
        ...
        def generate_events():
            try:
                # Send initial event
                yield StreamEvent(
                    event="start", data=json.dumps({"status": "started"})
                ).format()
                ...
                for chunk in generator:
                    if chunk:
                        full_response += chunk
                        yield StreamEvent(
                            event="chunk", data=json.dumps({"content": chunk})
                        ).format()
                ...
                yield StreamEvent(
                    event="done",
                    data=json.dumps(
                        {"status": "complete", "full_response": full_response}
                    ),
                ).format()
    ...
    def create_json_stream_response(
        ...
        def generate_events():
            try:
                ...
                yield StreamEvent(
                    event="error", data=json.dumps({"error": str(e), "model": model})
                ).format()
...
class NonStreamingToStreaming:
    ...
    @staticmethod
    def simulate_stream(
        full_response: str, chunk_size: int = 10, delay_ms: int = 0
    ) -> Generator[str, None, None]:
        """
