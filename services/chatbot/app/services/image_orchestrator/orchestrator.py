...
def _get_legacy_intent():
    from core.image_gen.intent import ImageIntent as CoreIntent
    from core.image_gen.intent import IntentResult, detect_intent
...
class ImageOrchestrationService:
    ...
    def _run_new_pipeline(
        ...
        if (
            scene.width == 1024
            and scene.height == 1024
            and (core_intent_result.width != 1024 or core_intent_result.height != 1024)
        ):
            scene = replace(
                scene,
        ...
        if not img_result.success:
            ...
            raise RuntimeError(f"Provider returned failure: {img_result.error}")
    ...
    def _fallback_to_legacy(
        ...
        try:
            ...
            logger.error(
                f"[ImageOrchestrationService] Legacy fallback also failed: {leg_exc}"
            )
            return ImageGenerationResult(
                is_image=False,
                fallback_to_llm=True,
        ...
        if result.intent != ImageIntent.NONE:
            yield {
                ...
                "data": {
                    ...
                    "is_edit": result.intent
                    in (ImageIntent.EDIT, ImageIntent.FOLLOWUP_EDIT),
                },
            }
...
_service_instance: ImageOrchestrationService | None = None
