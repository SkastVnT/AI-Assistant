...
_LIGHTING: list[tuple[str, str]] = [
    ...
    (
        r"cloudy|nhiều mây|overcast",
        "overcast sky, soft diffused light, no harsh shadows",
    ),
    (
        r"tối hơn|darker sky|trời tối",
        "darker atmosphere, low-key lighting, dramatic shadows",
    ),
...
_ENVIRONMENT: list[tuple[str, str]] = [
    ...
    (
        r"rừng nhiệt đới|tropical forest|jungle",
        "tropical jungle, dense canopy, lush vegetation",
    ),
    ...
    (
        r"sa mạc|desert|sand dune|cát",
        "desert landscape, rolling sand dunes, dry heat shimmer",
    ),
    ...
    (
        r"cyberpunk|tương lai|futuristic|sci-fi",
        "futuristic sci-fi city, neon signs, high-tech",
    ),
    (
        r"vũ trụ|space|galaxy|cosmos|thiên hà",
        "outer space, star field, nebula, cosmic scale",
    ),
    (
        r"đại dương sâu|deep ocean|underwater",
        "underwater scene, bioluminescent light, coral",
    ),
...
_COMPOSITION: list[tuple[str, str]] = [
    (
        r"cận cảnh (?:khuôn )?mặt|extreme close.?up face",
    ...
    (
        r"nhìn từ trên|bird.?s eye|aerial view|từ trên cao",
        "bird's eye view, top-down perspective",
    ),
    (
        r"góc thấp|low angle|nhìn từ dưới lên",
        "low angle shot, dramatic perspective from below",
    ),
    (
        r"góc nhìn người thứ nhất|first.?person|pov shot",
        "first-person POV, immersive perspective",
    ),
...
_CAMERA: list[tuple[str, str]] = [
    ...
    (
        r"telephoto|tele|200mm|zoom lens",
        "telephoto compression, 200mm, compressed depth",
    ),
    (r"tilt.?shift", "tilt-shift lens, selective focus, miniature effect"),
    (
        r"anamorphic|letterbox|cinemascope",
        "anamorphic lens, horizontal lens flares, cinematic",
    ),
    (r"bokeh|xóa phông|nền mờ", "portrait lens, strong bokeh, blurred background"),
    (
        r"chụp phim|film camera|analog",
        "analog film camera, film grain, film photography",
    ),
...
_VI_EDIT_VERBS: list[tuple[str, str]] = [
    ...
    # REMOVE
    (
        r"(?:bỏ|xóa|loại bỏ|loại|cắt bỏ|gỡ bỏ|xóa bỏ)\s+(?:cái\s+|con\s+|chiếc\s+)?",
        "remove",
    ),
    ...
    (
        r"(?:tối hơn|sáng hơn|to hơn|nhỏ hơn|mờ hơn|rõ hơn|brighter|darker|bigger|smaller)\s*$",
        "modify_general",
    ),
    (
        r"(?:make (?:it |)?(?:darker|lighter|brighter|bigger|smaller|softer))",
        "modify_general",
    ),
    # KEEP (consistency with change)
    (r"giữ\s+(?:nhân vật|người|character|subject).+(?:nhưng|but)", "keep_then_change"),
    (
        r"(?:keep|maintain)\s+(?:the\s+)?(?:character|subject|person).+(?:but|however)",
        "keep_then_change",
    ),
]
...
class ScenePlanner:
    ...
    def classify_and_plan(
        ...
        language: str = "vi",
        has_previous_image: bool = False,
        previous_scene: SceneSpec | None = None,
    ...
    def _fill_scene(
        ...
        scene = dc_replace(
            ...
            wants_consistency_with_previous=wants_consist
            or scene.wants_consistency_with_previous,
    ...
    def _detect_edit_ops(self, message: str, lower: str) -> list[EditOperation]:
        ...
        if replace_m:
            ...
            ops.append(
                EditOperation(
                    ...
                    t_en,
                    new_value=v_en,
                    raw_target=replace_m.group("target"),
                )
            )
            return ops
        ...
        if modify_m:
            ...
            ops.append(
                EditOperation(
                    "modify",
                    t_en,
                    modifier=modifier_en,
                    raw_target=modify_m.group("target"),
                )
            )
            return ops
        ...
        if standalone:
            ...
            ops.append(
                EditOperation(
                    "modify_general", "lighting/atmosphere", modifier=modifier_en
                )
            )
            return ops
        ...
        for op in ops:
            ...
            # ── change / replace ──────────────────────────────────────
                ...
                if _is_hair_target(raw_t):
                    ...
                    attrs = [
                        a
                        for a in attrs
                        if "hair" not in a.lower()
                        or any(c in a.lower() for c in ["ears", "style"])
                    ]
                    if v:
                    ...
                    attrs = [
                        a
                        for a in attrs
                        if raw_t.lower() not in a.lower() and t.lower() not in a.lower()
                    ]
                    if v:
                        attrs.append(v)
            ...
            # ── modify_general ────────────────────────────────────────
                ...
                if "darker" in modifier:
                    fields["lighting"] = (
                        existing + ", darker atmosphere, low-key"
                        if existing
                        else "darker atmosphere, dramatic shadows"
                    )
                elif "brighter" in modifier or "lighter" in modifier:
                    fields["lighting"] = (
                        existing + ", brighter" if existing else "bright natural light"
                    )
                else:
    ...
    def _extract_subject(
        self, message: str, lower: str, language: str, is_edit: bool
    ) -> str:
    ...
    def _extract_style(self, lower: str) -> str | None:
    ...
    def _extract_seed(self, lower: str) -> int | None:
    ...
    def _extract_negative_hints(self, lower: str, style: str | None) -> list[str]:
        hints: list[str] = []
        if style == "anime":
...
def _is_background_target(raw: str) -> bool:
    return bool(
        re.search(r"nền|phông|background|bầu trời|sky(?:\s+not)?\b", raw, re.IGNORECASE)
    )
...
def merge_scene_delta(
    ...
    merged = dc_replace(
        base_scene,
        strength=0.65,
        wants_consistency_with_previous=any(op.operation == "keep" for op in edit_ops)
