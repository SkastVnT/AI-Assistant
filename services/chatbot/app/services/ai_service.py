...
class AIService:
    ...
    def chat(
        ...
        try:
            return self._dispatch_to_provider(
                message, model_config, system_prompt, history, images
            )
        except ValueError:
            raise
            ...
            if fallback_name:
                ...
                return self._dispatch_to_provider(
                    message, fallback_config, system_prompt, history, images
                )
            raise
        ...
        if provider in ("openai", "deepseek", "xai"):
            if provider == "openai":
                ...
                client = openai.OpenAI(
                    api_key=self.deepseek_key, base_url=model_config["base_url"]
                )
            else:  # xai
                client = openai.OpenAI(
                    api_key=self.grok_key, base_url=model_config["base_url"]
                )
            ...
            try:
                ...
                logger.warning(
                    f"[AIService] Streaming failed for {model}, falling back: {e}"
                )
                return self.chat(
                    message=message,
                    model=model,
    ...
    def _chat_openai(
        self, message: str, config: dict, system_prompt: str, history: list[dict] | None
    ) -> dict[str, Any]:
    ...
    def _chat_deepseek(
        ...
        try:
            client = openai.OpenAI(
                api_key=self.deepseek_key, base_url=config["base_url"]
            )
    ...
    def _chat_grok(
        ...
        try:
            client = openai.OpenAI(api_key=self.grok_key, base_url=config["base_url"])
    ...
    def _chat_gemini(
        ...
        try:
            ...
            response = client.models.generate_content(
                model=config["model_id"], contents=full_prompt
            )
    ...
    def _get_fallback_model_excluding(self, excluded: str) -> str | None:
        ...
        for model_name, config in self.models.items():
            if (
                ...
                and config["provider"] != excluded_provider
            ):
                return model_name
        return None
