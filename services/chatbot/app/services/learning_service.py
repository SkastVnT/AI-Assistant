...
class LearningService:
    ...
    def __init__(self):
        ...
        self.local_data_path = (
            Path(__file__).parent.parent.parent / "local_data" / "learning"
        )
    ...
    def submit(
        ...
        logger.info(
            f"📚 Learning data submitted: {entry['_id']} (quality: {quality_score})"
        )

        return entry

    def submit_qa_pair(
        self, question: str, answer: str, source: str, auto_approve: bool = False
    ) -> dict[str, Any]:
    ...
    def archive_conversation(self, conversation: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(
            f"📁 Conversation archived: {archive_entry['_id']} (learn: {should_learn})"
        )

        return archive_entry

    def extract_qa_pairs(
        self, messages: list[dict], auto_approve: bool = False
    ) -> dict[str, Any]:
        """
            ...
            try:
                with open(file_path, encoding="utf-8") as f:
                    ...
                    if (
                        ...
                        and entry.get("is_approved") != is_approved
                    ):
                        continue
    ...
    def approve(self, data_id: str) -> dict[str, Any]:
        ...
        return self._update_entry(
            data_id, {"is_approved": True, "reviewed_at": datetime.now().isoformat()}
        )
    ...
    def list_archived_conversations(
        self, should_learn: bool | None = None
    ) -> list[dict[str, Any]]:
        ...
        for file_path in self.archive_path.glob("*.json"):
            try:
                with open(file_path, encoding="utf-8") as f:
                    ...
                    if (
                        should_learn is not None
                        and conv.get("should_learn") != should_learn
                    ):
                        continue
        ...
        return sorted(
            conversations, key=lambda x: x.get("deleted_at", ""), reverse=True
        )
    ...
    def _calculate_qa_quality(self, question: str, answer: str) -> float:
        ...
        if "```" in answer:  # Has code blocks
            score += 0.1
        if any(
            word in answer.lower() for word in ["ví dụ", "example", "để", "because"]
        ):
            score += 0.05
