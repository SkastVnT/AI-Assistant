...
class MemoryRepository(BaseRepository):
    ...
    def search_memories(
        self, user_id: str, query: str, limit: int = 10, min_importance: float = 0.0
    ) -> list[dict[str, Any]]:
        """
    ...
    def get_user_memories(
        ...
        """
        query = {"user_id": user_id, "importance": {"$gte": min_importance}}
    ...
    def update_importance(
        ...
        Returns:
            Updated memory
        """
        return self.update(memory_id, {"importance": min(1.0, max(0.0, importance))})

    def increment_access_count(self, memory_id: str) -> dict[str, Any] | None:
        """
    ...
    def get_by_tags(
        ...
        """
        return self.get_many(
            query={"user_id": user_id, "tags": {"$in": [t.lower() for t in tags]}},
            sort=[("importance", DESCENDING)],
            limit=limit,
        )

    def get_important_memories(
        self, user_id: str, threshold: float = 0.7, limit: int = 20
    ) -> list[dict[str, Any]]:
        """
        ...
        """
        return self.get_many(
            query={"user_id": user_id, "importance": {"$gte": threshold}},
            sort=[("importance", DESCENDING)],
            limit=limit,
        )
    ...
    def cleanup_expired(self) -> int:
        ...
        """
        return self.delete_many({"expires_at": {"$lte": datetime.utcnow()}})

    def decay_old_memories(
        self, user_id: str, days: int = 30, decay_factor: float = 0.9
    ) -> int:
        """
