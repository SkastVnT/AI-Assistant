        ...
        pipeline = [
            {"$match": match},
            {
                "$facet": {
                    ...
                    "data": [{"$sort": sort}, {"$skip": skip}, {"$limit": per_page}],
                }
            },
        ]
...
class BulkOperations:
    ...
    def flush_all(self) -> dict[str, int]:
        """Flush all buffers"""
        return {"inserted": self.flush_inserts(), "updated": self.flush_updates()}
...
def cached_query(cache_key_prefix: str, ttl: int = 3600, cache_none: bool = False):
    """
...
class ConnectionPool:
    ...
    def __init__(self):
        self._stats = {"connections_created": 0, "connections_reused": 0, "errors": 0}
...
class IndexManager:
    ...
    RECOMMENDED_INDEXES = {
        ...
        "messages": [
            ...
            {
                "keys": [("conversation_id", 1), ("is_deleted", 1)],
                "name": "idx_conv_deleted",
            },
        ],
        "memories": [
            {
                "keys": [("user_id", 1), ("importance", -1)],
                "name": "idx_user_importance",
            },
    ...
    def analyze_indexes(cls, db, collection_name: str) -> dict[str, Any]:
        ...
        result = {"collection": collection_name, "indexes": [], "recommendations": []}
        ...
        for rec in recommended:
            if rec["name"] not in existing_names:
                result["recommendations"].append({"action": "create", "index": rec})
