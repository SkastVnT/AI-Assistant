...
from core.config import DEEPSEEK_API_KEY, IMAGE_STORAGE_DIR, OPENAI_API_KEY
...
def sd_models():
    ...
    try:
        ...
        try:
            ...
            current = sd_client.get_current_model()
            model_titles = [
                model.get("title", model.get("model_name", "Unknown"))
                for model in models
            ]
            return jsonify({"models": model_titles, "current_model": current["model"]})
...
def sd_change_model():
    ...
    try:
        ...
        if success:
            return jsonify(
                {"success": True, "message": f"ÄÃ£ Ä‘á»•i model thÃ nh {model_name}"}
            )
        else:
            return (
...
def sd_preset_detail(preset_id):
    ...
    try:
        if PRESETS_AVAILABLE:
            ...
            if preset:
                return jsonify(
                    {"success": True, "preset_id": preset_id, "preset": preset}
                )
            else:
    ...
    try:
        ...
        if _username:
            try:
                ...
                logging.getLogger(__name__).warning(f"[sd] quota check: {_qe}")
        # -----------------------------------------------------------------
        ...
        params = {
            ...
            or preset_config.get("negative_prompt", "bad quality, blurry, distorted"),
            ...
            "cfg_scale": float(
                data.get("cfg_scale") or preset_config.get("cfg_scale", 7.0)
            ),
        ...
        logger.info(
            f"[TEXT2IMG] Generating with model: {params.get('model')}, prompt: {prompt[:50]}..."
        )
        ...
        if use_comfyui:
            ...
            if image_bytes:
                ...
                logger.error(
                    f"[TEXT2IMG] generate_image returned None for prompt: {prompt[:80]}"
                )
                return (
                    jsonify(
                        {
                            "error": "ComfyUI failed to generate image. Check chatbot logs for details."
                        }
                    ),
                    500,
                )
        ...
        if save_to_storage:
            ...
            if saved_filenames:
                _save_image_to_mongodb(
                    saved_filenames, cloud_urls, prompt, params, "text2img"
                )
        ...
        response_data = {
            ...
            "image": (
                (saved_filenames[0] if saved_filenames else base64_images[0])
                if (saved_filenames or base64_images)
                else None
            ),
...
def img2img():
    ...
    try:
        ...
        sd_client = get_comfyui_client(sd_api_url)
        logger.info(
            f"[IMG2IMG] Generating with denoising={params['denoising_strength']}"
        )
        ...
        if save_to_storage:
            ...
            if saved_filenames:
                _save_image_to_mongodb(
                    saved_filenames, cloud_urls, prompt, params, "img2img"
                )
            ...
            if CLOUD_UPLOAD_ENABLED:
                try:
                    ...
                    upload_result = uploader.upload_image(
                        str(filepath), title=f"AI: {prompt[:50]}"
                    )
...
def _save_image_to_mongodb(filenames, cloud_urls, prompt, params, model_type):
    ...
    try:
        from core.image_storage import save_to_firebase, save_to_mongodb
        ...
        logger.info(
            "[SD] Persisted %s %s image(s) to generated_images",
            len(filenames),
            model_type,
        )
...
def _process_prompt_result(result_json, tags, model_name):
    ...
    if not generated_negative:
        generated_negative = (
            "nsfw, nude, sexual, explicit, bad quality, blurry, worst quality"
        )
...
def sd_capabilities():
    ...
    return jsonify({"available": True, "capabilities": gen.get_capabilities()})
...
def sd_inpaint():
    ...
    if gen is None or not gen.is_available:
        return (
            jsonify({"error": "Inpainting not available â€” SD WebUI API offline"}),
            503,
        )
...
def sd_controlnet():
    ...
    if gen is None or not gen.is_available:
        return (
            jsonify({"error": "ControlNet not available â€” SD WebUI API offline"}),
            503,
        )
    ...
    if data.get("save_to_storage") and images:
        saved_filenames, cloud_urls = _save_images_to_storage(
            images, "controlnet", prompt, data
        )
        if MONGODB_ENABLED and saved_filenames:
            _save_image_to_mongodb(
                saved_filenames, cloud_urls, prompt, data, "controlnet"
            )
...
def sd_upscale():
    ...
    if gen is None or not gen.is_available:
        return (
            jsonify({"error": "Upscaling not available â€” SD WebUI API offline"}),
            503,
        )
    ...
    if data.get("save_to_storage") and image_out:
        saved_filenames, cloud_urls = _save_images_to_storage(
            [image_out], "upscaled", "upscale", data
        )
...
def sd_outpaint():
    ...
    if gen is None or not gen.is_available:
        return (
            jsonify({"error": "Outpainting not available â€” SD WebUI API offline"}),
            503,
        )
...
def sd_batch():
    ...
    try:
        ...
        for seed in seeds:
            ...
            if use_comfyui:
                ...
                if isinstance(res, dict) and res.get("error"):
                    logger.error(
                        f"[BATCH] Image generation error for seed {seed}: {res.get('error')}"
                    )
                    continue
        ...
        if data.get("save_to_storage") and all_b64:
            saved_filenames, cloud_urls = _save_images_to_storage(
                all_b64, "batch", prompt, data
            )
            if MONGODB_ENABLED and saved_filenames:
                _save_image_to_mongodb(
                    saved_filenames, cloud_urls, prompt, data, "batch"
                )
...
def get_cost_log():
    ...
    return jsonify(
        {"costs": costs[-50:], "total_usd": round(total, 4), "count": len(costs)}
    )
    ...
    try:
            ...
            if prompt_id2:
                ...
                if output2 and isinstance(output2, dict):
                    for node_output in output2.values():
                        caption = node_output.get("caption", "") or node_output.get(
                            "text", ""
                        )
        ...
        if not caption:
            return (
                jsonify(
                    {
                        "error": "No interrogation nodes available in ComfyUI (install WD14 Tagger or BLIP node)"
                    }
                ),
                422,
            )
