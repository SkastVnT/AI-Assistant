...
from core.base_chat import ChatContext, ModelConfig, ModelProvider
...
def chat_async_stream():
    ...
    try:
        ...
        if not message:
            return Response(
                StreamEvent(
                    event="error", data=json.dumps({"error": "Empty message"})
                ).format(),
    ...
    try:
        ...
        async def process_batch():
            ...
            for req in requests_list:
                ...
                tasks.append({"model": req.get("model", "grok"), "ctx": ctx})
