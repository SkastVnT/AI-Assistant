...
class LogOperation:
    ...
    def __init__(self, operation: str, logger: logging.Logger | None = None, **extra):
    ...
    def __enter__(self):
        ...
        self.logger.debug(
            ...
            extra={**self.extra, "operation": self.operation, "status": "started"},
        )
        return self
...
class LogEvents:
    ...
    def conversation_created(
        logger: logging.Logger, user_id: str, conversation_id: str, model: str
    ):
        logger.info(
    ...
    @staticmethod
    def message_sent(
        logger: logging.Logger, conversation_id: str, role: str, length: int
    ):
        logger.info(
            "Message sent",
            extra={
    ...
    def cache_hit(logger: logging.Logger, cache_key: str):
        logger.debug("Cache hit", extra={"event": "cache_hit", "cache_key": cache_key})

    @staticmethod
    def cache_miss(logger: logging.Logger, cache_key: str):
        logger.debug(
            "Cache miss", extra={"event": "cache_miss", "cache_key": cache_key}
        )

    @staticmethod
    def db_query(
        logger: logging.Logger, collection: str, operation: str, duration_ms: float
    ):
        logger.debug(
            "Database query",
            extra={
    ...
    def error(logger: logging.Logger, error_type: str, message: str, **extra):
        logger.error(
            ...
            extra={"event": "error", "error_type": error_type, **extra},
        )
