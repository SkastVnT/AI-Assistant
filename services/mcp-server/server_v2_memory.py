...
memory = get_memory_manager(db_path=RESOURCES_DIR / "memory" / "mcp_memory.db")
...
WARMUP_CONTEXT_CACHE = os.getenv(
    "MCP_WARMUP_CONTEXT_CACHE", "true"
...
def _safe_resolve_path(path_str: str, base_dir: Path) -> Path | None:
    """
...
def _run_startup_maintenance() -> None:
    ...
    try:
        ...
        logger.info(
            "Startup maintenance cleanup=%s hard_delete=%s",
            cleanup_stats,
            hard_delete_stats,
        )
    except Exception as e:
...
def search_files(
    ...
    target_exts = extensions.get(file_type)
...
def list_directory(
    directory_path: str = ".", show_hidden: bool = False
) -> dict[str, Any]:
    """
    ...
    if not full_path.exists():
        return _error_response(
            "DIRECTORY_NOT_FOUND", f"Directory not found: {directory_path}"
        )
...
def get_project_info() -> dict[str, Any]:
    ...
    services = [
        ...
        {"name": "Text2SQL", "port": 5002, "path": "services/text2sql"},
        {
            ...
            "port": 5003,
            "path": "services/document-intelligence",
        },
...
def search_logs(
    service_name: str, level: str = "ALL", max_lines: int = 100
) -> dict[str, Any]:
    """
    ...
    if not log_file.exists():
        return {"service": service_name, "error": f"Log file not found: {log_file}"}
...
def write_file(
    file_path: str, content: str, mode: str = "overwrite", create_dirs: bool = True
) -> dict[str, Any]:
    """
    ...
    if resolved is None:
        return _error_response(
            "INVALID_PATH", f"Invalid or restricted path: {file_path}"
        )
    ...
    try:
        ...
        return _error_response(
            "WRITE_FAILED", f"Write failed: {str(e)}", {"file_path": file_path}
        )
...
def git_diff(file_path: str | None = None) -> dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, cwd=BASE_DIR, timeout=15
        )
        return {
            "file": file_path,
            "diff": result.stdout,
...
def http_request(
    ...
    try:
        ...
        return _error_response(
            "HTTP_REQUEST_FAILED", f"Request failed: {str(e)}", retryable=True
        )
...
@_safe_tool
def search_memory(
    query: str, limit: int = 10, min_importance: int = 0
) -> dict[str, Any]:
    """
...
def get_recent_context(
    limit: int = 30, min_importance: int = 5, max_chars: int = 12000
) -> dict[str, Any]:
    """
    ...
    context_text = memory.get_context_for_session(
        max_observations=limit, min_importance=min_importance, max_chars=max_chars
    )
...
def clear_memory_context_cache(
    context_type: str | None = None, older_than_hours: int | None = None
) -> dict[str, Any]:
    """
    ...
    return memory.clear_context_cache(
        context_type=context_type, older_than_hours=older_than_hours
    )
...
def run_memory_maintenance(
    retention_days: int = 90, hard_delete_days: int = 365, vacuum: bool = False
) -> dict[str, Any]:
    """
...
@mcp.prompt()
def explain_code(file_path: str, function_name: str | None = None) -> str:
    """
...
def main():
    ...
    print("ðŸš€ Starting AI-Assistant MCP Server V2.0 WITH MEMORY...")
    print(f"ðŸ“ Base Directory: {BASE_DIR}")
    print(f"ðŸ§  Memory Database: {memory.db_path}")
    print(f"ðŸ“‹ Session ID: {SESSION_ID}")
    print("\nâœ¨ NEW FEATURES:")
    print("   ðŸ”§ Tools: All original tools + 11 memory tools")
    print("   ðŸ“¦ Resources: Config, docs + memory context")
    print("   ðŸ’¬ Prompts: Code review, debug, explain + session summary")
    print("   ðŸ§  MEMORY: Persistent storage, search history, AI observations")
    print("\nâœ… Server is ready!")
    ...
    try:
        ...
        print("\n\nðŸ›‘ Shutting down...")
        print("ðŸ’¾ Saving session summary...")
