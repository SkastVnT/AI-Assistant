...
class MemoryManager:
    ...
    def _normalize_text(self, value: str | None, max_len: int = 10000) -> str:
    ...
    def _normalize_list(
        self, value: list[str] | None, max_items: int = 50, max_len: int = 300
    ) -> list[str]:
    ...
    def _find_duplicate_observation(
        self, tool_name: str, observation: str, within_hours: int = 24
    ) -> str | None:
    ...
    def _build_context_cache_key(
        self, query: str, limit: int, min_importance: int, max_chars: int
    ) -> str:
    ...
    def _get_cached_context(
        self, project_name: str, context_type: str, cache_key: str
    ) -> dict[str, Any] | None:
    ...
    def clear_context_cache(
        self, context_type: str | None = None, older_than_hours: int | None = None
    ) -> dict[str, Any]:
    ...
    def get_context_cache_stats(self) -> dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            total = conn.execute("SELECT COUNT(*) AS c FROM memory_context").fetchone()[
                "c"
            ]
            active = conn.execute(
    ...
    def save_observation(
        ...
        normalized_type = (
            self._normalize_text(observation_type or "general", max_len=64).lower()
            or "general"
        )
        ...
        normalized_output = (
            self._normalize_text(tool_output, max_len=6000) if tool_output else None
        )

        duplicate_id = self._find_duplicate_observation(
            tool_name, normalized_observation
        )
        if duplicate_id:
    ...
    def search_observations(
        self, query: str, limit: int = 10, min_importance: int = 0
    ) -> list[dict[str, Any]]:
        """
    ...
    def get_recent_observations(
        self, limit: int = 50, min_importance: int = 0
    ) -> list[dict[str, Any]]:
    ...
    def get_observations_by_type(
        self, obs_type: str, limit: int = 20
    ) -> list[dict[str, Any]]:
    ...
    def get_context_for_session(
        ...
        observations = self.get_recent_observations(
            limit=max_observations, min_importance=min_importance
        )
    ...
    def get_relevant_context(
        ...
        cache_key = self._build_context_cache_key(
            query, limit, min_importance, max_chars
        )
        ...
        for i, obs in enumerate(hits, 1):
            ...
            lines.append(
                f"   tool: {obs.get('tool_name', 'unknown')} | time: {obs.get('timestamp', '')}"
            )
            lines.append("")
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                "SELECT AVG(importance) as avg_importance FROM observations"
            )
            avg_row = cursor.fetchone()
            totals["avg_importance"] = (
                round(avg_row["avg_importance"], 2)
                if avg_row and avg_row["avg_importance"] is not None
                else 0
            )
            ...
            """)
            totals["unique_observation_types"] = cursor.fetchone()[
