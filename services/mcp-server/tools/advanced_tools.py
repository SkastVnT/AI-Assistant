...
from .guard import clamp_max_rows, validate_select_only, validate_workspace_path
...
def git_branch_info() -> dict[str, Any]:
    ...
    try:
        ...
        branches = [
            b.strip().replace("* ", "") for b in result.stdout.strip().split("\n")
        ]
...
def query_sqlite_database(
    ...
    try:
        ...
        if query.strip().upper().startswith("SELECT"):
            ...
            return {"query": query, "rows": len(results), "data": results}
        else:
...
def list_database_tables(db_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"database": db_path, "tables": table_info, "total_tables": len(tables)}
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                for alias in node.names:
                    imports.append(
                        {"from": module, "import": alias.name, "alias": alias.asname}
                    )
...
def find_todos_in_code(
    directory: str = ".", pattern: str = r"#\s*TODO:?\s*(.+)"
) -> dict[str, Any]:
    """
...
def count_lines_in_project(extensions: list[str] = None) -> dict[str, Any]:
    ...
    try:
        ...
        for ext in extensions:
            ext_stats = {"files": 0, "lines": 0, "code": 0, "comments": 0, "blank": 0}

            for file_path in base_dir.rglob(f"*{ext}"):
                # Skip venv, node_modules
                if any(
                    p in file_path.parts
                    for p in [".venv", "venv", "node_modules", "__pycache__"]
                ):
                    continue
