...
export function enablePanelMode(id, opts = {}) {
    ...
    const applyState = () => {
        ...
        if (saved) {
            ...
            // Clamp width: never smaller than 380px or the natural width
            const minW = 380;
            const w = saved.w ? Math.max(minW, saved.w) : null;
            // Clamp position: keep at least 200px of the panel visible
            const safeLeft = Math.min(Math.max(0, saved.left), vw - 200);
            const safeTop  = Math.min(Math.max(0, saved.top),  vh - 80);
            content.style.setProperty('--panel-top',  safeTop + 'px');
            content.style.setProperty('--panel-left', safeLeft + 'px');
            content.style.setProperty('--panel-tx', '0px'); // clear centering transform
            if (w)      content.style.setProperty('--panel-w', w + 'px');
