...
`desktop/electron/payload/` before the first build.
...
Verify: `desktop\electron\payload\python311\python.exe -c "import venv, pip, ssl; print('ok')"`
...
To enable updates:
1. Bump `version` in `desktop/electron/package.json` (e.g. `0.1.0` → `0.1.1`).
...
- Curated `storage/` (character DBs, prompts, references)
...
- `private/`, `character_select_stand_alone_app-main/` (sidecars, opt-in)
